...
Toto téma popisuje podporu v F# pro provádění výpočtů asynchronně, to znamená, bez blokování provádění jiné práce. Například asynchronních výpočtů je možné psát aplikace, které mají uživatelská rozhraní, která nadále reagovat na uživatele, jelikož aplikace provede další práci.
...
Kromě `let!`, můžete použít `use!` provádět asynchronní vazby. Rozdíl mezi `let!` a `use!` je stejný jako rozdíl mezi `let` a `use`. Pro `use!`, vyřazení objektu z na konci aktuálního oboru. Mějte na paměti, že v aktuální verzi F# jazyka, `use!` neumožňuje hodnotu má být inicializován na hodnotu null, i když `use` nepodporuje.

## <a name="asynchronous-primitives"></a>Asynchronní primitiv

Je volána metoda, která provádí jednu asynchronní úlohu a vrátí výsledek *asynchronní primitivní*, a ty jsou navrženy speciálně pro použití s `let!`. Několik asynchronních primitivních elementů jsou definovány v F# základní knihovny. Tyto dvě metody pro webové aplikace, které jsou definovány v modulu [ `Microsoft.FSharp.Control.WebExtensions` ](https://msdn.microsoft.com/library/95ef17bc-ee3f-44ba-8a11-c90fcf4cf003): [ `WebRequest.AsyncGetResponse` ](https://msdn.microsoft.com/library/09a60c31-e6e2-4b5c-ad23-92a86e50060c) a [ `WebClient.AsyncDownloadString` ](https://msdn.microsoft.com/library/8a85a9b7-f712-4cac-a0ce-0a797f8ea32a). Obě primitivy stáhnout data z webové stránky, případě adresy URL. `AsyncGetResponse` vytvoří `System.Net.WebResponse` objektu, a `AsyncDownloadString` vytvoří řetězec, který představuje kód HTML na webové stránce.
...
Používat asynchronní metody v rozhraní .NET Framework, které jsou určeny pro s dalšími asynchronními modely F# asynchronního programovacího modelu, vytvořit funkci, která se vrátí F# `Async` objektu. F# Knihovna obsahuje funkce, které usnadňují to udělat.

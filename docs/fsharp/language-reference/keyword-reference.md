...
|-------|----|-----------|
|`abstract`|[Členy](members/index.md)<br /><br />[Abstraktní třídy](abstract-classes.md)|Určuje metodu, která buď nemá žádnou implementaci v typu, ve kterém je deklarovaná, nebo se je virtuální a má výchozí implementaci.|
|`and`|[`let`Vazby](functions/let-bindings.md)<br /><br />[Členy](members/index.md)<br /><br />[Omezení](generics/constraints.md)|Použít v vzájemně rekurzivní vazby, v vlastnosti deklarací a s více omezení na obecné parametry.|
|`as`|[Třídy](classes.md)<br /><br />[Shoda vzoru](Pattern-Matching.md)|Umožňuje poskytnout aktuální objekt třídy názvu objektu. Rovněž pojmenujte celý vzor v rámci vzor shody.|
...
|`default`|[Členy](members/index.md)|Označuje implementace abstraktní metody; Chcete-li vytvořit virtuální metodu použít společně s deklarace abstraktní metody.|
|`delegate`|[Delegáti](delegates.md)|Používá k deklaraci delegáta.|
|`do`|[do – vazby](functions/do-bindings.md)<br /><br />[Smyčky: `for...to` výraz](loops-for-to-expression.md)<br /><br />[Smyčky: `for...in` výraz](loops-for-in-expression.md)<br /><br />[Smyčky: `while...do` výraz](loops-while-do-expression.md)|Použít v opakování ve smyčce konstrukce nebo ke spouštění imperativní kódu.|
...
|`for`|[Smyčky: `for...to` výraz](loops-for-to-expression.md)<br /><br />[Smyčky: for... ve výrazu](loops-for-in-expression.md)|Použít v opakování ve smyčce konstrukce.|
...
|`in`|[Smyčky: for... ve výrazu](loops-for-in-expression.md)<br /><br />[Podrobná syntaxe](verbose-syntax.md)|Použít pro sekvenční výrazy a na podrobná syntaxe jednotlivé výrazy z vazby.|
...
|`member`|[Členy](members/index.md)|Používá k deklaraci vlastnosti nebo metody v typu objektu.|
...
|`mutable`|[let – vazby](functions/let-bindings.md)|Používá k deklaraci do proměnné, který je hodnota, která lze změnit.|
...
|`not`|[Operátor referenční dokumentace symbolů a](symbol-and-operator-reference/index.md)<br /><br />[Omezení](generics/constraints.md)|Ve skutečnosti není klíčové slovo. Ale `not struct` v kombinaci se používá jako obecný parametr omezení.|
...
|`or`|[Operátor referenční dokumentace symbolů a](symbol-and-operator-reference/index.md)<br /><br />[Omezení](generics/constraints.md)|Použít s Boolean podmínky jako logická hodnota `or` operátor. Ekvivalent hodnoty '||`.<br /><br />Také použít v omezení člen.|
|`override`|[Členy](members/index.md)|Slouží k implementaci verzi metodu abstraktní nebo virtuální, která se liší od základní verze.|
...
|`select`|[Výrazy dotazů](query-expressions.md)|Použít ve výrazech dotazů k určení, která pole nebo sloupce, které chcete extrahovat. Všimněte si, že se jedná o kontextové klíčové slovo, což znamená, že není ve skutečnosti o vyhrazené slovo a pouze funguje jako klíčové slovo v odpovídající kontext.|
|`static`|[Členy](members/index.md)|Slouží k určení metody nebo vlastnosti, která může být volána bez instance typu nebo členem hodnotu, která je sdílena mezi všechny instance typu.|
...
|`val`|[Explicitní pole: `val` – klíčové slovo](members/explicit-fields-the-val-keyword.md)<br /><br />[Podpisy](signatures.md)<br /><br />[Členy](members/index.md)|Použito v podpisu pro zobrazení hodnoty, nebo v typu deklarovat člena, v situacích, omezené.|
...
|`with`|[Výrazy shody](match-expressions.md)<br /><br />[Objektové výrazy](object-expressions.md)<br /><br />[Kopírování a aktualizace záznamů výrazy](copy-and-update-record-expressions.md)<br /><br />[Rozšíření typů](type-extensions.md)<br /><br />[Výjimky: `try...with` výraz](exception-handling/the-try-with-expression.md)|Použít v kombinaci s `match` – klíčové slovo v výrazy porovnávání vzorů. Také používají v objektové výrazy, rozšíření a záznamů kopírování výrazy zavádět člen definice a zavádět obslužné rutiny výjimek.|
|`yield`|[Pořadí](sequences.md)|Ve výrazu pořadí použít k vytvoření hodnoty pro pořadí.|

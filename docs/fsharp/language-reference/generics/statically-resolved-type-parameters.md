...
|-------|-------|-------------------|
|Syntaxe|`'T``'U`|`^T``^U`|
|Doba řešení|Doba běhu|Čas kompilace|
...
Vyřešený typ `(+@)` je založen na použití `(+)` a `(*)`, z nichž obě způsobují odvození typu pro odvození omezení členů u staticky vyřešených parametrů typu. Vyřešený typ, jak je znázorněno v F# Překladači, je následující.

...
Další informace naleznete v tématu [konstruktory (F #)](members/constructors.md).
...
Pro účely C# a Visual Basic interoperability můžete použít atributy `[<Optional; DefaultParameterValue<(...)>]` v F #, aby volající jako volitelnou viděli argument. To je ekvivalentní k definování argumentu jako volitelné v jazyce C# jako v `MyMethod(int i = 3)` .
...
Můžete také zadat nový objekt jako výchozí hodnotu parametru. `Foo`Člen by například mohl mít `CancellationToken` jako vstup volitelné místo:
...
V tomto případě kompilátor vygeneruje upozornění a bude zcela ignorovat oba atributy. Všimněte si, že výchozí hodnota `null` musí být typu s poznámkami, jinak kompilátor odvodí nesprávný typ, tj. `[<Optional; DefaultParameterValue(null:obj)>] o:obj` .

## <a name="passing-by-reference"></a>Předávání odkazem

Předání hodnoty F # odkazem zahrnuje [byrefs](byrefs.md), které jsou spravované typy ukazatelů. Pokyny pro typ, který se má použít, jsou následující:
...
Můžete použít řazenou kolekci členů jako návratovou hodnotu pro uložení `out` parametrů v metodách knihovny .NET. Alternativně můžete parametr zakládat `out` jako `byref` parametr. Následující příklad kódu ukazuje oba způsoby.

[!code-fsharp[Main](~/samples/snippets/fsharp/parameters-and-arguments-1/snippet3810.fs)]

## <a name="parameter-arrays"></a>Pole parametrů

V některých případech je nutné definovat funkci, která přebírá libovolný počet parametrů heterogenního typu. Nebylo by praktické vytvořit všechny možné přetížené metody pro všechny typy, které by mohly být použity. Implementace rozhraní .NET poskytují podporu pro tyto metody prostřednictvím funkce pole parametrů. Metoda, která přebírá pole parametrů v jeho podpisu, může být poskytnuta s libovolným počtem parametrů. Parametry jsou vloženy do pole. Typ prvků pole určuje typy parametrů, které mohou být předány do funkce. Definujete-li pole parametrů `System.Object` jako typ prvku, může kód klienta předat hodnoty libovolného typu.

V jazyce F # lze pole parametrů definovat pouze v metodách. Nelze je použít v samostatných funkcích nebo funkcích, které jsou definovány v modulech.

Pole parametrů definujete pomocí `ParamArray` atributu. `ParamArray`Atribut lze použít pouze pro poslední parametr.

Následující kód ilustruje volání metody .NET, která přijímá pole parametrů a definici typu v jazyce F #, která má metodu, která přijímá pole parametrů.

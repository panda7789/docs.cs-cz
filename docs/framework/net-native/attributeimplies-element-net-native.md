...
## <a name="remarks"></a>Poznámky  
 Element `<AttributeImplies>` se používá, pokud je jeho nadřazený typ atribut (to je třída odvozená z <xref:System.Attribute?displayProperty=nameWithType>). Pokud je atribut použit na konkrétní prvek programu, zásada definovaná elementem `<AttributeImplies>` se vztahuje na tento prvek programu.  

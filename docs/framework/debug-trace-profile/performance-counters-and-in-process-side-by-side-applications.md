...
|Hodnota|1 (0x00000001)|  
 ...
 Hodnota 0 pro `ProcessNameFormat` označuje, že je povoleno výchozí chování; To znamená, že Perfmon. exe zobrazuje čítače výkonu na základě jednotlivých aplikací. Když nastavíte hodnotu 1, program Perfmon. exe nejednoznačně vyřadí více verzí aplikace a poskytuje čítače výkonu pro jednotlivé moduly runtime. Jakákoli jiná hodnota nastavení klíče registru `ProcessNameFormat` není podporovaná a rezervovaná pro budoucí použití.  
  
 Po aktualizaci nastavení klíče registru `ProcessNameFormat` musíte restartovat soubor Perfmon. exe nebo všechny další uživatele čítačů výkonu, aby nová funkce pro pojmenovávání instancí fungovala správně.  
  
 Následující příklad ukazuje, jak změnit hodnotu `ProcessNameFormat` programově.  

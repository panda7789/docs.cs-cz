...
4. Vygeneruje tělo metody. V tomto příkladu je <xref:System.Reflection.Emit.ILGenerator> objekt použit k VYGENEROVÁNÍ jazyka MSIL (Microsoft Intermediate Language). Alternativně lze <xref:System.Reflection.Emit.DynamicILInfo> objekt použít ve spojení s nespravovanými generátory kódu k vygenerování těla metody pro <xref:System.Reflection.Emit.DynamicMethod>.  
...
5. Vytvořte instanci delegáta (deklarovaná v kroku 1), která představuje dynamickou metodu voláním přetížení <xref:System.Reflection.Emit.DynamicMethod.CreateDelegate%28System.Type%2CSystem.Object%29> metody. Vytvořením delegáta dokončí metodu a všechny další pokusy o změnu metody – například přidávání dalších MSIL – jsou ignorovány.  
...
## <a name="example"></a>Příklad  
 ...
 Druhá dynamická metoda má dva parametry typu `Example` a typu `int` (`Integer` v Visual Basic). Pokud byla dynamická metoda vytvořena, je svázána s instancí `Example`s použitím obecného delegáta, který má jeden argument typu. `int` Delegát nemá argument typu `Example` , protože první parametr metody vždy obdrží vázanou instanci. `Example` Když je vyvolán delegát, je zadán `int` pouze argument. Tato dynamická metoda přistupuje k soukromému poli `Example` třídy a vrátí součin privátního pole a `int` argumentu.  

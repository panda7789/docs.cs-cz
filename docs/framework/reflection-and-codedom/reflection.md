...
- Slouží <xref:System.Reflection.ConstructorInfo> ke zjištění informací, jako je název, parametry, modifikátory přístupu ( `public` například nebo `private`), a podrobnosti implementace (například `abstract` nebo `virtual`) konstruktoru. Použijte metodu <xref:System.Type.GetConstructors%2A> nebo <xref:System.Type.GetConstructor%2A> metody <xref:System.Type> k vyvolání konkrétního konstruktoru.  
  
- Slouží <xref:System.Reflection.MethodInfo> ke zjištění informací, jako je název, návratový typ, parametry, modifikátory přístupu ( `public` například nebo `private`) a podrobnosti implementace (například `abstract` nebo `virtual`) metody. Použijte metodu <xref:System.Type.GetMethods%2A> nebo <xref:System.Type.GetMethod%2A> metody <xref:System.Type> k vyvolání konkrétní metody.  
...
Například při použití operátoru jazyka `typeof` C# (`GetType` v Visual Basic) k získání <xref:System.Type> objektu je objekt skutečně a. `RuntimeType` `RuntimeType`je odvozen z <xref:System.Type> a poskytuje implementace všech abstraktních metod.  

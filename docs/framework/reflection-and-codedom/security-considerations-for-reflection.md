...
- Chcete-li kódu dovolit vyvolat libovolný NonPublic člen: váš kód musí být <xref:System.Security.Permissions.ReflectionPermission> udělen pomocí <xref:System.Security.Permissions.ReflectionPermissionFlag.MemberAccess?displayProperty=nameWithType> příznaku.
...
Předpokládejme například, že udělíte přístupová oprávnění k doméně aplikace a <xref:System.Security.Permissions.ReflectionPermission> k <xref:System.Security.Permissions.ReflectionPermissionFlag.RestrictedMemberAccess?displayProperty=nameWithType> příznaku a potom spustíte internetovou aplikaci se dvěma sestaveními, a a B.

...
## <a name="custom-binding"></a>Vlastní vazba  
 ...
 Následující příklad ukazuje jednoduchý vlastní pořadač, který neposkytuje převod typu argumentu. Kód `Simple_Type.dll` předchází hlavnímu příkladu. Nezapomeňte sestavit `Simple_Type.dll` a potom do projektu zahrnout odkaz v okamžiku sestavení.  

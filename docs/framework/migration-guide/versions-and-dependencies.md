...
- V sadě Visual Studio můžete zvolit .NET Framework 4.5.1 jako cílové rozhraní projektu (tím se nastaví <xref:Microsoft.Build.Tasks.GetReferenceAssemblyPaths.TargetFrameworkMoniker%2A?displayProperty=nameWithType> vlastnost) pro zkompilování projektu jako sestavení .NET Framework 4.5.1 nebo spustitelný soubor. Toto sestavení nebo spustitelný soubor je třeba spustit pouze v počítačích, ve kterých je nainstalována .NET Framework 4.5.1 nebo novější verze .NET Framework. Spustitelný soubor, který cílí na .NET Framework 4.5.1, se zablokuje spuštění v počítači, na kterém je jenom starší verze .NET Framework, třeba .NET Framework 4,5, nainstalovaná a uživatel se zobrazí výzva k instalaci .NET Framework 4.5.1. Kromě toho .NET Framework 4.5.1 sestavení nesmí být voláno z aplikace, která cílí na starší verzi .NET Framework, jako je například .NET Framework 4,5.  

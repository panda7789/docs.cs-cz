...
Verze .NET Framework (4,5 a novější) nainstalovaná na počítači je uvedená v registru na adrese **HKEY_LOCAL_MACHINE\\softwaru\\Microsoft\\NET Framework Setup\\NDP\\v4\\Full**. Pokud chybí **úplný** podklíč, pak .NET Framework 4,5 nebo vyšší není nainstalováno.
...
| .NET Framework 4,8     | Ve Windows 10 května 2019 Update a Windows 10 listopadu 2019 Update: 528040<br/>Ve Windows 10 a Windows serveru verze 1909:528209<br/>Ve všech ostatních operačních systémech Windows (včetně dalších operačních systémů Windows 10): 528049 |
...
01. Použijte metody <xref:Microsoft.Win32.RegistryKey.OpenBaseKey%2A?displayProperty=nameWithType> a <xref:Microsoft.Win32.RegistryKey.OpenSubKey%2A?displayProperty=nameWithType> pro přístup ke **HKEY_LOCAL_MACHINE\\softwaru\\Microsoft\\NET Framework Setup\\NDP\\v4\\Full** podklíč v registru Windows.

    > [!IMPORTANT]
    > Pokud aplikace, kterou používáte, je 32 a běží v 64 bitovém systému Windows, cesty registru se budou lišit od dříve uvedených. 64 registr je k dispozici v **HKEY_LOCAL_MACHINE\\softwaru\\Wow6432Node\\** podklíč. Například podklíč registru pro .NET Framework 4,5 je **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\Microsoft\\NET Framework Setup\\NDP\\v4\\Full**.
...
Každá verze .NET Framework od 1,1 do 4,0 je uvedena jako podklíč na **HKEY_LOCAL_MACHINE\\softwaru\\Microsoft\\NET Framework Setup\\NDP**. Následující tabulka uvádí cestu ke každé verzi .NET Framework. Pro většinu verzí je k dispozici **REG_DWORD hodnota** `1`, která označuje, že je tato verze nainstalovaná. V těchto podklíčích je také REG_SZ hodnota **verze** , která obsahuje řetězec verze.
...
| ------------------ | --------------- | ----- |
| 1.0                | **HKLM\\software\\\\Microsoftu. NETFramework\\Policy\\v 1.0\\3705**     | **Nainstalovat** REG_SZ rovná `1` |
| 1.1                | **HKLM\\software\\Microsoft\\.NET Framework Setup\\NDP\\v 1.1.4322**   | **Nainstalovat** REG_DWORD rovná `1` |
| 2.0                | **HKLM\\software\\Microsoft\\.NET Framework Setup\\NDP\\v 2.0.50727**  | **Nainstalovat** REG_DWORD rovná `1` |
| 3.0                | **HKLM\\software\\Microsoft\\nastavení rozhraní .NET Framework Setup\\NDP\\v 3.0\\Setup** | **InstallSuccess** REG_DWORD rovná `1` |
| 3,5                | **HKLM\\software\\Microsoft\\.NET Framework Setup\\NDP\\v 3.5**        | **Nainstalovat** REG_DWORD rovná `1` |
| Profil klienta 4,0 | **HKLM\\software\\Microsoft\\nastavení rozhraní .NET Framework Setup\\NDP\\v4\\Client**  | **Nainstalovat** REG_DWORD rovná `1` |
| úplný profil 4,0   | **HKLM\\software\\Microsoft\\.NET Framework Setup\\NDP\\v4\\Full**    | **Nainstalovat** REG_DWORD rovná `1` |

> [!IMPORTANT]
> Pokud aplikace, kterou používáte, je 32 a běží v 64 bitovém systému Windows, cesty registru se budou lišit od dříve uvedených. 64 registr je k dispozici v **HKEY_LOCAL_MACHINE\\softwaru\\Wow6432Node\\** podklíč. Například podklíč registru pro .NET Framework 3,5 je **HKEY_LOCAL_MACHINE\\SOFTWARE\\Wow6432Node\\instalaci Microsoft\\NET Framework\\NDP\\v 3.5**.
...
Použijte třídu <xref:Microsoft.Win32.RegistryKey?displayProperty=nameWithType> pro přístup ke **HKEY_LOCAL_MACHINE\\softwaru\\Microsoft\\NET Framework Setup\\** podklíč v registru systému Windows.
...
- **Třída `Environment`**
  ...
  > [!IMPORTANT]
  > U .NET Framework 4,5 a novějších verzí nepoužívejte vlastnost <xref:System.Environment.Version%2A?displayProperty=nameWithType> k detekci verze modulu CLR. Místo toho proveďte dotaz na registr, jak je popsáno v tématu [detekce .NET Framework 4,5 a novějších verzí](#detect-net-framework-45-and-later-versions).
  
  01. Pomocí dotazu na vlastnost <xref:System.Environment.Version?displayProperty=nameWithType> načtěte objekt <xref:System.Version>.

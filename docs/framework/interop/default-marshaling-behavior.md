...
## <a name="memory-management-with-the-interop-marshaler"></a>Správa paměti pomocí zařazovacího modulu spolupráce  
 Zařazování Interop se vždy pokouší uvolnit paměť přidělenou nespravovaným kódem. Toto chování je v souladu s pravidly správy paměti modelu COM, ale liší se od pravidel, která řídí nativní jazyk C++.  
  
 K záměně může dojít, pokud při použití vyvolání platformy očekáváte nativní chování C++ (bez uvolnění paměti), které pro ukazatele automaticky uvolňuje paměť. Například volání následující nespravované metody z knihovny DLL jazyka C++ neuvolní automaticky žádnou paměť.  

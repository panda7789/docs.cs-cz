...
|<xref:System.Runtime.InteropServices.DllImportAttribute.SetLastError>|Umožňuje volajícímu použít `Marshal.GetLastWin32Error` funkci rozhraní API k určení, zda došlo k chybě při provádění metody. V Visual Basic `true`výchozí hodnota je; v C# a C++je výchozí `false`hodnota.|  
...
## <a name="platform-invoke-security-considerations"></a>Požadavky na zabezpečení volání platformy  
 `Assert`Členy ,<xref:System.Security.Permissions.SecurityAction> a jsou označováni jako *modifikátory procházení zásobníku.* `Deny` `PermitOnly` Tyto členy jsou ignorovány, pokud jsou používány jako deklarativní atributy v deklaracích vyvolání platformy a příkazy IDL (Interface Definition Language) modelu COM.  
...
### <a name="platform-invoke-examples"></a>Příklady vyvolání platformy  
 ...
 V <xref:System.Security.Permissions.SecurityAction>následujícím příkladu `Assert`jsou modifikátory, `Deny`a `PermitOnly` ignorovány.  
...
```  
 ...
 `Demand` Nicméně modifikátor v následujícím příkladu je přijat.  
...
#### <a name="com-interop-examples"></a>Příklady spolupráce modelu COM  
 ...
 Následující deklarace rozhraní COM interop rozhraní ignorují `Assert`modifikátory, `PermitOnly` `Deny`a podobně jako příklady vyvolání platformy v předchozí části.  

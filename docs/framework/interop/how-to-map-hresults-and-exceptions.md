...
Metody modelu COM hlásí chyby vrácením hodnoty HRESULTs; metody rozhraní .NET hlásí je vyvoláním výjimek. Modul runtime zpracovává přechod mezi dvěma. Každá třída výjimky v .NET Framework se mapuje na HRESULT.  
 ...
 Všimněte si, že modul runtime někdy ignoruje `HRESULT` v případech, kdy je ve vlákně přítomno `IErrorInfo`.  K tomuto chování může dojít v případech, kdy `HRESULT` a `IErrorInfo` nepředstavovaly stejnou chybu.  
...
1. Pomocí následujícího kódu vytvořte novou třídu výjimky nazvanou `NoAccessException` a namapujte ji na `E_ACCESSDENIED`HRESULT.  
...
|**Zdrojová**|Řetězec vrácený z **IErrorInfo-> GetSource**|  

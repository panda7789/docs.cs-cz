...
1. Vytvořte pomocí požadovaných kroků ověření <xref:System.ServiceModel.Description.IEndpointBehavior.Validate%2A> v metodě. <xref:System.ServiceModel.Description.IEndpointBehavior> Následující kód poskytuje příklad. (Z ukázky [ověření zabezpečení](../samples/security-validation.md) jepořízená.)`InternetClientValidatorBehavior`
...
Následující příklad kódu ukazuje, jak přidat do souboru Machine. config běžné chování a uložit kopii na disk. Je pořízen z ukázky [ověření zabezpečení.](../samples/security-validation.md) `InternetClientValidatorBehavior`

...
||Místní uživatel|Local System|Uživatel domény|Počítač domény|  
|-|----------------|------------------|-----------------|--------------------|  
|Místní uživatel|NTLM|NTLM|NTLM|NTLM|  
|Local System|Anonymní protokol NTLM|Anonymní protokol NTLM|Anonymní protokol NTLM|Anonymní protokol NTLM|  
...
- Počítač domény: proces s identitou počítače běžící na počítači připojeném k doméně systému Windows. Příklad: `MachineName\Network Service`.  
  
> [!NOTE]
> Pověření služby je zachyceno při <xref:System.ServiceModel.ICommunicationObject.Open%2A> <xref:System.ServiceModel.ServiceHost> volání metody třídy. Přihlašovací údaje klienta se čtou při každém odeslání zprávy klientem.  
...
#### <a name="kerberos-protocol-direct-requires-the-service-to-run-under-a-domain-machine-account"></a>Protokol Kerberos Direct vyžaduje, aby služba běžela pod účtem počítače domény.  
 K tomu dochází `ClientCredentialType` , je-li vlastnost nastavena na hodnotu `Windows` a <xref:System.ServiceModel.MessageSecurityOverHttp.NegotiateServiceCredential%2A> vlastnost je nastavena na hodnotu `false` , jak je znázorněno v následujícím kódu.  
...
2. Vyžadovat vyjednávání SSPI:  
    ...
    1. Používáte-li standardní vazby, nastavte `NegotiateServiceCredential` vlastnost na hodnotu `true` .  
...
3. Vyžaduje, aby vyjednávání SSPI používalo protokol Kerberos, a to tak, že nepovolí použití NTLM:  
    ...
    2. Nebo to můžete provést v konfiguračním souboru nastavením `allowNtlm` atributu na `false` . Tento atribut je obsažen v [\<windows>](../../configure-apps/file-schema/wcf/windows-of-clientcredentials-element.md) .  
...
#### <a name="negotiate-ssp-falls-back-to-ntlm-but-ntlm-is-disabled"></a>Vyjednání SSP se vrátí do protokolu NTLM, ale protokol NTLM je zakázán.  
 <xref:System.ServiceModel.Security.WindowsClientCredential.AllowNtlm%2A>Vlastnost je nastavena na `false` , což způsobí, že Windows Communication Foundation (WCF) vyvolá výjimku, pokud je použit protokol NTLM. Nastavení této vlastnosti na `false` nemusí bránit v posílání přihlašovacích údajů protokolu NTLM přes síťový kabel.  
...
#### <a name="anonymous-ntlm-logon-occurs-but-anonymous-logons-are-disabled-by-default"></a>Dojde k anonymnímu přihlášení NTLM, ale anonymní přihlášení jsou ve výchozím nastavení zakázaná.  
 ...
 Další informace o zosobnění najdete v tématu [delegování a zosobnění](delegation-and-impersonation-with-wcf.md).  

...
## <a name="identity-types"></a>Typy identity  
 Služba může poskytnout pět typů identit. Každý typ identity odpovídá element, který může být obsažený uvnitř `<identity>` element v konfiguraci. Typ použitý závisí na scénáře a požadavky na zabezpečení služby. Následující tabulka popisuje každý typ identity.  

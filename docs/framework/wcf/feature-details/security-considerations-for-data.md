...
Při plánování práce s daty ve Windows Communication Foundation (WCF), musíte zvážit mnoho kategorií hrozeb. Následující tabulka uvádí nejdůležitější třídy hrozeb, které se vztahují ke zpracování dat. WCF poskytuje nástroje ke zmírnění tyto hrozby.  
...
### <a name="slow-stream-attacks"></a>Útoky pomalé datového proudu  
 ...
 Zmírnit tyto útoky, správně nastavené časové limity přenosu. Další informace najdete v tématu [přenosové kvóty](../../../../docs/framework/wcf/feature-details/transport-quotas.md). Za druhé, nikdy nepoužívejte synchronní `Read` nebo `Write` operace při práci s datovými proudy v WCF.  
...
### <a name="secure-xml-readers"></a>Zabezpečení čtečky XML  
 Informační sadu XML je základem veškeré zpracování zpráv ve WCF. Pokud existují přijímá data XML z nedůvěryhodného zdroje, počet možnosti útok s cílem služby, musí být omezeny. WCF poskytuje speciální, zabezpečte čtečky XML. Tyto čtečky automaticky vytvoří při použití jednu standardní kódování ve WCF (text, binary nebo MTOM).  
 ...
 Při práci přímo s čtečky XML (například při zápisu vlastní kodér nebo při práci přímo s <xref:System.ServiceModel.Channels.Message> – třída), vždy používali zabezpečené čtečky WCF, když je možné pracovat s nedůvěryhodné data. Vytvoření zabezpečeného čtečky volání jeden statický objektu pro vytváření přetížení metody <xref:System.Xml.XmlDictionaryReader.CreateTextReader%2A>, <xref:System.Xml.XmlDictionaryReader.CreateBinaryReader%2A>, nebo <xref:System.Xml.XmlDictionaryReader.CreateMtomReader%2A> na <xref:System.Xml.XmlDictionaryReader> třídy. Při vytváření čtečka čipových karet, předejte hodnoty zabezpečené kvóty. Nevolejte `Create` přetížení metody. Tyto nevytvářejte čtečku WCF. Místo toho se vytvoří čtečka čipových karet, který není chráněný pomocí funkce zabezpečení, které jsou popsané v této části.  
...
## <a name="threats-specific-to-the-binary-encoding"></a>Specifické hrozby pro binární kódování  
 Obsahuje binární kódování WCF podporuje XML *slovníku řetězce* funkce. Řetězec velký může kódované pomocí pouze několik bajtů. To umožňuje výrazné zvýšení výkonu, ale zavádí nové hrozby odmítnutí služby, které musí být omezeny.  
  
 Existují dva druhy slovník: *statické* a *dynamické*. Statické slovník je předdefinovaný seznam dlouhých řetězců, které může být reprezentován pomocí krátký kód v binární kódování. Tento seznam řetězců vyřešen, když čtečka je vytvořena a nelze jej změnit. Žádný z řetězce ve slovníku statické WCF používá ve výchozím nastavení není dostatečně velký pro představovat závažné ohrožení, odmítnutí služby, přestože může být přesto použít v slovníkový útok rozšíření. V pokročilých scénářích, kde zadáte vlastní statické slovník buďte opatrní při představení řetězců velké slovníku.  
...
### <a name="using-the-netdatacontractserializer-securely"></a>Pomocí NetDataContractSerializer bezpečně  
 <xref:System.Runtime.Serialization.NetDataContractSerializer> Serializace modul, který používá úzkou párování na typy. Toto je podobná <xref:System.Runtime.Serialization.Formatters.Binary.BinaryFormatter> a <xref:System.Runtime.Serialization.Formatters.Soap.SoapFormatter>. To znamená, určí, který typ instance načtením [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] název sestavení a typu z příchozí data. Přestože je součástí služby WCF, se nedá nijak zadaný vyřešily v této Serializační stroj; musí být napsané vlastní kód. `NetDataContractSerializer` Zajišťuje především pro usnadnění migrace z [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] remoting do WCF. Další informace najdete v části relevantní v [serializace a deserializace](../../../../docs/framework/wcf/feature-details/serialization-and-deserialization.md).  
...
## <a name="threats-specific-to-aspnet-ajax-integration"></a>Specifické hrozby integrace ASP.NET AJAX  
 Když uživatel implementuje <xref:System.ServiceModel.Description.WebScriptEnablingBehavior> nebo <xref:System.ServiceModel.Description.WebHttpBehavior>, WCF zpřístupní koncový bod, který může přijmout zprávy XML a JSON. Je však jen jednu sadu čtečky kvóty, používat službu čtecí modul XML a JSON čtečky. Některá nastavení kvóty může být vhodné pro jeden čtečky ale příliš velký pro druhý.  
...
## <a name="a-note-on-components"></a>Poznámka: na komponenty  
 WCF je systém flexibilní a přizpůsobit. Většinu obsahu v tomto tématu se zaměřit na nejběžnější scénáře použití WCF. Je však možné tvoří součásti, které poskytuje WCF v mnoha různými způsoby. Je důležité porozumět zabezpečení důsledky použití jednotlivých součástí. Zejména:  
  
-   Pokud musíte použít čtečky XML, použijte čtečky <xref:System.Xml.XmlDictionaryReader> třída poskytuje rozdíl od jiných čtenářů. Bezpečné čtečky jsou vytvořené pomocí <xref:System.Xml.XmlDictionaryReader.CreateTextReader%2A>, <xref:System.Xml.XmlDictionaryReader.CreateBinaryReader%2A>, nebo <xref:System.Xml.XmlDictionaryReader.CreateMtomReader%2A> metody. Nepoužívejte <xref:System.Xml.XmlReader.Create%2A> metoda. Vždy konfigurujte čtečky s bezpečné kvóty. Moduly serializace ve WCF, jsou zabezpečené jenom při použití s zabezpečené čtečky XML z WCF.  

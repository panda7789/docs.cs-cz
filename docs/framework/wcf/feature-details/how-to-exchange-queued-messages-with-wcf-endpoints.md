...
3. Implementujte metody kontraktu služby definovaného v rozhraní třídy.  
     ...
     Všimněte si <xref:System.ServiceModel.OperationBehaviorAttribute> , že se `SubmitPurchaseOrder` nachází na metodě. To určuje, že tuto operaci je nutné volat v rámci transakce a že transakce se automaticky dokončí po dokončení metody.  
...
5. Definujte v konfiguraci, která určuje adresu služby a používá standardní <xref:System.ServiceModel.NetMsmqBinding> vazbu. <xref:System.ServiceModel.Description.ServiceEndpoint> Další informace o použití konfigurace WCF najdete v tématu [Konfigurace služeb WCF](../configuring-services.md).  
...
2. Definujte v konfiguraci, která určuje adresu a používá standardní <xref:System.ServiceModel.NetMsmqBinding> vazbu, jak je znázorněno v následujícím příkladu. <xref:System.ServiceModel.Description.ServiceEndpoint>  

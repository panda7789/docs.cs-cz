...
4. Abychom mohli spustit službu WCF, musíme deklarovat koncový bod, který zveřejňuje toto rozhraní služby na konkrétní adrese URL pomocí konkrétní vazby WCF. To se obvykle provádí přidáním následujících sekcí do souboru web.config projektu serveru.  
...
5. Službu WCF lze následně spustit pomocí následujícího kódu:  
   ...
   ```  
     ...
     Po spuštění této třídy ServiceHost používá soubor web.config k vytvoření správného kontraktu, vazby a koncového bodu. Další informace o konfiguračních souborech najdete v tématu [Konfigurace služeb pomocí konfiguračních souborů](./configuring-services-using-configuration-files.md). Tento styl spuštění serveru se označuje jako samoobslužné hostování. Další informace o dalších možnostech hostování služeb WCF najdete v tématu [hostingové služby](./hosting-services.md).  
  
6. app.config klientského projektu musí deklarovat párové informace o vazbě pro koncový bod služby. Nejjednodušší způsob, jak to provést v aplikaci Visual Studio, je použít **Přidat odkaz na službu**, který automaticky aktualizuje soubor app.config. Tyto změny lze také přidat ručně.  
...
4. Musíme upravit konfigurační soubor serveru tak, že provedete následující dvě věci, jak je znázorněno v následujícím příkladu:  
    ...
    1. Deklarujete \<client> oddíl, který popisuje koncový bod objektu Session. To je nezbytné, protože server v této situaci také funguje jako klient.  
...
5. Klienta nakonfigurujeme deklarováním stejných koncových bodů v souboru app.config projektu.  

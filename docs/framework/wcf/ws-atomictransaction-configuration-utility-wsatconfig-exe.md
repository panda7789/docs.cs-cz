...
|-accountsCerts:\<>&#124;"Issuer\SubjectName", >|Určuje čárkami oddělený seznam certifikátů, které se mohou účastnit protokolu WS-AtomicTransaction. Certifikáty jsou označeny kryptografickým otiskem nebo dvojicí Issuer\SubjectName. Pokud je název předmětu prázdný, použijte {EMPTY}.|  
|-endpointCert: < počítač&#124;\<Issuer\SubjectName >&#124;>|Používá certifikát počítače nebo jiný certifikát místního koncového bodu určený dvojicí kryptografický otisk nebo Issuer\SubjectName. Pokud je název subjektu prázdný, používá se {EMPTY}.|  

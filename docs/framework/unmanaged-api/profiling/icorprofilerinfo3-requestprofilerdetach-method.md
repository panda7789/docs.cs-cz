...
|CORPROF_E_IRREVERSIBLE_INSTRUMENTATION_PRESENT|Odpojení není možné, protože Profiler použil instrumentované kód jazyka MSIL (Microsoft Intermediate Language) nebo vložené `enter` / `leave` háky. Nedošlo k pokusu o odpojení; Profiler je stále plně připojený.<br /><br /> **Poznámka:** Instrumentovaná MSIL je kód, který poskytuje Profiler pomocí metody [SetILFunctionBody –](icorprofilerinfo-setilfunctionbody-method.md) .|  
...
## <a name="remarks"></a>Poznámky  
 Během procesu odpojení se vlákno odpojení (vlákno vytvořené speciálně pro odpojení profileru) občas kontroluje, jestli všechna vlákna ukončila kód profileru. Profiler by měl poskytnout odhad toho, jak dlouho by měl tento parametr přebírat `dwExpectedCompletionMilliseconds` . Dobrá hodnota, která se má použít, je obvyklá doba, kterou Profiler stráví všemi danými `ICorProfilerCallback*` metodami. Tato hodnota by neměla být menší než polovina maximální doby, po kterou profiler očekává útratu.  
  
 Vlákno odpojení používá `dwExpectedCompletionMilliseconds` k rozhodnutí, jak dlouho se má přejít do režimu spánku, než zkontroluje, jestli je kód zpětného volání profileru vyjmutý ze všech zásobníků. I když se podrobnosti o následujícím algoritmu mohou v budoucích vydáních modulu CLR změnit, ukazuje jeden způsob, jak `dwExpectedCompletionMilliseconds` lze použít při určování, kdy je bezpečné zrušit načtení profileru. Vlákno odpojení je prvním režimu spánku pro `dwExpectedCompletionMilliseconds` milisekundy. Pokud po probuzení z režimu spánku, CLR najde, že kód zpětného volání profileru je stále přítomen, a to po dobu dvou milisekund odpojování vlákna `dwExpectedCompletionMilliseconds` . Pokud po probuzení z tohoto druhého režimu spánku najde odpojené vlákno, že kód zpětného volání profileru je stále přítomen, po dobu 10 minut přejde do režimu spánku. Vlákno odpojení pokračuje v kontrole každých 10 minut.  

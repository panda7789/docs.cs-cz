...
## <a name="remarks"></a>Poznámky  
 Ladicí program může pomocí `CorDebugInterfaceVersion` výčtu ve funkci [CreateDebuggingInterfaceFromVersion –](../hosting/createdebugginginterfacefromversion-function.md) určit nejvyšší verzi .NET Framework, kterou ladicí program podporuje.  
...
- .NET Framework 4 zavádí kombinaci verzí rozhraní. Například `ICorDebugThread3` a `ICorDebugThread4` se zobrazí jako třetí a čtvrté verze `ICorDebugThread` rozhraní. .NET Framework 4 také zavádí první verzi `ICorDebugStackWalk` rozhraní a druhou verzi `ICorDebugNativeFrame` rozhraní (`ICorDebugNativeFrame2`).  

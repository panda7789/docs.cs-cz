...
`strSuperclass`\
pro Pokud není `null` nebo prázdné, určuje název nadřazené třídy; enumerátor vrátí pouze podtřídy této třídy. Pokud je `null` nebo prázdné a `lFlags` je WBEM_FLAG_SHALLOW, vrátí pouze třídy nejvyšší úrovně (třídy bez nadřazené třídy). Pokud je `null` nebo prázdné a `lFlags` je `WBEM_FLAG_DEEP`, vrátí všechny třídy v oboru názvů.
...
| `WBEM_FLAG_DEEP` | 0,8 | Výčet zahrnuje všechny podtřídy v hierarchii, ale ne tuto třídu. |

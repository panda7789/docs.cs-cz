...
Všimněte si, že atributy XML slouží ke sdělování informací nezbytných pro definování navigační vlastnosti: atribut `Name` obsahuje název vlastnosti, `Relationship` obsahuje název přidaného přidružení a `FromRole` a `ToRole` obsahuje konce přidružení.

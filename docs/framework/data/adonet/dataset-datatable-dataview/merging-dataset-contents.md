...
Následující příklad kódu Sloučí dvě `DataSet` objekty s různými schématy do jedné `DataSet` kombinované schémata dvě příchozí `DataSet` objekty.

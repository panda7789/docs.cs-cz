...
## <a name="remarks"></a>Poznámky  
 `WSDualHttpBinding` poskytuje stejnou podporu pro protokoly webových služeb jako `WSHttpBinding`, ale pro použití s meziduplexní kontrakty. `WSDualHttpBinding` podporuje pouze zabezpečení SOAP a vyžaduje spolehlivé zasílání zpráv. Tato vazba vyžaduje, aby měl klient veřejný identifikátor URI, který pro službu poskytuje koncový bod zpětného volání. Tato vlastnost je k dispozici v atributu `clientBaseAddress`. Duální vazba zpřístupňuje IP adresu klienta ke službě. Klient by měl použít zabezpečení, aby zajistil, že se připojí pouze ke službám, kterým důvěřuje.  

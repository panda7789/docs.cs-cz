...
|třídy TransferMode.|<xref:System.ServiceModel.TransferMode>Hodnota, která označuje, zda služba nakonfigurovaná s vazbou používá režimy přenosu zpráv v toku nebo v režimu vyrovnávací paměti (nebo obojí). Výchozí formát je `Buffered`.|  
...
|[\<security>](security-of-webhttpbinding.md)|Definuje nastavení zabezpečení pro vazbu. Tento prvek je typu <xref:System.ServiceModel.Configuration.WebHttpSecurityElement> .|  

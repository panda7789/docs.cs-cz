...
|validateOnCreate|Volitelná logická hodnota, která určuje, zda bude při otevření hostitele WorkflowServiceHost provedeno ověření definice pracovního postupu.  Pokud je tento atribut nastaven na `true`, je provedeno ověření pracovního postupu při `WorkflowServiceHost.Open` každém volání. Pokud jsou nalezeny chyby ověřování, <xref:System.Workflow.ComponentModel.Compiler.WorkflowValidationFailedException> je vyvolána chyba.<br /><br /> Pokud je tato vlastnost nastavena na `false`hodnotu, nebude provedena žádná validace definice pracovního postupu.<br /><br /> Výchozí hodnota této vlastnosti je `true`.|  

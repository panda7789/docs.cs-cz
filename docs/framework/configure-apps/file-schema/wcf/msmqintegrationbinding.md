...
|deadLetterQueue|A <xref:System.ServiceModel.MsmqBindingBase.DeadLetterQueue%2A>. hodnota určující typ fronty nedoručených zpráv, která se má použít, pokud<br /><br /> Fronta nedoručených zpráv je umístění, do kterého se přenesou zprávy, které se nepodařilo doručit do aplikace.<br /><br /> Pro zprávy, které vyžadují zabezpečení ExactlyOnce (tj. `exactlyOnce` atribut je nastaven na `true`hodnotu), se tento atribut standardně používá pro nedoručené transakční fronty v rámci systému MSMQ.<br /><br /> Pro zprávy, které nevyžadují žádné záruky, je tento atribut `null`nastaven na hodnotu.|  

...
## <a name="remarks"></a>Poznámky  
 `WSHttpBinding` Je podobná `BasicHttpBinding` , ale poskytuje další funkce, webové služby. Pomocí přenosového protokolu HTTP a poskytuje zabezpečení zpráv, jak to dělá BasicHttpBinding, ale také poskytuje transakce, spolehlivého zasílání zpráv a WS-Addressing, buď povolen ve výchozím nastavení, nebo dostupné prostřednictvím nastavení jeden ovládací prvek.  

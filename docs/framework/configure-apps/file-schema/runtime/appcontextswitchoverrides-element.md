...
|`System.Xml.`<br /><br /> `IgnoreEmptyKeySequences`|Určuje, zda jsou ignorovány prázdná pořadí klíčů v složených klíčů pomocí ověření schématu XSD. Další informace najdete v tématu [omezení rizik: Ověření schématu XML](~/docs/framework/migration-guide/mitigation-xml-schema-validation.md).|.NET Framework 4.6|  

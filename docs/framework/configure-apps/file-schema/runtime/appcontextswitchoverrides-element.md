...
|`Switch.System.Diagnostics.`<br/>`IgnorePortablePDBsInStackTraces`|Určuje, zda získat trasování zásobníku při použití přenosných PDB může obsahovat informace o zdroji souboru a řádku. `false` zdrojový soubor a řádku informace; v opačném `true`.|Rozhraní .NET framework 4.7.2|
...
|`Switch.System.Drawing.Text.`<br/>`DoNotRemoveGdiFontsResourcesFromFontCollection`|Určuje, zda <xref:System.Drawing.Text.PrivateFontCollection?displayProperty=nameWithType> objekty jsou správně uvolněno při přidání do kolekce pomocí <xref:System.Drawing.Text.PrivateFontCollection.AddFontFile(System.String)?displayProperty=nameWithType> metoda. `true` Chcete-li zachovat starší verze chování; `false` k uvolnění všech objektů privátní písma. |Rozhraní .NET framework 4.7.2|
...
|`Switch.System.IO.Compression.`<br/>`DoNotUseNativeZipLibraryForDecompression`|Určuje, zda nativní rutiny operačního systému se používá pro dekompresi pomocí <xref:System.IO.Compression.DeflateStream> třídy. `false` použití nativních rozhraní API; `true` používat <xref:System.IO.Compression.DeflateStream> implementace.|Rozhraní .NET framework 4.7.2|
...
|`Switch.System.Uri.`<br/>`DontEnableStrictRFC3986ReservedCharacterSets`|Určuje, zda některé kódováním procent znaky, které byly někdy dekódovat jsou nyní konzistentně vlevo kódovaný. Pokud `true`, jsou dekódované; v opačném `false`.|Rozhraní .NET framework 4.7.2|
|`Switch.System.Uri.`<br/>`DontKeepUnicodeBidiFormattingCharacters`|Určuje ošetření obousměrného znaky kódování Unicode v identifikátory URI. `true` pro odstranění je z identifikátory URI; `false` zachování a procent zakódovat je.|Rozhraní .NET framework 4.7.2|
...
|`Switch.System.Windows.Controls.Text.`<br/>`UseAdornerForTextboxSelectionRendering`|Určuje, zda je k dispozici pro vykreslování bez Adorner na základě výběru <xref:System.Windows.Controls.TextBox> a <xref:System.Windows.Controls.PasswordBox> ovládací prvky, aby se zabránilo occluded text (`false`), nebo zda je text reprezentován jenom ve vrstvě Adorner (`true`).|Rozhraní .NET framework 4.7.2|
...
|`Switch.System.Windows.Forms.`<br/>`UseLegacyContextMenuStripSourceControlValue`|Určuje, zda <xref:System.Windows.Forms.ContextMenuStrip.SourceControl?displayProperty=nameWithType> vlastnost vrací zdrojového kódu, když uživatel otevře v nabídce z vnořený <xref:System.Windows.Forms.ToolStripMenuItem> ovládacího prvku. `true` Chcete-li vrátit `null`, starší verze chování; `false` vrátit zdrojového kódu.|Rozhraní .NET framework 4.7.2|
|`Switch.System.Windows.Input.Stylus.`<br/>`EnablePointerSupport`|Určuje, zda je volitelný `WM_POINTER`– na základě touch/pera zásobníku je povolena v aplikacích WPF. Další informace najdete v tématu [omezení rizik: na základě ukazatel Touch a podporu pera](../../../migration-guide/mitigation-pointer-based-touch-and-stylus-support.md)|Rozhraní .NET framework 4.7|
|`Switch.System.Windows.Markup.`<br/>`DoNotUseSha256ForMarkupCompilerChecksumAlgorithm`|Určuje, zda je použitý pro kontrolní součty výchozí algoritmus hash SHA256 (`false`) nebo SHA1 (`true`).|Rozhraní .NET framework 4.7.2|
...
|`Switch.UseLegacyAccessibilityFeatures.2`|Ovládací prvky, jestli funkce usnadnění přístupu k dispozici v rozhraní .NET Framework 4.7.2 jsou povoleny (`false`) nebo zakázat (`true`). Pokud `true`, `Switch.UseLegacyAccessibilityFeatures` musí být také `true` k povolení funkcí rozhraní .NET Framework 4.7.1 usnadnění.|Rozhraní .NET framework 4.7.2|

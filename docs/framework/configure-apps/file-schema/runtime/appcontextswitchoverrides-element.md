...
ASP.NET aplikaci můžete nakonfigurovat tak, aby používala nastavení kompatibility přidáním [\<Add>](../appsettings/add-element-for-appsettings.md) elementu do [\<appSettings>](../appsettings/index.md) oddílu web.config souboru.

Následující příklad používá `<add>` prvek pro přidání dvou nastavení do `<appSettings>` oddílu web.config souboru:

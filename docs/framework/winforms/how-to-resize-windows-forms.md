...
Můžete zadat velikost formuláře Windows několika způsoby. Můžete změnit výška a šířka formuláře programově nastavením nové hodnoty pro <xref:System.Windows.Forms.Form.Size%2A> vlastnost, nebo upravit <xref:System.Windows.Forms.Control.Height%2A> nebo <xref:System.Windows.Forms.Control.Width%2A> vlastnosti jednotlivě. Pokud používáte [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)], můžete změnit velikost pomocí Návrhář formulářů Windows. Viz také [postupy: Změna velikosti Windows Forms pomocí návrháře](http://msdn.microsoft.com/library/37k2zkwx\(v=vs.110\)).  

---
title: "Ovládací prvky Windows Forms používané k výpisu možností"
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net-framework
ms.reviewer: 
ms.suite: 
ms.technology: dotnet-winforms
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- Windows Forms controls, listing options
- option lists in Windows Forms
ms.assetid: 5bc064c7-bc1f-4b62-8f4b-252f864b118e
caps.latest.revision: "10"
author: dotnet-bot
ms.author: dotnetcontent
manager: wpickett
ms.openlocfilehash: 0e1b26ec97f4b379e6b2d75a407408b8382bca52
ms.sourcegitcommit: bd1ef61f4bb794b25383d3d72e71041a5ced172e
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 10/18/2017
---
# <a name="windows-forms-controls-used-to-list-options"></a>Ovládací prvky Windows Forms používané k výpisu možností
Pokud chcete uživatelům poskytnout seznam na výběr možnosti, můžete přidat řadu ovládacích prvků na formuláři Windows. V závislosti na tom, kolik chcete omezit vaši uživatelé vstup, můžete přidat <xref:System.Windows.Forms.ListBox> řízení, <xref:System.Windows.Forms.ComboBox> ovládací prvek, nebo <xref:System.Windows.Forms.CheckedListBox> ovládacího prvku. Pomocí následujících odkazů k určení, který ovládací prvek nejlepší vyhovuje vašim potřebám.  
  
## <a name="in-this-section"></a>V tomto oddílu  
 [Kdy použít prvek Windows Forms ComboBox místo prvku ListBox](../../../../docs/framework/winforms/controls/when-to-use-a-windows-forms-combobox-instead-of-a-listbox.md)  
 Doporučuje vhodného na základě seznamu řízení v závislosti na požadavky a omezení svého formuláře systému Windows.  
  
 [Postupy: přístup ke konkrétním položek v systému Windows Forms ComboBox, ListBox nebo CheckedListBox – ovládací prvek](../../../../docs/framework/winforms/controls/access-specific-items-in-a-wf-combobox-listbox-or-checkedlistbox.md)  
 Poskytuje pokyny pro určení prostřednictvím kódu programu, která položka v seznamu se zobrazí v dané pozici.  
  
 [Postupy: Přidání a odebrání položek z Windows Forms ComboBox, ListBox nebo CheckedListBox – ovládací prvek](../../../../docs/framework/winforms/controls/add-and-remove-items-from-a-wf-combobox.md)  
 Poskytuje pokyny pro přidání nebo odebrání položek z ovládacího prvku seznam položek.  
  
 [Postupy: vytvoření vyhledávací tabulky pro Windows Forms ComboBox, ListBox nebo CheckedListBox – ovládací prvek](../../../../docs/framework/winforms/controls/create-a-lookup-table-for-a-wf-combobox-listbox.md)  
 Poskytuje pokyny pro zobrazení a ukládání dat formuláře v užitečné formátech.  
  
 [Postupy: vytvoření vazby Windows Forms ComboBox nebo ListBox – ovládací prvek k datům](../../../../docs/framework/winforms/controls/how-to-bind-a-windows-forms-combobox-or-listbox-control-to-data.md)  
 Poskytuje pokyny pro vytvoření vazby ovládacího prvku seznam založený na zdroj dat.  
  
 [Postupy: řazení obsahu ovládacího prvku Windows Forms ComboBox, ListBox nebo CheckedListBox – ovládací prvek](../../../../docs/framework/winforms/controls/sort-the-contents-of-a-wf-combobox-listbox-or-checkedlistbox-control.md)  
 Vysvětluje, jak řadit data seznamu na svůj zdroj dat.  
  
## <a name="reference"></a>Odkaz  
 <xref:System.Windows.Forms.CheckedListBox>  
 Tato třída popisuje a obsahuje odkazy na všechny její členy.  
  
 <xref:System.Windows.Forms.ComboBox>  
 Tato třída popisuje a obsahuje odkazy na všechny její členy.  
  
 <xref:System.Windows.Forms.ListBox>  
 Tato třída popisuje a obsahuje odkazy na všechny její členy.  
  
## <a name="related-sections"></a>Související oddíly  
 [Přehled ovládacího prvku CheckedListBox](../../../../docs/framework/winforms/controls/checkedlistbox-control-overview-windows-forms.md)  
 Vysvětluje, co je tento ovládací prvek a jeho klíčové funkce a vlastnosti.  
  
 [Přehled ovládacího prvku ComboBox](../../../../docs/framework/winforms/controls/combobox-control-overview-windows-forms.md)  
 Vysvětluje, co je tento ovládací prvek a jeho klíčové funkce a vlastnosti.  
  
 [Přehled ovládacího prvku ListBox](../../../../docs/framework/winforms/controls/listbox-control-overview-windows-forms.md)  
 Vysvětluje, co je tento ovládací prvek a jeho klíčové funkce a vlastnosti.  
  
 [Ovládací prvky používané ve formulářích Windows](../../../../docs/framework/winforms/controls/controls-to-use-on-windows-forms.md)  
 Poskytuje úplný seznam Windows Forms – ovládací prvky, odkazy na informace o jejich používání.

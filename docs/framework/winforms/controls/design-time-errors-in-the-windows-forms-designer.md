...
Pokud se Návrhář formulářů nepovede načíst z důvodu chyby ve vašem kódu, v komponentě třetí strany nebo jinde, zobrazí se místo návrháře chybová stránka. Tato chybová stránka nemusí nutně znamenat chybu v návrháři. Chyba se může nacházet někde na stránce s kódem na pozadí s názvem \<your-form-name> . Designer.cs. Chyby se zobrazují v sbalitelných žlutých pruzích s odkazem na přechod na umístění chyby na znakové stránce.
...
> - Pro aplikace v jazyce C++ nemají chyby odkazy na umístění kódu.
...
### <a name="identifier-name-is-not-a-valid-identifier"></a>' \<identifier name> ' není platný identifikátor.

Tato chyba označuje, že pole, metoda, událost nebo objekt jsou nesprávně pojmenovány.

### <a name="name-already-exists-in-project-name"></a>' \<name> ' již existuje v ' \<project name> '

Chybová zpráva: "' \<name> již existuje v ' \<project name> '. Zadejte prosím jedinečný název. "
...
### <a name="toolbox-tab-name-is-not-a-toolbox-category"></a>' \<Toolbox tab name> ' není kategorie panelu nástrojů.

Návrhář třetí strany se pokusil o přístup k kartě na panelu nástrojů, který neexistuje. Obraťte se na dodavatele součásti.

### <a name="a-requested-language-parser-is-not-installed"></a>Není nainstalovaný požadovaný jazykový analyzátor.
...
### <a name="a-service-required-for-generating-and-parsing-source-code-is-missing"></a>Chybí služba požadovaná pro generování a analýzu zdrojového kódu.

Jedná se o problém se součástí třetí strany. Obraťte se na dodavatele součásti.

### <a name="an-exception-occurred-while-trying-to-create-an-instance-of-object-name"></a>Při pokusu o vytvoření instance došlo k výjimce. \<object name>

Chybová zpráva: došlo k výjimce při pokusu o vytvoření instance \<object name> . Výjimka byla " \<exception string\> ".
...
### <a name="another-editor-has-document-name-open-in-an-incompatible-mode"></a>\<document name>V nekompatibilním režimu je otevřen jiný Editor.

Chybová zpráva: "jiný Editor byl \<document name> otevřen v nekompatibilním režimu. Zavřete prosím Editor a zkuste tuto operaci znovu. "
...
### <a name="another-editor-has-made-changes-to-document-name"></a>Jiný Editor provedl změny v ' \<document name> '
...
### <a name="another-editor-has-the-file-open-in-an-incompatible-mode"></a>Jiný editor má otevřený soubor v nekompatibilním režimu

Chybová zpráva: "jiný editor má otevřený soubor v nekompatibilním režimu. Zavřete prosím Editor a zkuste tuto operaci znovu. "

Tato zpráva je podobná "jiný Editor je \<document name> otevřen v nekompatibilním režimu", ale Visual Studio nemůže určit název souboru. Pokud chcete tuto chybu opravit, zavřete Editor, který má soubor otevřený, a zkuste to znovu.

### <a name="array-rank-rank-in-array-is-too-high"></a>Rozměr pole \<rank in array> je příliš vysoký.
...
Chybová zpráva: sestavení \<assembly name> nelze otevřít. Ověřte, zda soubor stále existuje. "
...
### <a name="cannot-access-the-visual-studio-toolbox-at-this-time"></a>V tuto chvíli nejde získat přístup k sadě nástrojů sady Visual Studio.
...
### <a name="cannot-bind-an-event-handler-to-the-event-name-event-because-it-is-read-only"></a>Nelze vytvořit vazby obslužné rutiny události k \<event name> události, protože je jen pro čtení.
...
### <a name="cannot-create-a-method-name-for-the-requested-component-because-it-is-not-a-member-of-the-design-container"></a>Nelze vytvořit název metody pro požadovanou součást, protože se nejedná o člen kontejneru návrhu.
...
### <a name="cannot-name-the-object-name-because-it-is-already-named-name"></a>Nelze pojmenovat objekt ' \<name> ', protože je již pojmenován ' \<name> '
...
### <a name="cannot-remove-or-destroy-inherited-component-component-name"></a>Zděděnou součást nelze odstranit ani zničit \<component name> .
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>Kategorie ' \<Toolbox tab name> ' nemá nástroj pro třídu ' \<class name> '
...
### <a name="class-class-name-has-no-matching-constructor"></a>Třída \<class name> nemá žádný vyhovující konstruktor.
...
### <a name="code-generation-for-property-property-name-failed"></a>Generování kódu pro vlastnost \<property name> ' ' se nezdařilo.
...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>Součást ' \<component name> ' nevolala do konstruktoru Container. Add ()
...
Chcete-li chybu opravit, obraťte se na dodavatele součásti. Nebo, pokud se jedná o komponentu, kterou jste vytvořili, zavolejte `IContainer.Add` metodu v konstruktoru komponenty.

### <a name="component-name-cannot-be-empty"></a>Název součásti nemůže být prázdný.

Tato chyba nastane, když se pokusíte přejmenovat komponentu na prázdnou hodnotu.

### <a name="could-not-access-the-variable-variable-name-because-it-has-not-been-initialized-yet"></a>Nelze získat přístup k proměnné ' \<variable name> ', protože dosud nebyla inicializována.
...
### <a name="could-not-find-type-type-name"></a>Nepovedlo se najít \<type name> typ.

Chybová zpráva: nepovedlo se najít typ \<type name> . Ujistěte se, že je odkazováno na sestavení, které obsahuje tento typ. Pokud je tento typ součástí projektu vývoje, ujistěte se, že projekt byl úspěšně sestaven. "
...
### <a name="could-not-load-type-type-name"></a>Typ nelze \<type name> načíst.

Chybová zpráva: typ nelze načíst \<type name> . Ujistěte se prosím, že se do odkazů projektu přidalo sestavení obsahující tento typ. "
...
### <a name="could-not-locate-the-project-item-templates-for-inherited-components"></a>Nejde najít šablony položek projektu pro zděděné součásti.
...
### <a name="delegate-class-class-name-has-no-invoke-method-is-this-class-a-delegate"></a>Třída delegáta \<class name> neobsahuje metodu Invoke. Je tato třída delegátem?
...
### <a name="duplicate-declaration-of-member-member-name"></a>Duplicitní deklarace člena \<member name>
...
### <a name="error-reading-resources-from-the-resource-file-for-the-culture-culture-name"></a>Při čtení prostředků ze zdrojového souboru pro jazykovou verzi ' ' došlo k chybě. ' \<culture name> '
...
### <a name="error-reading-resources-from-the-resource-file-for-the-default-culture-culture-name"></a>Při čtení prostředků ze souboru prostředků výchozí jazykové verze došlo k chybě \<culture name> .
...
### <a name="failed-to-parse-method-method-name"></a>Nepovedlo se analyzovat \<method name> metodu.

Chybová zpráva: Nepodařilo se analyzovat metodu \<method name> . Analyzátor ohlásil následující chybu: ' \<error string> '. Podívejte se prosím na Seznam úkolů potenciální chyby. "
...
### <a name="invalid-component-name-component-name"></a>Neplatný název součásti: ' \<component name> '
...
### <a name="the-type-class-name-is-made-of-several-partial-classes-in-the-same-file"></a>Typ \<class name> ' ' se skládá z několika částečných tříd ve stejném souboru
...
### <a name="the-assembly-assembly-name-could-not-be-found"></a>Sestavení \<assembly name> ' ' se nepodařilo najít.

Chybová zpráva: sestavení \<assembly name> nebylo nalezeno. Ujistěte se, že je odkazováno na sestavení. Pokud je sestavení součástí aktuálního vývojového projektu, ujistěte se, že projekt byl sestaven. "

Tato chyba je podobná typu "typ" nebyl \<type name> nalezen, ale k této chybě obvykle dochází z důvodu atributu metadata. Chcete-li tuto chybu opravit, ověřte, zda jsou odkazována všechna sestavení používaná atributy.

### <a name="the-assembly-name-assembly-name-is-invalid"></a>Název sestavení \<assembly name> ' ' je neplatný.
...
### <a name="the-base-class-class-name-cannot-be-designed"></a>Základní třídu \<class name> nelze navrhnout.
...
### <a name="the-base-class-class-name-could-not-be-loaded"></a>Základní třídu ' \<class name> ' se nepovedlo načíst.
...
### <a name="the-class-class-name-cannot-be-designed-in-this-version-of-visual-studio"></a>Třída \<class name> nemůže být navržena v této verzi sady Visual Studio.
...
### <a name="the-class-name-is-not-a-valid-identifier-for-this-language"></a>Název třídy není platným identifikátorem pro tento jazyk.
...
### <a name="the-component-cannot-be-added-because-it-contains-a-circular-reference-to-reference-name"></a>Komponentu nelze přidat, protože obsahuje cyklický odkaz na ' \<reference name> '
...
### <a name="the-designer-cannot-be-modified-at-this-time"></a>V tuto chvíli nelze změnit návrháře.
...
### <a name="the-designer-could-not-be-shown-for-this-file-because-none-of-the-classes-within-it-can-be-designed"></a>Pro tento soubor nelze zobrazit návrháře, protože žádná z tříd v rámci něj nemůže být navržena.
...
### <a name="the-designer-for-base-class-class-name-is-not-installed"></a>Návrhář pro základní třídu není \<class name> nainstalován.
...
### <a name="the-designer-must-create-an-instance-of-type-type-name-but-it-cant-because-the-type-is-declared-as-abstract"></a>Návrhář musí vytvořit instanci typu \<type name> ' ', ale nelze jej, protože typ je deklarován jako abstraktní.
...
### <a name="the-file-could-not-be-loaded-in-the-designer"></a>Soubor se nepovedlo načíst do návrháře.
...
### <a name="the-language-for-this-file-does-not-support-the-necessary-code-parsing-and-generation-services"></a>Jazyk tohoto souboru nepodporuje nezbytné služby pro analýzu a generování kódu.
...
### <a name="the-language-parser-class-class-name-is-not-implemented-properly"></a>Třída analyzátoru jazyka \<class name> není správně implementovaná.

Chybová zpráva: třída analyzátoru jazyka není \<class name> správně implementovaná. Obraťte se na dodavatele s aktualizovaným modulem analyzátoru. "
...
### <a name="the-name-name-is-already-used-by-another-object"></a>Název \<name> ' ' je již využíván jiným objektem.
...
### <a name="the-object-object-name-does-not-implement-the-icomponent-interface"></a>Objekt \<object name> neimplementuje rozhraní IComponent.

Aplikace Visual Studio se pokusila o vytvoření komponenty, ale objekt, který vytvořil, neimplementuje <xref:System.ComponentModel.IComponent> rozhraní. Pro opravu se obraťte na dodavatele součásti.

### <a name="the-object-object-name-returned-null-for-the-property-property-name-but-this-is-not-allowed"></a>Objekt ' \<object name> ' vrátil hodnotu null pro vlastnost ' \<property name> ', ale tato hodnota není povolena.
...
### <a name="the-serialization-data-object-is-not-of-the-proper-type"></a>Datový objekt serializace není správného typu.
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>Služba \<service name> je povinná, ale nebyla nalezena.

Chybová zpráva: Služba \<service name> je povinná, ale nebyla nalezena. Může se jednat o problém s instalací sady Visual Studio. "
...
### <a name="the-service-instance-must-derive-from-or-implement-interface-name"></a>Instance služby musí být odvozena z nebo implementace \<interface name> .
...
### <a name="the-text-in-the-code-window-could-not-be-modified"></a>Text v okně kódu nebylo možné změnit.
...
### <a name="the-toolbox-enumerator-object-only-supports-retrieving-one-item-at-a-time"></a>Objekt enumerátoru sady nástrojů podporuje pouze načítání jedné položky v daném okamžiku.
...
### <a name="the-toolbox-item-for-component-name-could-not-be-retrieved-from-the-toolbox"></a>\<component name>Z panelu nástrojů nelze načíst položku sady nástrojů pro ' '

Chybová zpráva: položku sady nástrojů pro ' \<component name> ' nelze načíst ze sady nástrojů. Ujistěte se, že sestavení, které obsahuje položku sady nástrojů, je správně nainstalováno. Položka sady nástrojů vyvolala následující chybu: \<error string> . "

V dané součásti došlo k výjimce při jejím použití v aplikaci Visual Studio. Obraťte se na dodavatele součásti.

### <a name="the-toolbox-item-for-toolbox-item-name-could-not-be-retrieved-from-the-toolbox"></a>\<Toolbox item name>Z panelu nástrojů nelze načíst položku sady nástrojů pro ' '

Chybová zpráva: položku sady nástrojů pro ' \<Toolbox item name> ' nelze načíst ze sady nástrojů. Zkuste položku odebrat ze sady nástrojů a přidat ji zpátky. "
...
### <a name="the-type-type-name-could-not-be-found"></a>Typ \<type name> ' ' nebyl nalezen.

Chybová zpráva: typ "" nebyl \<type name> nalezen. Ujistěte se, že je odkazováno na sestavení, které obsahuje daný typ. Pokud je sestavení součástí aktuálního vývojového projektu, ujistěte se, že projekt byl sestaven. "
...
### <a name="the-type-resolution-service-may-only-be-called-from-the-main-application-thread"></a>Služba pro rozlišení typu se může volat jenom z hlavního vlákna aplikace.
...
### <a name="the-variable-variable-name-is-either-undeclared-or-was-never-assigned"></a>Proměnná ' \<variable name> ' je buď nedeklarovaná, nebo nebyla nikdy přiřazena.
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Pro příkaz nabídky ' ' již existuje obslužná rutina příkazu ' \<menu command name> '
...
### <a name="there-is-already-a-component-named-component-name"></a>Komponenta s názvem již \<component name> existuje.

Chybová zpráva: již existuje součást s názvem \<component name> . Komponenty musí mít jedinečné názvy a názvy nesmí rozlišovat velká a malá písmena. Název také nemůže být v konfliktu s názvem žádné součásti ve zděděné třídě. "
...
### <a name="there-is-already-a-toolbox-item-creator-registered-for-the-format-format-name"></a>Pro formát se už zaregistroval tvůrce položky sady \<format name> nástrojů.
...
### <a name="this-language-engine-does-not-support-a-codemodel-with-which-to-load-a-designer"></a>Tento jazykový modul nepodporuje CodeModel, se kterým se má načíst Návrhář.
...
### <a name="type-type-name-does-not-have-a-constructor-with-parameters-of-types-parameter-type-names"></a>Typ \<type name\> nemá konstruktor s parametry typů \<parameter type names> .
...
### <a name="unable-to-add-reference-reference-name-to-the-current-application"></a>Do aktuální aplikace se nedá přidat odkaz. \<reference name>

Chybová zpráva: "nelze přidat odkaz ' \<reference name> ' do aktuální aplikace. Ověřte, zda již není odkazováno na jinou verzi ' \<reference name> '.
...
### <a name="unable-to-check-out-the-current-file"></a>Nepovedlo se rezervovat aktuální soubor.
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Nepovedlo se najít stránku s názvem \<Options dialog box tab name> .
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Nelze najít vlastnost ' \<property name> ' na stránce ' \<Options dialog box tab name> '
...
### <a name="visual-studio-cannot-open-a-designer-for-the-file-because-the-class-within-it-does-not-inherit-from-a-class-that-can-be-visually-designed"></a>Visual Studio nemůže otevřít návrháře pro soubor, protože třída, která se v něm nachází, nedědí ze třídy, kterou je možné vizuálně navrhovat.
...
### <a name="visual-studio-cannot-save-or-load-instances-of-the-type-type-name"></a>Visual Studio nemůže uložit nebo načíst instance \<type name> typu.

Jedná se o problém se součástí třetí strany. Obraťte se na dodavatele součásti.

### <a name="visual-studio-is-unable-to-open-document-name-in-design-view"></a>Visual Studio nemůže otevřít \<document name> v zobrazení Návrh.

Chybová zpráva: "aplikace Visual Studio nemůže otevřít ' \<document name> ' v zobrazení Návrh. Pro tento typ souboru není nainstalován žádný analyzátor. "

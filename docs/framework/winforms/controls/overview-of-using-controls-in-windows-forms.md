...
## <a name="simple-windows-forms-applications"></a>Jednoduché aplikace windows ových formulářů  
 Aplikace Windows Forms se přinejmenším skládá z následujících prvků:  
  
- Jedna nebo více tříd, které jsou odvozeny z <xref:System.Windows.Forms.Form?displayProperty=nameWithType>.  
  
- Metoda, `Main` která vyvolá `static` `shared` <xref:System.Windows.Forms.Application.Run%2A> metodu ( v jazyce Visual Basic) a předá jí <xref:System.Windows.Forms.Form> instanci. Metoda <xref:System.Windows.Forms.Application.Run%2A> zpracovává zprávy z operačního systému do aplikace.  
...
## <a name="using-controls-in-a-windows-forms-application"></a>Použití ovládacích prvků v aplikaci windows forms  
 Následující příklad kódu ukazuje jednoduchou aplikaci, která ilustruje, jak aplikace windows forms používají ovládací prvky a zpracovávají události. Příklad se skládá ze tří tlačítek ve formuláři; každé tlačítko změní barvu pozadí po klepnutí.  

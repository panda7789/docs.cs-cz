...
|Scénář|Implementace|  
|--------------|--------------------|  
|Režim výplně sloupce slouží k zobrazení dat podobné velikosti v relativně malém počtu sloupců, které zabírají celou šířku ovládacího prvku bez zobrazení vodorovného posuvníku.|Vlastnost <xref:System.Windows.Forms.DataGridView.AutoSizeColumnsMode%2A> nastavte na hodnotu <xref:System.Windows.Forms.DataGridViewAutoSizeColumnsMode.Fill>.|  
|Použijte režim vyplnění sloupce s hodnotami zobrazení různých velikostí.|Vlastnost <xref:System.Windows.Forms.DataGridView.AutoSizeColumnsMode%2A> nastavte na hodnotu <xref:System.Windows.Forms.DataGridViewAutoSizeColumnsMode.Fill>. Proveďte inicializaci relativních šířek sloupců nastavením vlastností sloupce <xref:System.Windows.Forms.DataGridViewColumn.FillWeight%2A> nebo voláním metody ovládacího prvku <xref:System.Windows.Forms.DataGridView.AutoResizeColumns%2A> po vyplnění ovládacího prvku daty.|  
|Použijte režim vyplnění sloupce s hodnotami proměnlivá důležitost.|Vlastnost <xref:System.Windows.Forms.DataGridView.AutoSizeColumnsMode%2A> nastavte na hodnotu <xref:System.Windows.Forms.DataGridViewAutoSizeColumnsMode.Fill>. Nastavte velké <xref:System.Windows.Forms.DataGridViewColumn.MinimumWidth%2A> hodnoty pro sloupce, které musí vždy zobrazovat některá data nebo použít jinou možnost změny velikosti než režim výplně pro konkrétní sloupce.|  

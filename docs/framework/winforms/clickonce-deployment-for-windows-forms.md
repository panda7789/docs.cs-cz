...
Následující témata popisují technologii ClickOnce, která se používá k snadnému nasazení model Windows Forms aplikací do klientských počítačů.  
  
## <a name="related-sections"></a>Související oddíly  
 [Výběr strategie nasazení ClickOnce](/visualstudio/deployment/choosing-a-clickonce-deployment-strategy)  
 Nabízí několik možností pro nasazení aplikací ClickOnce.  
  
 [Výběr strategie aktualizace ClickOnce](/visualstudio/deployment/choosing-a-clickonce-update-strategy)  
 Nabízí několik možností pro aktualizaci aplikací ClickOnce.  
  
 [Zabezpečování aplikací ClickOnce](/visualstudio/deployment/securing-clickonce-applications)  
 Vysvětluje důsledky zabezpečení při nasazení ClickOnce.  
  
 [Řešení potíží s nasazením ClickOnce](/visualstudio/deployment/troubleshooting-clickonce-deployments)  
 Popisuje různé problémy, ke kterým může dojít při nasazování aplikací ClickOnce a dokumentuje chybové zprávy nejvyšší úrovně, které ClickOnce může generovat.  
  
 [ClickOnce a nastavení aplikace](/visualstudio/deployment/clickonce-and-application-settings)  
 Popisuje, jak ClickOnce nasazení funguje s nastavením aplikace, které ukládá nastavení aplikace a uživatele pro budoucí načítání.  
  
 [Přehled nasazení důvěryhodných aplikací](/visualstudio/deployment/trusted-application-deployment-overview)  
 Popisuje funkci ClickOnce, která umožňuje, aby důvěryhodné aplikace běžely s vyšší úrovní oprávnění na klientských počítačích.  
  
 [ClickOnce a kód Authenticode](/visualstudio/deployment/clickonce-and-authenticode)  
 Popisuje, jak se technologie Authenticode používá při nasazení důvěryhodných aplikací.  
  
 [Návod: Ruční nasazení aplikace ClickOnce](/visualstudio/deployment/walkthrough-manually-deploying-a-clickonce-application)  
 Ukazuje použití nástrojů příkazového řádku a sady SDK k nasazení aplikace ClickOnce bez použití aplikace Visual Studio.  
 ...
 Demonstruje jednorázovou konfiguraci klientských počítačů nezbytných pro nasazení důvěryhodných aplikací.  
 ...
 Ukazuje konfiguraci aplikace ClickOnce pomocí nástrojů sady SDK pro kontrolu jiného umístění pro nové verze aplikace.  
 ...
 Ukazuje použití volání rozhraní API k načtení sestavení při prvním pokusu aplikace o jeho načtení.  
 ...
 Ukazuje načítání parametrů z adresy URL použité ke spuštění aplikace ClickOnce.  
  
 [ClickOnce – přehled mezipaměti](/visualstudio/deployment/clickonce-cache-overview)  
 Popisuje mezipaměť, která se používá k ukládání aplikací ClickOnce na místním počítači.  
 ...
 Popisuje, jak přistupovat k místním datovým souborům a vzdáleným zdrojům dat z aplikace ClickOnce.  

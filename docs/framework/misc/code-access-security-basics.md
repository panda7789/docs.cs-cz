...
Následující fragment kódu ukazuje imperativní syntaxi pro vyžádání, že volající vašeho kódu mají vlastní oprávnění s názvem `MyPermission`. Toto oprávnění je hypotetické vlastní oprávnění a neexistuje v .NET Framework. V `MyMethod`se vytvoří nová instance `MyPermission`, která chrání jenom tuto metodu se voláním zabezpečení.

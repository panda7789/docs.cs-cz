...
> [!NOTE]
> V této části se dozvíte o potížích se zabezpečením při deklaraci metody jako obou `virtual` i `internal` ( `Overloads` `Overridable` `Friend` v Visual Basic). Toto upozornění se vztahuje pouze na verze .NET Framework 1,0 a 1,1, ale nevztahuje se na novější verze.  

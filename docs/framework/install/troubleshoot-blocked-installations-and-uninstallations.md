...
|Rozhraní .NET Framework 4.5 *.x*/4.6 *.x*/4.7 *.x* je již součástí tohoto operačního systému. Není potřeba instalovat rozhraní .NET Framework 4.5 *.x*/4.6 *.x*/4.7 *.x* redistributable.|Žádná akce.<br /><br /> Pokud chcete zjistit, jaké verze rozhraní .NET Framework jsou nainstalovány v systému, naleznete v tématu [jak: Zjištění nainstalovaných verzí rozhraní .NET Framework](../../../docs/framework/migration-guide/how-to-determine-which-versions-are-installed.md). Zobrazit [požadavky na systém](../../../docs/framework/get-started/system-requirements.md) pro podporované operační systémy.|  

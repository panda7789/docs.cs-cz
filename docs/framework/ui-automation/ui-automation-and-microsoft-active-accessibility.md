...
## <a name="servers-and-clients"></a>Servery a klienty  
 ...
 Zprostředkovatelé automatizace uživatelského rozhraní můžete zadat informace Active Accessibility klientům a Active Accessibility servery mohou poskytovat informace pro automatizaci uživatelského rozhraní klientské aplikace. Ale protože Active Accessibility nevystavuje co nejvíce informací [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)], dva modely nejsou plně kompatibilní.  
  
<a name="UI_Elements_compare"></a>   
## <a name="ui-elements"></a>Prvky uživatelského rozhraní  
 Uvede Active Accessibility [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] prvky buď jako `IAccessible` rozhraní nebo jako identifikátor podřízené. Je obtížné pro porovnání dvou `IAccessible` ukazatele k určení, pokud se odkazují na stejný prvek.  
...
## <a name="tree-views-and-navigation"></a>Stromová zobrazení a navigaci  
 ...
 Mnoho pohyb mezi elementy automatizace, které se koncovým uživatelům Active Accessibility jsou vystaveny ve stromové struktuře. Podívat se na všechny prvky k určení, které mají smysl mít klientské aplikace.  
 ...
 Navigace mezi elementy v Active Accessibility, je buď spatial (například přechodu na element, který je na levé straně na obrazovce), logický (například přechod na další položku nabídky nebo na další položku v pořadí v rámci dialogového okna), nebo hierarchické () například přesun prvním podřízeným objektem kontejneru, nebo z podřízené k nadřazené úloze). Hierarchická navigace je složité fakt, že podřízené prvky nejsou vždy objekty, které implementují `IAccessible`.  
...
## <a name="roles-and-control-types"></a>Role a typy ovládacích prvků  
 ...
 V následující tabulce jsou uvedeny mapování role Active Accessibility [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] řídit typy.  
  
|Role služby Active Accessibility|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] Typ ovládacího prvku|  
|----------------------------------------------------------------------|----------------------------------------------------------------------------------------|  
...
## <a name="states-and-properties"></a>Stavy a vlastnosti  
 V Active Accessibility podporují prvky společnou sadu vlastností a některé vlastnosti (například `accState`) musí popisovat velmi různé věci, v závislosti na roli elementu. Servery musí implementovat všechny metody `IAccessible` , který vrátí vlastnosti, včetně těch, které nejsou relevantní pro element.  
  
 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] definuje mnoho dalších vlastností, z nichž některé odpovídají stavům v Active Accessibility. Některé jsou společné pro všechny prvky, ale ostatní jsou specifické pro typy ovládacích prvků a vzorů ovládacích prvků. Vlastnosti se rozlišují výhradně podle jedinečné identifikátory a většinu vlastností lze načíst s použitím jedné metody <xref:System.Windows.Automation.AutomationElement.GetCurrentPropertyValue%2A> nebo <xref:System.Windows.Automation.AutomationElement.GetCachedPropertyValue%2A>. Mnoho vlastností jsou také snadno získat z <xref:System.Windows.Automation.AutomationElement.Current%2A> a <xref:System.Windows.Automation.AutomationElement.Cached%2A> přistupující objekty vlastnosti.  
...
|`get_accHelpTopic`|Není podporován v [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]||  
 ...
 Následující tabulka uvádí, které [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] vlastnosti odpovídají Active Accessibility stavu konstanty.  
  
|Stav Active Accessibility|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] Vlastnost|Změna stavu aktivace?|  
|-----------------------------------------------------------------------|------------------------------------------------------------------------------------|----------------------------|  
...
|STATE_SYSTEM_UNAVAILABLE|<xref:System.Windows.Automation.AutomationElement.IsEnabledProperty>|Ano|  
 ...
 Následující stavy buď nebyly prováděny většina Active Accessibility řídit servery nebo nemají žádný ekvivalent v [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)].  
  
|Stav Active Accessibility|Poznámky|  
|-----------------------------------------------------------------------|-------------|  
...
|STATE_SYSTEM_EXTSELECTABLE|Není široce implementované servery Active Accessibility|  
|STATE_SYSTEM_MARQUEED|Není široce implementované servery Active Accessibility|  
|STATE_SYSTEM_SELFVOICING|Není široce implementované servery Active Accessibility|  
|STATE_SYSTEM_TRAVERSED|Není k dispozici v [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]|  
|STATE_SYSTEM_ALERT_HIGH|Není široce implementované servery Active Accessibility|  
|STATE_SYSTEM_ALERT_MEDIUM|Není široce implementované servery Active Accessibility|  
|STATE_SYSTEM_ALERT_LOW|Není široce implementované servery Active Accessibility|  
|STATE_SYSTEM_FLOATING|Není široce implementované servery Active Accessibility|  
...
## <a name="events"></a>Události  
 Mechanismus události v [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)], na rozdíl od Active Accessibility, nevyžaduje Windows směrování událostí (která je úzce vázané pomocí okna úchyty) a nevyžaduje, aby klientská aplikace nastavit zavěšení. Odběry událostí můžete doladíte, ne jenom na konkrétní události, ale konkrétní části stromu. Poskytovatelé lze optimalizovat také při jejich vyvolávání událostí udržovat přehled o jaké události se sleduje pro.  
 ...
 Následující tabulka ukazuje souvztažnost Active Accessibility WinEvents a [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] události.  
...
|EVENT_OBJECT_REORDER|V Active Accessibility použít není konzistentní. Žádné přímo odpovídající událost je definována v [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)].|  

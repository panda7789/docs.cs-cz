...
---
# <a name="annotations-schema"></a><span data-ttu-id="65494-102">Schéma poznámek</span><span class="sxs-lookup"><span data-stu-id="65494-102">Annotations Schema</span></span>
<span data-ttu-id="65494-103">Toto téma popisuje definici schématu XML (XSD) používá rozhraní Microsoft Framework poznámky k ukládání a načítání dat anotace uživatele.</span><span class="sxs-lookup"><span data-stu-id="65494-103">This topic describes the XML schema definition (XSD) used by the Microsoft Annotations Framework to save and retrieve user annotation data.</span></span>  
  
 <span data-ttu-id="65494-104">[!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] Serializuje anotace data z interního vyjádření na formátu XML.</span><span class="sxs-lookup"><span data-stu-id="65494-104">The [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] serializes annotation data from an internal representation to an XML format.</span></span>  <span data-ttu-id="65494-105">Formát XML používaný pro tento převod je popsán [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] schéma XSD.</span><span class="sxs-lookup"><span data-stu-id="65494-105">The XML format used for this conversion is described by the [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XSD Schema.</span></span>  <span data-ttu-id="65494-106">Schéma definuje ve formátu XML nezávislého na implementaci, který slouží k výměně anotace dat mezi aplikacemi.</span><span class="sxs-lookup"><span data-stu-id="65494-106">The schema defines the implementation-independent XML format that can be used to exchange annotation data between applications.</span></span>  
  
 <span data-ttu-id="65494-107">[!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] Definici schématu XML se skládá ze dvou subschemas</span><span class="sxs-lookup"><span data-stu-id="65494-107">The [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XML schema definition consists of two subschemas</span></span>  
  
-   <span data-ttu-id="65494-108">Poznámky Core schématu XML (základní schéma).</span><span class="sxs-lookup"><span data-stu-id="65494-108">The Annotations XML Core Schema (Core Schema).</span></span>  
  
-   <span data-ttu-id="65494-109">Komentáře XML základní schéma (základní schéma).</span><span class="sxs-lookup"><span data-stu-id="65494-109">The Annotations XML Base Schema (Base Schema).</span></span>  
  
 <span data-ttu-id="65494-110">Základní schéma definuje strukturu XML primární <xref:System.Windows.Annotations.Annotation>.</span><span class="sxs-lookup"><span data-stu-id="65494-110">The Core Schema defines the primary XML structure of an <xref:System.Windows.Annotations.Annotation>.</span></span>  <span data-ttu-id="65494-111">Většina XML elementů definováno ve schématu Core odpovídají typům v <xref:System.Windows.Annotations> oboru názvů.</span><span class="sxs-lookup"><span data-stu-id="65494-111">The majority of XML elements defined in the Core Schema correspond to types in the <xref:System.Windows.Annotations> namespace.</span></span>  <span data-ttu-id="65494-112">Základní schéma poskytuje tři Rozšiřovací body, ve kterém aplikace můžete přidat svoje vlastní data XML.</span><span class="sxs-lookup"><span data-stu-id="65494-112">The Core Schema exposes three extension points where applications can add their own XML data.</span></span>  <span data-ttu-id="65494-113">Zahrnout tyto Rozšiřovací body <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>nebo "Obsah".</span><span class="sxs-lookup"><span data-stu-id="65494-113">These extension points include the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and "Content".</span></span>  <span data-ttu-id="65494-114">(Obsahu prvky jsou k dispozici ve formě <xref:System.Xml.XmlElement> seznamu.)</span><span class="sxs-lookup"><span data-stu-id="65494-114">(Content elements are provided in the form of an <xref:System.Xml.XmlElement> list.)</span></span>  
  
 <span data-ttu-id="65494-115">Definuje rozšíření pro základní schéma popsaných v tomto tématu <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>a obsah typů, které jsou zahrnuty v počáteční verzi Windows Presentation Foundation (WPF).</span><span class="sxs-lookup"><span data-stu-id="65494-115">The Base Schema described in this topic defines the extensions for the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and Content types included with the initial Windows Presentation Foundation (WPF) release.</span></span>  
  
<a name="CoreSchema"></a>   
## <a name="annotations-xml-core-schema"></a><span data-ttu-id="65494-116">Schéma XML Core poznámek</span><span class="sxs-lookup"><span data-stu-id="65494-116">Annotations XML Core Schema</span></span>  
 <span data-ttu-id="65494-117">Schéma poznámek XML Core definuje strukturu XML, který se používá k ukládání <xref:System.Windows.Annotations.Annotation> objekty.</span><span class="sxs-lookup"><span data-stu-id="65494-117">The Annotations XML Core Schema defines the XML structure that is used to store <xref:System.Windows.Annotations.Annotation> objects.</span></span>  
...
<a name="BaseSchema"></a>   
## <a name="annotations-xml-base-schema"></a><span data-ttu-id="65494-118">Základní schéma XML poznámek</span><span class="sxs-lookup"><span data-stu-id="65494-118">Annotations XML Base Schema</span></span>  
 <span data-ttu-id="65494-119">Základní schéma definuje strukturu XML pro tři abstraktní elementů definováno ve schématu Core – <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, a <xref:System.Windows.Annotations.AnnotationResource.Contents%2A>.</span><span class="sxs-lookup"><span data-stu-id="65494-119">The Base Schema defines the XML structure for the three abstract elements defined in the Core Schema – <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and <xref:System.Windows.Annotations.AnnotationResource.Contents%2A>.</span></span>  
...
<a name="SampleXML"></a>   
## <a name="sample-xml-produced-by-annotations-xmlstreamstore"></a><span data-ttu-id="65494-120">Ukázkový soubor XML vytvořené metodou XmlStreamStore poznámky</span><span class="sxs-lookup"><span data-stu-id="65494-120">Sample XML Produced by Annotations XmlStreamStore</span></span>  
 <span data-ttu-id="65494-121">XML, který následuje zobrazí výstup anotací <xref:System.Windows.Annotations.Storage.XmlStreamStore> a organizaci, který obsahuje tři anotace – zvýraznění textu rychlé – poznámky a stonek ink-note ukázkový soubor.</span><span class="sxs-lookup"><span data-stu-id="65494-121">The XML that follows shows the output of an Annotations <xref:System.Windows.Annotations.Storage.XmlStreamStore> and the organization of a sample file that contains three annotations - a highlight, a text sticky-note, and an ink stick-note.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="65494-122">Viz také:</span><span class="sxs-lookup"><span data-stu-id="65494-122">See also</span></span>
...
- [<span data-ttu-id="65494-123">Přehled poznámek</span><span class="sxs-lookup"><span data-stu-id="65494-123">Annotations Overview</span></span>](../../../../docs/framework/wpf/advanced/annotations-overview.md)

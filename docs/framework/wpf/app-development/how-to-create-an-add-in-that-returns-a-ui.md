...
## <a name="implementing-the-contract-pipeline-segment"></a>Implementace segmentu kanálu smluv  
 Metoda musí být definována kontraktem pro vrácení uživatelského rozhraní a vrácená hodnota musí být typu <xref:System.AddIn.Contract.INativeHandleContract>. To je znázorněno `GetAddInUI` metodou `IWPFAddInContract` kontraktu v následujícím kódu.  
...
## <a name="implementing-the-add-in-view-pipeline-segment"></a>Implementace segmentu kanálu zobrazení doplňku  
 Vzhledem k tomu <xref:System.Windows.FrameworkElement>, že doplněk implementuje [!INCLUDE[TLA2#tla_ui#plural](../../../../includes/tla2sharptla-uisharpplural-md.md)] třídu, která poskytuje jako podtříd třídy, metoda v zobrazení doplňku, která koreluje s `IWPFAddInView.GetAddInUI` musí vracet hodnotu typu <xref:System.Windows.FrameworkElement>. Následující kód ukazuje zobrazení smlouvy o doplňku, které je implementováno jako rozhraní.  
...
## <a name="implementing-the-add-in-side-adapter-pipeline-segment"></a>Implementace segmentu kanálu adaptéru na straně doplňku  
 Metoda kontraktu vrátí <xref:System.AddIn.Contract.INativeHandleContract>, ale doplněk vrátí a <xref:System.Windows.FrameworkElement> (jak je uvedeno v zobrazení doplňku). V <xref:System.Windows.FrameworkElement> důsledku toho musí být převod <xref:System.AddIn.Contract.INativeHandleContract> na hranici před přejezdem hranice izolace. Tuto práci provádí adaptér doplňku na straně voláním <xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>, jak je znázorněno v následujícím kódu.  
...
## <a name="implementing-the-host-view-pipeline-segment"></a>Implementace segmentu kanálu zobrazení hostitele  
 Vzhledem k tomu, že hostitelská <xref:System.Windows.FrameworkElement>aplikace zobrazí a, metoda v zobrazení hostitele, ke `IWPFAddInHostView.GetAddInUI` kterému koreluje, musí vracet hodnotu <xref:System.Windows.FrameworkElement>typu. Následující kód ukazuje zobrazení hostitele kontraktu, implementované jako rozhraní.  
...
## <a name="implementing-the-host-side-adapter-pipeline-segment"></a>Implementace segmentu kanálu adaptéru na straně hostitele  
 Metoda kontraktu vrátí <xref:System.AddIn.Contract.INativeHandleContract>, ale hostitelská aplikace <xref:System.Windows.FrameworkElement> očekává (jak je určeno zobrazením hostitele). V <xref:System.AddIn.Contract.INativeHandleContract> důsledku toho musí být převod na hranici <xref:System.Windows.FrameworkElement> izolace převeden na. Tuto práci provádí hostitelský adaptér na straně hostitele voláním <xref:System.AddIn.Pipeline.FrameworkElementAdapters.ContractToViewAdapter%2A>, jak je znázorněno v následujícím kódu.  
...
## <a name="implementing-the-add-in"></a>Implementace doplňku  
 V případě, že je vytvořen adaptér doplňku a zobrazení doplňku, musí doplněk`WPFAddIn1.AddIn`() `IWPFAddInView.GetAddInUI` implementovat metodu pro <xref:System.Windows.Controls.UserControl> vrácení <xref:System.Windows.FrameworkElement> objektu (v tomto příkladu). Implementace <xref:System.Windows.Controls.UserControl> ,`AddInUI`, je zobrazena v následujícím kódu.  
 ...
 Implementace `IWPFAddInView.GetAddInUI` pomocí doplňku jednoduše potřebuje vrátit novou `AddInUI`instanci, jak je znázorněno v následujícím kódu.  
...
## <a name="implementing-the-host-application"></a>Implementace hostitelské aplikace  
 S vytvořeným hostitelským adaptérem a zobrazením hostitele může hostitelská aplikace použít model doplňku .NET Framework k otevření kanálu, získání zobrazení hostitele doplňku a volání `IWPFAddInHostView.GetAddInUI` metody. Tyto kroky jsou uvedeny v následujícím kódu.  

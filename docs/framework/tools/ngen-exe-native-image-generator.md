...
- Nová akce způsobí, `update` že znovu vytvoří image, jejichž platnost byla zrušena.
...
Další informace o používání Ngen.exe a nativní bitové kopie najdete v tématu [nativní Image Service](#native-image-service).

> [!NOTE]
> Syntaxe Ngen.exe pro verze 1,0 a 1,1 .NET Framework se dá najít v [předchozí syntaxi generátoru nativních imagí (Ngen.exe)](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/ms165073(v=vs.100)).
...
V následující tabulce je uvedena syntaxe každého z nich `action` . Popisy jednotlivých částí sady `action` najdete v tabulkách [argumenty](#ArgumentTable), [úrovně priority](#PriorityTable), [scénáře](#ScenarioTable)a [Konfigurace](#ConfigTable) . V tabulce [Options](#OptionTable) jsou popsány `options` přepínače Help a.
...
|--------------|-----------------|
|`assemblyName`|Plný zobrazovaný název sestavení. Například, `"myAssembly, Version=2.0.0.0, Culture=neutral, PublicKeyToken=0038abc9deabfle5"`. **Poznámka:**  `myAssembly`Pro akce a můžete uvést částečný název sestavení, například `display` `uninstall` . <br /><br /> V jednom příkazovém řádku nástroje Ngen.exe lze zadat pouze jedno sestavení.|
...
> [!CAUTION]
> Nástroj Ngen.exe nespouštějte pro sestavení, která nemají plnou důvěryhodnost. Počínaje .NET Framework 4 Ngen.exe zkompiluje sestavení s úplným vztahem důvěryhodnosti a zásady CAS (Code Access Security) již nejsou vyhodnocovány.

Počínaje verzí .NET Framework 4 již nelze načíst nativní bitové kopie, které jsou vygenerovány pomocí Ngen.exe, do aplikací spuštěných s částečnou důvěryhodností. Namísto toho je vyvolán kompilátor za běhu (JIT).
...
Nástroj Ngen.exe udržuje počet odkazů na závislosti. Předpokládejme například, že `MyAssembly.exe` a `YourAssembly.exe` jsou nainstalovány v mezipaměti nativní bitové kopie a oba mají odkazy na `OurDependency.dll` . Pokud `MyAssembly.exe` je odinstalován, nedojde `OurDependency.dll` k odinstalaci. Odstraní se jenom v případě, že `YourAssembly.exe` se taky odinstaluje.
...
- Použijte <xref:System.LoaderOptimizationAttribute> atribut pro aplikaci.
...
Například pokud spustíte sestavení ve scénáři ladění nebo profilace, modul runtime vyhledá nativní bitovou kopii, která byla vygenerována s `/Debug` `/Profile` možnostmi nebo. Pokud nemůže najít odpovídající nativní bitovou kopii, vrátí se modul runtime ke standardní JIT kompilaci. Jediným způsobem, jak ladit nativní bitové kopie, je vytvořit nativní bitovou kopii s `/Debug` možností.
...
<xref:System.Runtime.CompilerServices.DependencyAttribute>Atributy a umožňují, aby se <xref:System.Runtime.CompilerServices.DefaultDependencyAttribute> Ngen.exey pomocné vazby.
...
Výchozí pokyny pro vazby jsou zapotřebí pouze pro sestavení, která budou použita okamžitě a často aplikací, která na nich závisí. Použijte <xref:System.Runtime.CompilerServices.DefaultDependencyAttribute> s u <xref:System.Runtime.CompilerServices.LoadHint.Always?displayProperty=nameWithType> těchto sestavení k určení, že se má použít pevná vazba.
...
> [!NOTE]
> Při odinstalaci `/debug` scénářů nedojde k odinstalování scénáře, který zahrnuje i `/profile``/debug.`
...
|Rozhraní .NET Framework NGEN v 4.0.30319 64|No|Yes|
...
Můžete také ručně spustit úlohu nativní bitové kopie prostřednictvím uživatelského rozhraní Plánovač úloh nebo ručním voláním NGen.exe. Pokud je úloha spuštěná kteroukoli z těchto metod, bude pokračovat v provozu, i když počítač přestane být nečinný. Image vytvořené ručně pomocí NGen.exe jsou upřednostňovány, aby umožňovaly předvídatelné chování pro instalační programy aplikací.
...
Služba také spolupracuje s příkazem ruční Ngen.exe. Ruční příkazy mají přednost před aktivitou na pozadí.
...
Než začnete s instalací nebo upgradem, doporučujeme pozastavit službu. Tím se zajistí, že se služba nespustí, zatímco instalační program kopíruje soubory nebo umísťují sestavení do globální mezipaměti sestavení (GAC). Následující Ngen.exe příkazový řádek pozastaví službu:
...
Chcete-li při instalaci nové aplikace nebo při aktualizaci sdílené součásti odložit generování nativních imagí, použijte `/queue` možnost s `install` `update` akcemi nebo. Následující Ngen.exe příkazový řádek nainstaluje nativní bitovou kopii pro sdílenou součást a provede aktualizaci všech kořenových složek, které mohly být ovlivněné:
...
`update`Akce znovu vygeneruje všechny neověřené nativní bitové kopie, nikoli jenom ty, které používají `MyComponent` .
...
Můžete vynutit, aby se akce zařazené do fronty probíhaly synchronně pomocí `executeQueuedItems` akce. Pokud zadáte volitelnou prioritu, tato akce ovlivní pouze akce zařazené do fronty, které mají stejnou nebo nižší prioritu. Výchozí priorita je 3, takže následující Ngen.exe příkaz zpracuje všechny akce zařazené do fronty okamžitě a nevrátí, dokud nebudou dokončeny:

```console
ngen executeQueuedItems
```

Synchronní příkazy jsou spouštěny Ngen.exe a nepoužívají službu nativních bitových kopií. Můžete provádět akce pomocí Ngen.exe, když je spuštěná nativní Image Service.

### <a name="service-shutdown"></a>Vypnutí služby

Po zahájení spuštění příkazu Ngen.exe, který obsahuje `/queue` možnost, bude služba spuštěna na pozadí, dokud nebudou všechny akce dokončeny. Služba ukládá svůj stav, takže v případě potřeby může pokračovat v několika restartováních. Pokud služba zjistí, že žádné další akce nejsou zařazeny do fronty, obnoví její stav tak, aby se při příštím spuštění počítače nerestartoval a pak se sám ukončí.

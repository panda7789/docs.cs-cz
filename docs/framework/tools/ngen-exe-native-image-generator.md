...
<span data-ttu-id="2beea-108">Změny nástroje Ngen. exe v .NET Framework 4:</span><span class="sxs-lookup"><span data-stu-id="2beea-108">Changes to Ngen.exe in the .NET Framework 4:</span></span>
...
<span data-ttu-id="2beea-123">V příkazovém řádku zadejte následující:</span><span class="sxs-lookup"><span data-stu-id="2beea-123">At the command prompt, type the following:</span></span>
...
|<span data-ttu-id="2beea-147">`executeQueuedItems` [<code>1&#124;2&#124;3</code>]</span><span class="sxs-lookup"><span data-stu-id="2beea-147">`executeQueuedItems` [<code>1&#124;2&#124;3</code>]</span></span><br /><br /> <span data-ttu-id="2beea-148">-nebo-</span><span class="sxs-lookup"><span data-stu-id="2beea-148">-or-</span></span><br /><br /> <span data-ttu-id="2beea-149">`eqi` [1&#124;2&#124;3]</span><span class="sxs-lookup"><span data-stu-id="2beea-149">`eqi` [1&#124;2&#124;3]</span></span>|<span data-ttu-id="2beea-150">Spustí úlohy kompilace ve frontě.</span><span class="sxs-lookup"><span data-stu-id="2beea-150">Execute queued compilation jobs.</span></span><br /><br /> <span data-ttu-id="2beea-151">Je-li zadána priorita, jsou spuštěny úlohy kompilace s větší nebo shodnou prioritou.</span><span class="sxs-lookup"><span data-stu-id="2beea-151">If a priority is specified, compilation jobs with greater or equal priority are executed.</span></span> <span data-ttu-id="2beea-152">Není-li priorita zadána, jsou spuštěny všechny úlohy kompilace.</span><span class="sxs-lookup"><span data-stu-id="2beea-152">If no priority is specified, all queued compilation jobs are executed.</span></span>|
...
## <a name="scenarios"></a><span data-ttu-id="2beea-173">Scénáře</span><span class="sxs-lookup"><span data-stu-id="2beea-173">Scenarios</span></span>
...
- [<span data-ttu-id="2beea-230">Pevná vazba</span><span class="sxs-lookup"><span data-stu-id="2beea-230">Hard binding</span></span>](#HardBinding)
...
<span data-ttu-id="2beea-246">Akce `uninstall` také rozpoznává scénáře, takže můžete odinstalovat všechny scénáře nebo pouze vybrané scénáře.</span><span class="sxs-lookup"><span data-stu-id="2beea-246">The `uninstall` action also recognize scenarios, so you can uninstall all scenarios or only selected scenarios.</span></span>
...
- <span data-ttu-id="2beea-266">Nativní bitové kopie vyžadují menší počáteční pracovní sadu, protože není zapotřebí použít kompilátor JIT.</span><span class="sxs-lookup"><span data-stu-id="2beea-266">Native images require a smaller initial working set because there is no need for the JIT compiler.</span></span>
...
- <span data-ttu-id="2beea-279">Bude-li aplikace spuštěna v rámci Terminálového serveru, nativní bitové kopie umožní sdílení znakových stránek.</span><span class="sxs-lookup"><span data-stu-id="2beea-279">If your application will be run under Terminal Server, native images allow sharing of code pages.</span></span>
...
<span data-ttu-id="2beea-313">Společnost Microsoft používá <xref:System.Runtime.CompilerServices.DefaultDependencyAttribute> k určení, že pevná vazba je výchozím nastavením pro velmi malý počet sestavení v .NET Framework, jako je například mscorlib. dll.</span><span class="sxs-lookup"><span data-stu-id="2beea-313">Microsoft uses the <xref:System.Runtime.CompilerServices.DefaultDependencyAttribute> to specify that hard binding is the default for a very small number of assemblies in the .NET Framework, such as mscorlib.dll.</span></span>
...
### <a name="invalid-images"></a><span data-ttu-id="2beea-324">Neplatné obrázky</span><span class="sxs-lookup"><span data-stu-id="2beea-324">Invalid images</span></span>
...
- <span data-ttu-id="2beea-340">Verze operačního systému, pokud je změna z řady Windows 9x na řadu Windows NT.</span><span class="sxs-lookup"><span data-stu-id="2beea-340">The version of the operating system, if the change is from the Windows 9x family to the Windows NT family.</span></span>
...
<span data-ttu-id="2beea-408">Následující příkaz odinstaluje všechny scénáře pro kořenový `ClientApp.exe`:</span><span class="sxs-lookup"><span data-stu-id="2beea-408">The following command uninstalls all scenarios for the root `ClientApp.exe`:</span></span>
...
<span data-ttu-id="2beea-417">Úloha nativní bitové kopie je úloha systému Windows, která generuje a udržuje nativní bitové kopie.</span><span class="sxs-lookup"><span data-stu-id="2beea-417">The native image task is a Windows task that generates and maintains native images.</span></span> <span data-ttu-id="2beea-418">Úloha nativní bitové kopie generuje a obnoví nativní bitové kopie automaticky pro podporované scénáře.</span><span class="sxs-lookup"><span data-stu-id="2beea-418">The native image task generates and reclaims native images automatically for supported scenarios.</span></span> <span data-ttu-id="2beea-419">Umožňuje také instalačním nástrojům použít [Ngen. exe (generátor nativních bitových kopií)](ngen-exe-native-image-generator.md) k vytváření a aktualizaci nativních imagí v odloženém čase.</span><span class="sxs-lookup"><span data-stu-id="2beea-419">It also enables installers to use [Ngen.exe (Native Image Generator)](ngen-exe-native-image-generator.md) to create and update native images at a deferred time.</span></span>

<span data-ttu-id="2beea-420">Úloha nativní bitové kopie je zaregistrovaná jednou pro každou architekturu procesoru podporovanou v počítači, aby se povolila kompilace pro aplikace cílené na jednotlivé architektury:</span><span class="sxs-lookup"><span data-stu-id="2beea-420">The native image task is registered once for each CPU architecture supported on a computer, to allow compilation for applications that target each architecture:</span></span>

|<span data-ttu-id="2beea-421">Název úlohy</span><span class="sxs-lookup"><span data-stu-id="2beea-421">Task name</span></span>|<span data-ttu-id="2beea-422">32 – bitový počítač</span><span class="sxs-lookup"><span data-stu-id="2beea-422">32-bit computer</span></span>|<span data-ttu-id="2beea-423">64 – bitový počítač</span><span class="sxs-lookup"><span data-stu-id="2beea-423">64-bit computer</span></span>|
|---------------|----------------------|----------------------|
|<span data-ttu-id="2beea-424">Rozhraní .NET Framework NGEN v 4.0.30319</span><span class="sxs-lookup"><span data-stu-id="2beea-424">NET Framework NGEN v4.0.30319</span></span>|<span data-ttu-id="2beea-425">Ano</span><span class="sxs-lookup"><span data-stu-id="2beea-425">Yes</span></span>|<span data-ttu-id="2beea-426">Ano</span><span class="sxs-lookup"><span data-stu-id="2beea-426">Yes</span></span>|
|<span data-ttu-id="2beea-427">Rozhraní .NET Framework NGEN v 4.0.30319 64</span><span class="sxs-lookup"><span data-stu-id="2beea-427">NET Framework NGEN v4.0.30319 64</span></span>|<span data-ttu-id="2beea-428">Ne</span><span class="sxs-lookup"><span data-stu-id="2beea-428">No</span></span>|<span data-ttu-id="2beea-429">Ano</span><span class="sxs-lookup"><span data-stu-id="2beea-429">Yes</span></span>|

<span data-ttu-id="2beea-430">Úloha nativní bitové kopie je k dispozici v .NET Framework 4,5 a novějších verzích při použití v systému Windows 8 nebo novějším.</span><span class="sxs-lookup"><span data-stu-id="2beea-430">The native image task is available in the .NET Framework 4.5 and later versions, when running on Windows 8 or later.</span></span> <span data-ttu-id="2beea-431">V dřívějších verzích Windows používá .NET Framework [službu nativních bitových kopií](#native-image-service).</span><span class="sxs-lookup"><span data-stu-id="2beea-431">On earlier versions of Windows, the .NET Framework uses the [Native Image Service](#native-image-service).</span></span>

### <a name="task-lifetime"></a><span data-ttu-id="2beea-432">Doba života úlohy</span><span class="sxs-lookup"><span data-stu-id="2beea-432">Task Lifetime</span></span>

<span data-ttu-id="2beea-433">Obecně platí, že Windows Plánovač úloh spustí úlohu nativní bitové kopie každou noc, když je počítač nečinný.</span><span class="sxs-lookup"><span data-stu-id="2beea-433">In general, the Windows Task Scheduler starts the native image task every night when the computer is idle.</span></span> <span data-ttu-id="2beea-434">Úkol vyhledá všechny odložené práce, které jsou zařazené do fronty instalačními programy aplikací, všechny odložené požadavky na aktualizaci nativních imagí a jakékoli automatické vytváření imagí.</span><span class="sxs-lookup"><span data-stu-id="2beea-434">The task checks for any deferred work that is queued by application installers, any deferred native image update requests, and any automatic image creation.</span></span> <span data-ttu-id="2beea-435">Úkol dokončí nedokončené pracovní položky a pak se ukončí.</span><span class="sxs-lookup"><span data-stu-id="2beea-435">The task completes outstanding work items and then shuts down.</span></span> <span data-ttu-id="2beea-436">Pokud se počítač zastaví v nečinnosti, zatímco je úloha spuštěná, úloha se zastaví.</span><span class="sxs-lookup"><span data-stu-id="2beea-436">If the computer stops being idle while the task is running, the task stops.</span></span>

<span data-ttu-id="2beea-437">Můžete také spustit úlohu nativní bitové kopie ručně prostřednictvím uživatelského rozhraní Plánovač úloh nebo ručním voláním nástroje NGen. exe.</span><span class="sxs-lookup"><span data-stu-id="2beea-437">You can also start the native image task manually through the Task Scheduler UI or through manual calls to NGen.exe.</span></span> <span data-ttu-id="2beea-438">Pokud je úloha spuštěná kteroukoli z těchto metod, bude pokračovat v provozu, i když počítač přestane být nečinný.</span><span class="sxs-lookup"><span data-stu-id="2beea-438">If the task is started through either of these methods, it will continue running when the computer is no longer idle.</span></span> <span data-ttu-id="2beea-439">Image vytvořené ručně pomocí nástroje NGen. exe jsou upřednostňovány, aby umožňovaly předvídatelné chování pro instalační programy aplikací.</span><span class="sxs-lookup"><span data-stu-id="2beea-439">Images created manually by using NGen.exe are prioritized to enable predictable behavior for application installers.</span></span>

## <a name="native-image-service"></a><span data-ttu-id="2beea-440">Nativní bitová kopie služby</span><span class="sxs-lookup"><span data-stu-id="2beea-440">Native Image Service</span></span>

<span data-ttu-id="2beea-441">Nativní Image Service je služba systému Windows, která generuje a udržuje nativní bitové kopie.</span><span class="sxs-lookup"><span data-stu-id="2beea-441">The native image service is a Windows service that generates and maintains native images.</span></span> <span data-ttu-id="2beea-442">Nativní Image Service umožňuje vývojářům odložit instalaci a aktualizaci nativních imagí na období, kdy je počítač nečinný.</span><span class="sxs-lookup"><span data-stu-id="2beea-442">The native image service allows the developer to defer the installation and update of native images to periods when the computer is idle.</span></span>

<span data-ttu-id="2beea-443">Za normálních okolností je nativní Image Service inicializovaná instalačním programem (instalační program) pro aplikaci nebo aktualizaci.</span><span class="sxs-lookup"><span data-stu-id="2beea-443">Normally, the native image service is initiated by the installation program (installer) for an application or update.</span></span> <span data-ttu-id="2beea-444">U akcí s prioritou 3 se služba spouští během nečinnosti počítače.</span><span class="sxs-lookup"><span data-stu-id="2beea-444">For priority 3 actions, the service executes during idle time on the computer.</span></span> <span data-ttu-id="2beea-445">Služba ukládá svůj stav a v případě potřeby může pokračovat v několika restartováních.</span><span class="sxs-lookup"><span data-stu-id="2beea-445">The service saves its state and is capable of continuing through multiple reboots if necessary.</span></span> <span data-ttu-id="2beea-446">Více kompilací obrázků lze zařadit do fronty.</span><span class="sxs-lookup"><span data-stu-id="2beea-446">Multiple image compilations can be queued.</span></span>

<span data-ttu-id="2beea-447">Služba také komunikuje s ručním příkazem Ngen. exe.</span><span class="sxs-lookup"><span data-stu-id="2beea-447">The service also interacts with the manual Ngen.exe command.</span></span> <span data-ttu-id="2beea-448">Ruční příkazy mají přednost před aktivitou na pozadí.</span><span class="sxs-lookup"><span data-stu-id="2beea-448">Manual commands take precedence over background activity.</span></span>

> [!NOTE]
> <span data-ttu-id="2beea-449">V systému Windows Vista je název zobrazený pro nativní Image Service "Microsoft.NET Framework NGEN v 2.0.50727 _X86" nebo "Microsoft.NET Framework NGEN v 2.0.50727 _X64".</span><span class="sxs-lookup"><span data-stu-id="2beea-449">On Windows Vista, the name displayed for the native image service is "Microsoft.NET Framework NGEN v2.0.50727_X86" or "Microsoft.NET Framework NGEN v2.0.50727_X64".</span></span> <span data-ttu-id="2beea-450">Ve všech starších verzích Microsoft Windows se jedná o název ".NET runtime Optimization Service v 2.0.50727 _X86" nebo ".NET runtime Optimization Service v 2.0.50727 _X64".</span><span class="sxs-lookup"><span data-stu-id="2beea-450">On all earlier versions of Microsoft Windows, the name is ".NET Runtime Optimization Service v2.0.50727_X86" or ".NET Runtime Optimization Service v2.0.50727_X64".</span></span>

### <a name="launching-deferred-operations"></a><span data-ttu-id="2beea-451">Spouštění odložených operací</span><span class="sxs-lookup"><span data-stu-id="2beea-451">Launching Deferred Operations</span></span>

<span data-ttu-id="2beea-452">Než začnete s instalací nebo upgradem, doporučujeme pozastavit službu.</span><span class="sxs-lookup"><span data-stu-id="2beea-452">Before beginning an installation or upgrade, pausing the service is recommended.</span></span> <span data-ttu-id="2beea-453">Tím se zajistí, že se služba nespustí, zatímco instalační program kopíruje soubory nebo umísťují sestavení do globální mezipaměti sestavení (GAC).</span><span class="sxs-lookup"><span data-stu-id="2beea-453">This ensures that the service does not execute while the installer is copying files or putting assemblies in the global assembly cache.</span></span> <span data-ttu-id="2beea-454">Následující příkazový řádek nástroje Ngen. exe pozastaví službu:</span><span class="sxs-lookup"><span data-stu-id="2beea-454">The following Ngen.exe command line pauses the service:</span></span>

```console
ngen queue pause
```

<span data-ttu-id="2beea-455">Když jsou všechny odložené operace zařazené do fronty, následující příkaz umožní službě obnovit:</span><span class="sxs-lookup"><span data-stu-id="2beea-455">When all deferred operations have been queued, the following command allows the service to resume:</span></span>

```console
ngen queue continue
```

<span data-ttu-id="2beea-456">Chcete-li při instalaci nové aplikace nebo při aktualizaci sdílené součásti odložit generování nativních imagí, použijte možnost `/queue` s akcemi `install` nebo `update`.</span><span class="sxs-lookup"><span data-stu-id="2beea-456">To defer native image generation when installing a new application or when updating a shared component, use the `/queue` option with the `install` or `update` actions.</span></span> <span data-ttu-id="2beea-457">Následující příkazové řádky nástroje Ngen. exe nainstalují nativní bitovou kopii pro sdílenou součást a provedou aktualizaci všech kořenových složek, které mohly být ovlivněny:</span><span class="sxs-lookup"><span data-stu-id="2beea-457">The following Ngen.exe command lines install a native image for a shared component and perform an update of all roots that may have been affected:</span></span>
...
<span data-ttu-id="2beea-458">Akce `update` znovu vygeneruje všechny nativní bitové kopie, jejichž platnost byla zrušena, nejen ty, které používají `MyComponent`.</span><span class="sxs-lookup"><span data-stu-id="2beea-458">The `update` action regenerates all native images that have been invalidated, not just those that use `MyComponent`.</span></span>

<span data-ttu-id="2beea-459">Pokud se vaše aplikace skládá z mnoha kořenů, můžete řídit prioritu odložených akcí.</span><span class="sxs-lookup"><span data-stu-id="2beea-459">If your application consists of many roots, you can control the priority of the deferred actions.</span></span> <span data-ttu-id="2beea-460">Následující příkazy zařadí do fronty instalaci tří kořenů.</span><span class="sxs-lookup"><span data-stu-id="2beea-460">The following commands queue the installation of three roots.</span></span> <span data-ttu-id="2beea-461">Nejprve je nainstalovaná `Assembly1`, ale nečeká na dobu nečinnosti.</span><span class="sxs-lookup"><span data-stu-id="2beea-461">`Assembly1` is installed first, without waiting for idle time.</span></span> <span data-ttu-id="2beea-462">`Assembly2` je nainstalována i bez čekání na dobu nečinnosti, ale po dokončení všech akcí s prioritou 1.</span><span class="sxs-lookup"><span data-stu-id="2beea-462">`Assembly2` is also installed without waiting for idle time, but after all priority 1 actions have completed.</span></span> <span data-ttu-id="2beea-463">`Assembly3` se nainstaluje, když služba zjistí, že je počítač nečinný.</span><span class="sxs-lookup"><span data-stu-id="2beea-463">`Assembly3` is installed when the service detects that the computer is idle.</span></span>
...
<span data-ttu-id="2beea-464">Můžete vynutit, aby se akce zařazené do fronty probíhaly synchronně pomocí akce `executeQueuedItems`.</span><span class="sxs-lookup"><span data-stu-id="2beea-464">You can force queued actions to occur synchronously by using the `executeQueuedItems` action.</span></span> <span data-ttu-id="2beea-465">Pokud zadáte volitelnou prioritu, tato akce ovlivní pouze akce zařazené do fronty, které mají stejnou nebo nižší prioritu.</span><span class="sxs-lookup"><span data-stu-id="2beea-465">If you supply the optional priority, this action affects only the queued actions that have equal or lower priority.</span></span> <span data-ttu-id="2beea-466">Výchozí priorita je 3, takže následující příkaz Ngen. exe zpracuje všechny akce zařazené do fronty hned a nevrátí, dokud nebudou dokončeny:</span><span class="sxs-lookup"><span data-stu-id="2beea-466">The default priority is 3, so the following Ngen.exe command processes all queued actions immediately, and does not return until they are finished:</span></span>

```console
ngen executeQueuedItems
```

<span data-ttu-id="2beea-467">Synchronní příkazy jsou spouštěny pomocí nástroje Ngen. exe a nepoužívají službu nativních bitových kopií.</span><span class="sxs-lookup"><span data-stu-id="2beea-467">Synchronous commands are executed by Ngen.exe and do not use the native image service.</span></span> <span data-ttu-id="2beea-468">Můžete provést akce pomocí nástroje Ngen. exe, když je spuštěná nativní Image Service.</span><span class="sxs-lookup"><span data-stu-id="2beea-468">You can execute actions using Ngen.exe while the native image service is running.</span></span>

### <a name="service-shutdown"></a><span data-ttu-id="2beea-469">Vypnutí služby</span><span class="sxs-lookup"><span data-stu-id="2beea-469">Service Shutdown</span></span>

<span data-ttu-id="2beea-470">Po spuštění příkazu Ngen. exe, který obsahuje možnost `/queue`, se služba spustí na pozadí, dokud nebudou všechny akce dokončeny.</span><span class="sxs-lookup"><span data-stu-id="2beea-470">After being initiated by the execution of an Ngen.exe command that includes the `/queue` option, the service runs in the background until all actions have been completed.</span></span> <span data-ttu-id="2beea-471">Služba ukládá svůj stav, takže v případě potřeby může pokračovat v několika restartováních.</span><span class="sxs-lookup"><span data-stu-id="2beea-471">The service saves its state so that it can continue through multiple reboots if necessary.</span></span> <span data-ttu-id="2beea-472">Pokud služba zjistí, že žádné další akce nejsou zařazeny do fronty, obnoví její stav tak, aby se při příštím spuštění počítače nerestartoval a pak se sám ukončí.</span><span class="sxs-lookup"><span data-stu-id="2beea-472">When the service detects that there are no more actions queued, it resets its status so that it will not restart the next time the computer is booted, and then it shuts itself down.</span></span>

### <a name="service-interaction-with-clients"></a><span data-ttu-id="2beea-473">Interakce služby s klienty</span><span class="sxs-lookup"><span data-stu-id="2beea-473">Service Interaction with Clients</span></span>

<span data-ttu-id="2beea-474">V .NET Framework verze 2,0 je jediná interakce s nativní imagí pomocí nástroje příkazového řádku Ngen. exe.</span><span class="sxs-lookup"><span data-stu-id="2beea-474">In the .NET Framework version 2.0, the only interaction with the native image service is through the command-line tool Ngen.exe.</span></span> <span data-ttu-id="2beea-475">Použijte nástroj příkazového řádku v instalačních skriptech k zařazení akcí do fronty pro službu nativních bitových kopií a pro interakci se službou.</span><span class="sxs-lookup"><span data-stu-id="2beea-475">Use the command-line tool in installation scripts to queue actions for the native image service and to interact with the service.</span></span>

## <a name="see-also"></a><span data-ttu-id="2beea-476">Viz také:</span><span class="sxs-lookup"><span data-stu-id="2beea-476">See also</span></span>

- [<span data-ttu-id="2beea-477">Nástroje</span><span class="sxs-lookup"><span data-stu-id="2beea-477">Tools</span></span>](index.md)
- [<span data-ttu-id="2beea-478">Proces spravovaného spuštění</span><span class="sxs-lookup"><span data-stu-id="2beea-478">Managed Execution Process</span></span>](../../standard/managed-execution-process.md)
- [<span data-ttu-id="2beea-479">Jak běhové prostředí vyhledává sestavení</span><span class="sxs-lookup"><span data-stu-id="2beea-479">How the Runtime Locates Assemblies</span></span>](../deployment/how-the-runtime-locates-assemblies.md)
- [<span data-ttu-id="2beea-480">Příkazové řádky</span><span class="sxs-lookup"><span data-stu-id="2beea-480">Command Prompts</span></span>](developer-command-prompt-for-vs.md)

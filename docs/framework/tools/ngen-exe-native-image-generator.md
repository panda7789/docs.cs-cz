...
<span data-ttu-id="b65c5-103">Generátor nativních bitových kopií (Ngen.exe) je nástroj zvyšující výkon spravovaných aplikací.</span><span class="sxs-lookup"><span data-stu-id="b65c5-103">The Native Image Generator (Ngen.exe) is a tool that improves the performance of managed applications.</span></span> <span data-ttu-id="b65c5-104">Nástroj Ngen.exe vytváří nativní bitové kopie, což jsou soubory obsahující zkompilovaný strojový kód specifický pro procesor, a instaluje je do mezipaměti nativních bitových kopií v místním počítači.</span><span class="sxs-lookup"><span data-stu-id="b65c5-104">Ngen.exe creates native images, which are files containing compiled processor-specific machine code, and installs them into the native image cache on the local computer.</span></span> <span data-ttu-id="b65c5-105">Modul runtime může ke kompilaci původního sestavení použít nativní bitové kopie z mezipaměti namísto kompilátoru JIT (just-in-time).</span><span class="sxs-lookup"><span data-stu-id="b65c5-105">The runtime can use native images from the cache instead of using the just-in-time (JIT) compiler to compile the original assembly.</span></span>  
 ...
 <span data-ttu-id="b65c5-106">Změny Ngen.exe v [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)]:</span><span class="sxs-lookup"><span data-stu-id="b65c5-106">Changes to Ngen.exe in the [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)]:</span></span>  
...
## <a name="priority-levels"></a><span data-ttu-id="b65c5-164">Úrovně priority</span><span class="sxs-lookup"><span data-stu-id="b65c5-164">Priority Levels</span></span>  
...
>  <span data-ttu-id="b65c5-220">Doménově neutrální kód nemůže být uvolněn a výkon může být mírně snížen, zvláště při přistupování ke statickým členům.</span><span class="sxs-lookup"><span data-stu-id="b65c5-220">Domain-neutral code cannot be unloaded, and performance may be slightly slower, particularly when accessing static members.</span></span>  
 ...
 <span data-ttu-id="b65c5-221">V této části Poznámky:</span><span class="sxs-lookup"><span data-stu-id="b65c5-221">In this Remarks section:</span></span>  
...
-   <span data-ttu-id="b65c5-273">Je-li aplikace spouštěna v prostředí, které využívá mnoho sdílených komponent, nativní bitové kopie umožňují sdílení komponent mezi mnoha procesy.</span><span class="sxs-lookup"><span data-stu-id="b65c5-273">If your application runs in an environment that uses many shared components, native images allow the components to be shared by multiple processes.</span></span>  
...
-   <span data-ttu-id="b65c5-338">Verze operačního systému, pokud se změna týká ze systémů řady Windows 9 x řady systému Windows NT.</span><span class="sxs-lookup"><span data-stu-id="b65c5-338">The version of the operating system, if the change is from the Windows 9x family to the Windows NT family.</span></span>  
...
```  
 ...
 <span data-ttu-id="b65c5-400">Aktualizace všech bitových kopií může být časově náročný proces.</span><span class="sxs-lookup"><span data-stu-id="b65c5-400">Updating all images can be a lengthy process.</span></span> <span data-ttu-id="b65c5-401">Aktualizace pro spuštění pomocí služby nativních bitových kopií můžete fronty pomocí `/queue` možnost.</span><span class="sxs-lookup"><span data-stu-id="b65c5-401">You can queue the updates for execution by the native image service by using the `/queue` option.</span></span> <span data-ttu-id="b65c5-402">Další informace o `/queue` možnost a instalace priority, najdete v části [nativní bitové kopie služby](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).</span><span class="sxs-lookup"><span data-stu-id="b65c5-402">For more information on the `/queue` option and installation priorities, see [Native Image Service](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).</span></span>  
  
```  
ngen update /queue  
```  
  
### <a name="uninstalling-images"></a><span data-ttu-id="b65c5-403">Odinstalování bitových kopií</span><span class="sxs-lookup"><span data-stu-id="b65c5-403">Uninstalling Images</span></span>  
 <span data-ttu-id="b65c5-404">Nástroj Ngen.exe udržuje seznam závislostí, takže sdílené komponenty jsou odstraněny pouze v případě, že byla odebrána všechna sestavení, která na nich závisí.</span><span class="sxs-lookup"><span data-stu-id="b65c5-404">Ngen.exe maintains a list of dependencies, so that shared components are removed only when all assemblies that depend on them have been removed.</span></span> <span data-ttu-id="b65c5-405">Kromě toho, pokud byla sdílená komponenta nainstalována jako kořen, nebude odstraněna.</span><span class="sxs-lookup"><span data-stu-id="b65c5-405">In addition, a shared component is not removed if it has been installed as a root.</span></span>  
  
 <span data-ttu-id="b65c5-406">Příkaz odinstaluje všechny scénáře pro kořenovou `ClientApp.exe`:</span><span class="sxs-lookup"><span data-stu-id="b65c5-406">The following command uninstalls all scenarios for the root `ClientApp.exe`:</span></span>  
  
```  
ngen uninstall ClientApp  
```  
  
 <span data-ttu-id="b65c5-407">`uninstall` Akci lze použít k odebrání konkrétních scénářů.</span><span class="sxs-lookup"><span data-stu-id="b65c5-407">The `uninstall` action can be used to remove specific scenarios.</span></span> <span data-ttu-id="b65c5-408">Příkaz odinstaluje všechny scénáře ladění pro `ClientApp.exe`:</span><span class="sxs-lookup"><span data-stu-id="b65c5-408">The following command uninstalls all debug scenarios for `ClientApp.exe`:</span></span>  
...
> [!NOTE]
>  <span data-ttu-id="b65c5-409">Odinstalace `/debug` scénáře neodinstaluje scénáře, který zahrnuje obě `/profile` a`/debug.`</span><span class="sxs-lookup"><span data-stu-id="b65c5-409">Uninstalling `/debug` scenarios does not uninstall a scenario that includes both `/profile` and `/debug.`</span></span>  
  
 <span data-ttu-id="b65c5-410">Příkaz odinstaluje všechny scénáře pro určitou verzi `ClientApp.exe`:</span><span class="sxs-lookup"><span data-stu-id="b65c5-410">The following command uninstalls all scenarios for a specific version of `ClientApp.exe`:</span></span>  
  
```  
ngen uninstall "ClientApp, Version=1.0.0.0"  
```  
  
 <span data-ttu-id="b65c5-411">Následující příkazy odinstalovat všechny scénáře pro `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` nebo právě tento scénář ladění pro toto sestavení:</span><span class="sxs-lookup"><span data-stu-id="b65c5-411">The following commands uninstall all scenarios for `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` or just the debug scenario for that assembly:</span></span>  
...
```  
 ...
 <span data-ttu-id="b65c5-412">Stejně jako u `install` akce, poskytuje rozšíření vyžaduje buď provádění Ngen.exe z adresáře, který obsahuje sestavení nebo zadat úplnou cestu.</span><span class="sxs-lookup"><span data-stu-id="b65c5-412">As with the `install` action, supplying an extension requires either executing Ngen.exe from the directory containing the assembly or specifying a full path.</span></span>  
  
 <span data-ttu-id="b65c5-413">Příklady týkající se služby nativních bitových kopií naleznete v tématu [nativní bitové kopie služby](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).</span><span class="sxs-lookup"><span data-stu-id="b65c5-413">For examples relating to the native image service, see [Native Image Service](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).</span></span>  
  
## <a name="native-image-task"></a><span data-ttu-id="b65c5-414">Úloha pro nativní bitové kopie</span><span class="sxs-lookup"><span data-stu-id="b65c5-414">Native Image Task</span></span>  
 <span data-ttu-id="b65c5-415">Úloha nativních bitových kopií je úloh systému Windows, která vytváří a udržuje nativní bitové kopie.</span><span class="sxs-lookup"><span data-stu-id="b65c5-415">The native image task is a Windows task that generates and maintains native images.</span></span> <span data-ttu-id="b65c5-416">Úloha nativních bitových kopií generuje a získá nativní bitové kopie automaticky pro podporované scénáře.</span><span class="sxs-lookup"><span data-stu-id="b65c5-416">The native image task generates and reclaims native images automatically for supported scenarios.</span></span> <span data-ttu-id="b65c5-417">(Viz [vytváření nativní bitové kopie](http://msdn.microsoft.com/en-us/2bc8b678-dd8d-4742-ad82-319e9bf52418).) Umožňuje také instalační programy používat [Ngen.exe (Generátor nativních obrázků)](../../../docs/framework/tools/ngen-exe-native-image-generator.md) k vytváření a aktualizaci nativní bitové kopie odložené najednou.</span><span class="sxs-lookup"><span data-stu-id="b65c5-417">(See [Creating Native Images](http://msdn.microsoft.com/en-us/2bc8b678-dd8d-4742-ad82-319e9bf52418).) It also enables installers to use [Ngen.exe (Native Image Generator)](../../../docs/framework/tools/ngen-exe-native-image-generator.md) to create and update native images at a deferred time.</span></span>  
  
 <span data-ttu-id="b65c5-418">Jakmile pro každý procesor architektura podporovaný na počítači, aby kompilace pro aplikace, že cíl každou architekturu registraci nativních bitových kopií úlohy:</span><span class="sxs-lookup"><span data-stu-id="b65c5-418">The native image task is registered once for each CPU architecture supported on a computer, to allow compilation for applications that target each architecture:</span></span>  
  
|<span data-ttu-id="b65c5-419">Název úlohy</span><span class="sxs-lookup"><span data-stu-id="b65c5-419">Task name</span></span>|<span data-ttu-id="b65c5-420">32bitový počítač</span><span class="sxs-lookup"><span data-stu-id="b65c5-420">32-bit computer</span></span>|<span data-ttu-id="b65c5-421">64bitový počítač</span><span class="sxs-lookup"><span data-stu-id="b65c5-421">64-bit computer</span></span>|  
|---------------|----------------------|----------------------|  
|<span data-ttu-id="b65c5-422">NET Framework NGEN v4.0.30319</span><span class="sxs-lookup"><span data-stu-id="b65c5-422">NET Framework NGEN v4.0.30319</span></span>|<span data-ttu-id="b65c5-423">Ano</span><span class="sxs-lookup"><span data-stu-id="b65c5-423">Yes</span></span>|<span data-ttu-id="b65c5-424">Ano</span><span class="sxs-lookup"><span data-stu-id="b65c5-424">Yes</span></span>|  
|<span data-ttu-id="b65c5-425">NET Framework NGEN v4.0.30319 64</span><span class="sxs-lookup"><span data-stu-id="b65c5-425">NET Framework NGEN v4.0.30319 64</span></span>|<span data-ttu-id="b65c5-426">Ne</span><span class="sxs-lookup"><span data-stu-id="b65c5-426">No</span></span>|<span data-ttu-id="b65c5-427">Ano</span><span class="sxs-lookup"><span data-stu-id="b65c5-427">Yes</span></span>|  
  
 <span data-ttu-id="b65c5-428">Tato úloha nativních bitových kopií je je k dispozici v rozhraní .NET Framework 4.5 a novějších verzích, při spuštění v systému Windows 8 nebo novější.</span><span class="sxs-lookup"><span data-stu-id="b65c5-428">The native image task is is available in the .NET Framework 4.5 and later versions, when running on Windows 8 or later.</span></span> <span data-ttu-id="b65c5-429">V dřívějších verzích systému Windows, používá rozhraní .NET Framework [nativní bitové kopie služby](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).</span><span class="sxs-lookup"><span data-stu-id="b65c5-429">On earlier versions of Windows, the .NET Framework uses the [Native Image Service](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).</span></span>  
  
### <a name="task-lifetime"></a><span data-ttu-id="b65c5-430">Doba platnosti úloh</span><span class="sxs-lookup"><span data-stu-id="b65c5-430">Task Lifetime</span></span>  
 <span data-ttu-id="b65c5-431">Obecně platí plánovače úloh systému Windows spustí úlohu nativních bitových kopií každou noc, když je počítač v nečinnosti.</span><span class="sxs-lookup"><span data-stu-id="b65c5-431">In general, the Windows Task Scheduler starts the native image task every night when the computer is idle.</span></span> <span data-ttu-id="b65c5-432">Úloha zkontroluje všechny odložené práci, kterou je zařazena ve frontě instalační programy aplikací, všechny žádosti o aktualizaci odložené nativních bitových kopií a vytvoření všechny automatické bitové kopie.</span><span class="sxs-lookup"><span data-stu-id="b65c5-432">The task checks for any deferred work that is queued by application installers, any deferred native image update requests, and any automatic image creation.</span></span> <span data-ttu-id="b65c5-433">Úloha dokončení zbývajících pracovní položky a pak se ukončí.</span><span class="sxs-lookup"><span data-stu-id="b65c5-433">The task completes outstanding work items and then shuts down.</span></span> <span data-ttu-id="b65c5-434">Pokud se počítač zastaví nečinnosti, když je spuštěn úkol, zastaví úlohu.</span><span class="sxs-lookup"><span data-stu-id="b65c5-434">If the computer stops being idle while the task is running, the task stops.</span></span>  
  
 <span data-ttu-id="b65c5-435">Můžete také spustit úlohu nativních bitových kopií ručně prostřednictvím uživatelského rozhraní plánovače úloh nebo ruční volání NGen.exe.</span><span class="sxs-lookup"><span data-stu-id="b65c5-435">You can also start the native image task manually through the Task Scheduler UI or through manual calls to NGen.exe.</span></span> <span data-ttu-id="b65c5-436">Pokud úloha se spustí pomocí jedné z těchto metod, bude spuštěn, když se počítač již není nečinný.</span><span class="sxs-lookup"><span data-stu-id="b65c5-436">If the task is started through either of these methods, it will continue running when the computer is no longer idle.</span></span> <span data-ttu-id="b65c5-437">Bitové kopie ručně vytvořené pomocí NGen.exe budou mít vyšší prioritu, chcete-li povolit předvídatelné chování pro instalační programy aplikací.</span><span class="sxs-lookup"><span data-stu-id="b65c5-437">Images created manually by using NGen.exe are prioritized to enable predictable behavior for application installers.</span></span>  
  
## <a name="native-image-service"></a><span data-ttu-id="b65c5-438">Nativní bitová kopie služby</span><span class="sxs-lookup"><span data-stu-id="b65c5-438">Native Image Service</span></span>  
 <span data-ttu-id="b65c5-439">Služba nativních bitových kopií je služba systému Windows, která vytváří a udržuje nativní bitové kopie.</span><span class="sxs-lookup"><span data-stu-id="b65c5-439">The native image service is a Windows service that generates and maintains native images.</span></span> <span data-ttu-id="b65c5-440">Službu nativních bitových kopií umožňuje vývojáři odložení instalace a aktualizace nativních bitových kopií do doby, kdy počítač nečinný.</span><span class="sxs-lookup"><span data-stu-id="b65c5-440">The native image service allows the developer to defer the installation and update of native images to periods when the computer is idle.</span></span>  
  
 <span data-ttu-id="b65c5-441">Za normálních okolností službu nativních bitových kopií inicializuje instalačního programu (Instalační program) pro aplikace nebo aktualizace.</span><span class="sxs-lookup"><span data-stu-id="b65c5-441">Normally, the native image service is initiated by the installation program (installer) for an application or update.</span></span> <span data-ttu-id="b65c5-442">Pro akce s prioritou 3 služba provede během doby nečinnosti v počítači.</span><span class="sxs-lookup"><span data-stu-id="b65c5-442">For priority 3 actions, the service executes during idle time on the computer.</span></span> <span data-ttu-id="b65c5-443">Služba uloží stav a se může nadále prostřednictvím více restartování v případě potřeby.</span><span class="sxs-lookup"><span data-stu-id="b65c5-443">The service saves its state and is capable of continuing through multiple reboots if necessary.</span></span> <span data-ttu-id="b65c5-444">Více kompilace bitové kopie můžete zařadit do fronty.</span><span class="sxs-lookup"><span data-stu-id="b65c5-444">Multiple image compilations can be queued.</span></span>  
  
 <span data-ttu-id="b65c5-445">Službu taky komunikuje s ruční Ngen.exe příkaz.</span><span class="sxs-lookup"><span data-stu-id="b65c5-445">The service also interacts with the manual Ngen.exe command.</span></span> <span data-ttu-id="b65c5-446">Ruční příkazy mají přednost před aktivita na pozadí.</span><span class="sxs-lookup"><span data-stu-id="b65c5-446">Manual commands take precedence over background activity.</span></span>  
  
> [!NOTE]
>  <span data-ttu-id="b65c5-447">V systému Windows Vista je název zobrazený pro službu nativních bitových kopií "Microsoft.NET Framework NGEN v2.0.50727_X86" nebo "Microsoft.NET Framework NGEN v2.0.50727_X64".</span><span class="sxs-lookup"><span data-stu-id="b65c5-447">On Windows Vista, the name displayed for the native image service is "Microsoft.NET Framework NGEN v2.0.50727_X86" or "Microsoft.NET Framework NGEN v2.0.50727_X64".</span></span> <span data-ttu-id="b65c5-448">Na všechny starší verze systému Microsoft Windows je název "Služba optimalizace modulu Runtime .NET v2.0.50727_X86" nebo "Služba optimalizace modulu Runtime .NET v2.0.50727_X64".</span><span class="sxs-lookup"><span data-stu-id="b65c5-448">On all earlier versions of Microsoft Windows, the name is ".NET Runtime Optimization Service v2.0.50727_X86" or ".NET Runtime Optimization Service v2.0.50727_X64".</span></span>  
  
### <a name="launching-deferred-operations"></a><span data-ttu-id="b65c5-449">Spuštění operace odložené</span><span class="sxs-lookup"><span data-stu-id="b65c5-449">Launching Deferred Operations</span></span>  
 <span data-ttu-id="b65c5-450">Před zahájením instalace nebo upgradu, se doporučuje pozastavení služby.</span><span class="sxs-lookup"><span data-stu-id="b65c5-450">Before beginning an installation or upgrade, pausing the service is recommended.</span></span> <span data-ttu-id="b65c5-451">Tím se zajistí, že služba nespustí při kopírování souborů nebo uvedení sestavení v globální mezipaměti sestavení Instalační služby.</span><span class="sxs-lookup"><span data-stu-id="b65c5-451">This ensures that the service does not execute while the installer is copying files or putting assemblies in the global assembly cache.</span></span> <span data-ttu-id="b65c5-452">Následující příkazový řádek Ngen.exe pozastaví službu:</span><span class="sxs-lookup"><span data-stu-id="b65c5-452">The following Ngen.exe command line pauses the service:</span></span>  
  
```  
ngen queue pause  
```  
  
 <span data-ttu-id="b65c5-453">Pokud všechny odložené operace byla zařazena do fronty, následující příkaz umožňuje službu, kterou chcete obnovit:</span><span class="sxs-lookup"><span data-stu-id="b65c5-453">When all deferred operations have been queued, the following command allows the service to resume:</span></span>  
  
```  
ngen queue continue  
```  
  
 <span data-ttu-id="b65c5-454">Chcete-li při instalaci nové aplikace, nebo při aktualizaci sdílené součásti, odložení generování nativních bitových kopií, použijte `/queue` možnost s `install` nebo `update` akce.</span><span class="sxs-lookup"><span data-stu-id="b65c5-454">To defer native image generation when installing a new application or when updating a shared component, use the `/queue` option with the `install` or `update` actions.</span></span> <span data-ttu-id="b65c5-455">Na příkazovém řádku následující Ngen.exe instalace nativních bitových kopií pro sdílené součást a proveďte aktualizaci všechny kořenové adresáře, které byly:</span><span class="sxs-lookup"><span data-stu-id="b65c5-455">The following Ngen.exe command lines install a native image for a shared component and perform an update of all roots that may have been affected:</span></span>  
...
```  
 ...
 <span data-ttu-id="b65c5-456">`update` Akce regeneruje všechny nativní bitové kopie, které se zrušila platnost, nikoli pouze ty, které používají `MyComponent`.</span><span class="sxs-lookup"><span data-stu-id="b65c5-456">The `update` action regenerates all native images that have been invalidated, not just those that use `MyComponent`.</span></span>  
  
 <span data-ttu-id="b65c5-457">Pokud vaše aplikace obsahuje mnoho kořeny, můžete řídit prioritu odložené akce.</span><span class="sxs-lookup"><span data-stu-id="b65c5-457">If your application consists of many roots, you can control the priority of the deferred actions.</span></span> <span data-ttu-id="b65c5-458">Následující příkazy ve frontě pro instalaci tři kořenových certifikačních autorit.</span><span class="sxs-lookup"><span data-stu-id="b65c5-458">The following commands queue the installation of three roots.</span></span> <span data-ttu-id="b65c5-459">`Assembly1`je nainstalována jako první, bez čekání na čas nečinnosti.</span><span class="sxs-lookup"><span data-stu-id="b65c5-459">`Assembly1` is installed first, without waiting for idle time.</span></span> <span data-ttu-id="b65c5-460">`Assembly2`je také nainstalován bez čekání na dobu nečinnosti, ale po dokončení všech akcí priority 1.</span><span class="sxs-lookup"><span data-stu-id="b65c5-460">`Assembly2` is also installed without waiting for idle time, but after all priority 1 actions have completed.</span></span> <span data-ttu-id="b65c5-461">`Assembly3`nainstaluje se při službu zjistí, že je počítač v nečinnosti.</span><span class="sxs-lookup"><span data-stu-id="b65c5-461">`Assembly3` is installed when the service detects that the computer is idle.</span></span>  
...
```  
 ...
 <span data-ttu-id="b65c5-462">Můžete vynutit akce ve frontě proběhnout synchronně pomocí `executeQueuedItems` akce.</span><span class="sxs-lookup"><span data-stu-id="b65c5-462">You can force queued actions to occur synchronously by using the `executeQueuedItems` action.</span></span> <span data-ttu-id="b65c5-463">Pokud zadáte volitelné prioritu, tato akce ovlivní pouze zařazených do fronty akce, které mají stejné nebo nižší prioritu.</span><span class="sxs-lookup"><span data-stu-id="b65c5-463">If you supply the optional priority, this action affects only the queued actions that have equal or lower priority.</span></span> <span data-ttu-id="b65c5-464">Výchozí priorita je 3, takže příkaz Ngen.exe okamžitě zpracovává všechny akce ve frontě a nevrací až skončí:</span><span class="sxs-lookup"><span data-stu-id="b65c5-464">The default priority is 3, so the following Ngen.exe command processes all queued actions immediately, and does not return until they are finished:</span></span>  
  
```  
ngen executeQueuedItems  
```  
  
 <span data-ttu-id="b65c5-465">Synchronní příkazy jsou provedený Ngen.exe a nepoužívejte službu nativních bitových kopií.</span><span class="sxs-lookup"><span data-stu-id="b65c5-465">Synchronous commands are executed by Ngen.exe and do not use the native image service.</span></span> <span data-ttu-id="b65c5-466">Můžete provést akce s použitím Ngen.exe během spuštěné služby nativních bitových kopií.</span><span class="sxs-lookup"><span data-stu-id="b65c5-466">You can execute actions using Ngen.exe while the native image service is running.</span></span>  
  
### <a name="service-shutdown"></a><span data-ttu-id="b65c5-467">Vypnutí služby</span><span class="sxs-lookup"><span data-stu-id="b65c5-467">Service Shutdown</span></span>  
 <span data-ttu-id="b65c5-468">Po se iniciovat provádění příkaz Ngen.exe, která zahrnuje `/queue` možnost, služba bude spuštěna na pozadí dokud všechny akce byly dokončeny.</span><span class="sxs-lookup"><span data-stu-id="b65c5-468">After being initiated by the execution of an Ngen.exe command that includes the `/queue` option, the service runs in the background until all actions have been completed.</span></span> <span data-ttu-id="b65c5-469">Služba uloží stav, aby mohl pokračovat prostřednictvím více restartování v případě potřeby.</span><span class="sxs-lookup"><span data-stu-id="b65c5-469">The service saves its state so that it can continue through multiple reboots if necessary.</span></span> <span data-ttu-id="b65c5-470">Když službu zjistí, že neexistují žádné další akce ve frontě, obnoví stav tak, že ji nebude restartování při příštím spuštění počítače a pak se ukončí sám.</span><span class="sxs-lookup"><span data-stu-id="b65c5-470">When the service detects that there are no more actions queued, it resets its status so that it will not restart the next time the computer is booted, and then it shuts itself down.</span></span>  
  
### <a name="service-interaction-with-clients"></a><span data-ttu-id="b65c5-471">Služba interakci s klienty</span><span class="sxs-lookup"><span data-stu-id="b65c5-471">Service Interaction with Clients</span></span>  
 <span data-ttu-id="b65c5-472">V rozhraní .NET Framework verze 2.0 je jediná interakce s nativních bitových kopií služby pomocí nástroje příkazového řádku Ngen.exe.</span><span class="sxs-lookup"><span data-stu-id="b65c5-472">In the .NET Framework version 2.0, the only interaction with the native image service is through the command-line tool Ngen.exe.</span></span> <span data-ttu-id="b65c5-473">Pomocí nástroje příkazového řádku ve skriptech instalace do fronty akce pro službu nativních bitových kopií a komunikovat se službou.</span><span class="sxs-lookup"><span data-stu-id="b65c5-473">Use the command-line tool in installation scripts to queue actions for the native image service and to interact with the service.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="b65c5-474">Viz také</span><span class="sxs-lookup"><span data-stu-id="b65c5-474">See Also</span></span>  
 [<span data-ttu-id="b65c5-475">Služba nativních bitových kopií</span><span class="sxs-lookup"><span data-stu-id="b65c5-475">Native Image Service</span></span>](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309)  
 [<span data-ttu-id="b65c5-476">Úloha nativních bitových kopií</span><span class="sxs-lookup"><span data-stu-id="b65c5-476">Native Image Task</span></span>](http://msdn.microsoft.com/en-us/9b1f7590-4e0d-4737-90ef-eaf696932afb)  
 [<span data-ttu-id="b65c5-477">Nástroje</span><span class="sxs-lookup"><span data-stu-id="b65c5-477">Tools</span></span>](../../../docs/framework/tools/index.md)  
 [<span data-ttu-id="b65c5-478">Proces spravovaného spouštění</span><span class="sxs-lookup"><span data-stu-id="b65c5-478">Managed Execution Process</span></span>](../../../docs/standard/managed-execution-process.md)  
 [<span data-ttu-id="b65c5-479">Jak běhové prostředí vyhledává sestavení</span><span class="sxs-lookup"><span data-stu-id="b65c5-479">How the Runtime Locates Assemblies</span></span>](../../../docs/framework/deployment/how-the-runtime-locates-assemblies.md)  
 [<span data-ttu-id="b65c5-480">Příkazové řádky</span><span class="sxs-lookup"><span data-stu-id="b65c5-480">Command Prompts</span></span>](../../../docs/framework/tools/developer-command-prompt-for-vs.md)

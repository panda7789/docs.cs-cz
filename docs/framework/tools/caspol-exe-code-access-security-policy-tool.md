...
> [!IMPORTANT]
> Počínaje .NET Framework 4 nástroj Caspol. exe neovlivňuje zásady CAS, pokud `true` [ \<není element > legacyCasPolicy](../../../docs/framework/configure-apps/file-schema/runtime/netfx40-legacysecuritypolicy-element.md) nastaven na hodnotu. Libovolná nastavení zobrazená nebo upravená pomocí nástroje CasPol.exe ovlivňují pouze aplikace, které se přihlašují pomocí zásad CAS. Další informace najdete v tématu [změny zabezpečení](../../../docs/framework/security/security-changes.md).  
...
|**-e** [**xecution**] &#124; { **vypnuto**}|Vypíná nebo zapíná mechanismus, jenž ověřuje, která oprávnění se mají před spuštěním kódu provést. **Poznámka:**  Tento přepínač je odebraný v .NET Framework 4 a novějších verzích. Další informace najdete v tématu [změny zabezpečení](../../../docs/framework/security/security-changes.md).|  

...
|`outputDirectory`|Pro aplikace Windows 8. x pro Store určí adresář, ve kterém se zapíše soubor. resw, který obsahuje prostředky řetězce v `filename.extension`. `outputDirectory` už musí existovat.|  
...
### <a name="conditionally-compiling-resources"></a>Podmíněná kompilace prostředků  
 ...
 V souboru. txt nebo. restext použijte `#ifdef`...`#endif` konstrukce pro zahrnutí prostředku do binárního souboru. Resources v případě, že je definován symbol, a použijete konstrukci `#if !`... `#endif` k zahrnutí prostředku, pokud není definován symbol. V době kompilace pak definujete symboly pomocí možnosti `/define:` následovaného seznamem symbolů oddělených čárkami. Porovnání je použita; velikost symbolů definovaných v `/define` musí odpovídat znakům v textových souborech, které mají být zkompilovány.  
 ...
 *Language*  
 Jazyk, v němž má být vygenerován zdrojový kód pro třídu prostředků se silnými typy. Možné hodnoty jsou `cs`, `C#`a `csharp` pro C# kód, `vb` a `visualbasic` pro kód Visual Basic, `vbs` a `vbscript` pro kód jazyka VBScript a `c++`, `mc`a `cpp` C++ kódu.  

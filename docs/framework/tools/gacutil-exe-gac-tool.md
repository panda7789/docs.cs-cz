...
> ```
>
> Je to proto, že interně Gacutil.exe sestaví cestu až MAX_PATH znaků, které se skládají z následujících prvků:
>
...
Následující příkaz se nainstaluje `myDll.dll` do globální mezipaměti sestavení (GAC) a přidá odkaz pro jeho počítání. Sestavení `myDll.dll` používá aplikace `MyApp` . `UNINSTALL_KEY MyApp`Parametr určuje klíč registru, který se přidá `MyApp` do ovládacího panelu Přidat nebo odebrat programy v systému Windows. Parametr Description je zadán jako `My Application Description` .

```console
gacutil /i /r myDll.dll UNINSTALL_KEY MyApp "My Application Description"
```

Následující příkaz se nainstaluje `myDll.dll` do globální mezipaměti sestavení (GAC) a přidá odkaz pro jeho počítání. Parametr schématu, `FILEPATH` a parametr ID `c:\applications\myApp\myApp.exe` Zadejte cestu k aplikaci, která instaluje `myDll.dll.` Parametr Description, je zadána jako `MyApp` .

```console
gacutil /i /r myDll.dll FILEPATH c:\applications\myApp\myApp.exe MyApp
```

Následující příkaz se nainstaluje `myDll.dll` do globální mezipaměti sestavení (GAC) a přidá odkaz pro jeho počítání. Parametr schématu `OPAQUE` umožňuje přizpůsobit parametry ID a popisu.

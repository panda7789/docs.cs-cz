...
Existují dva způsoby, jak nasadit rozhraní .NET pro Apache Spark úlohy do datacihly: `spark-submit` a nastavit jar. 

## <a name="deploy-using-spark-submit"></a>Nasazení pomocí Spark-Submit

K odeslání .NET pro úlohy Apache Spark do datacihly můžete použít příkaz [Spark-Submit](https://spark.apache.org/docs/latest/submitting-applications.html) . `spark-submit` umožňuje odesílání pouze do clusteru, který se vytvoří na vyžádání.
...
2. Nahrajte příslušný `microsoft-spark-<spark-version>-<spark-dotnet-version>.jar`.

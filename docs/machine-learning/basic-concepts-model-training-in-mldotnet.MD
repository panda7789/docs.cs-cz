...
- [*Transformer* ](#transformer) reprezentovaná jako `ITransformer` rozhraní.
  ...
  - Další příklad, text tokenizátor přijímá jeden textový sloupec a výstupní sloupce vektoru s jednotlivá slova extrahovaná z texty.
- [*Čtecí modul dat* ](#data-reader) reprezentovaná jako `IDataReader<T>` rozhraní.
  ...
  - Například *TextLoader* je `IDataReader<FileSource>`. Vezme zdrojového souboru a vytvoří data. 
...
- [*Funkce předpovědi* ](#prediction-function) vyjádřené `PredictionFunction<TSrc, TDst>` třídy.
...
Transformátoru je komponenta, která přijímá data, některé s ní pracují a vrátí nový Transformovaná data.

Většina transformátory v ML.NET mají tendenci provozovat na jednom *vstupní sloupec* v době a vytvoří *výstupní sloupec*. Například `new HashTransformer("foo", "bar")` přijímá hodnoty ze sloupce "foo", je pomocí algoritmu hash a umístit je do sloupce "panel". 

Také je běžné, že názvy sloupců vstupních a výstupních jsou stejné. V takovém případě staré sloupce se nahradí novým. Například `new HashTransformer("foo")` vezme hodnoty ze sloupce "foo", hodnota hash je a put je zpět do "foo". 

Všechny transformace, samozřejmě, vytváří nová data zobrazit při `Transform` je volána. Mějte na paměti, data zobrazení jsou neměnné.
...
žádný skutečný výpočtu se stane. Až poté, co se zobrazí kurzor z `newData` a provede počáteční hodnota využívání `newData` vyvolat `transformer` logiku transformace (a ještě, že pouze pokud `transformer` dotyčný je potřeba k vytvoření požadované sloupce).

### <a name="transformer-chains"></a>Transformace řetězců

Je užitečné vlastnost transformátoru *sekvenční provádění transformátory můžete frázi zatím jiné transformer*:
...
Čtecí modul dat není ML.NET součást pro vytváření dat: použije instanci `T` a vrátí data z ní.
...
Jiné podobnosti transformátory je, že od laxně vyhodnocení dat *čtenáři jsou opožděná*: žádný (nebo minimální) skutečné "čtení" se stane, když voláte `dataReader.Read()`: pouze pokud je požadováno kurzor na Výsledná data se Čtečka začít pracovat.
...
Funkce předpovědi se dají považovat za počítač, který transformátoru se vztahuje na jeden řádek.
...
Ale to je náročné a má za následek nákladů na výkon. Místo toho objekt funkce předpovědi provádí stejný pracovní, ale rychleji a efektivněji pomocí metody rozšíření `MakePredictionFunction`:

...
|`statements`|Volitelná. Jeden nebo více příkazů mezi `With` a `End With`, které mohou odkazovat na členy objektu, které jsou vytvářeny vyhodnocením `objectExpression`.|
...
`objectExpression` je vyhodnocen jednou po vstupu do bloku. Nemůžete znovu přiřadit `objectExpression` v rámci bloku `With`.

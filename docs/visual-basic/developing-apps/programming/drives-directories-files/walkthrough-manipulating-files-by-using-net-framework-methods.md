...
1. <xref:System.Windows.Forms.ComboBox> `PickEntries`slouží k zobrazení dat, na kterých uživatel každou položku odesílá, takže uživatel může vybrat položku z konkrétní datum. Vytvořte obslužnou rutinu události <xref:System.Windows.Forms.Control.Click> k tlačítku `GetEntries` a přidejte následující kód.

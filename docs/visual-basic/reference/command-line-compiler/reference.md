...
## <a name="remarks"></a>Poznámky  
 Soubory, které importujete, musí obsahovat metadata sestavení. Mimo sestavení jsou viditelné pouze veřejné typy. Možnost [-addmodule –](addmodule.md) Importuje metadata z modulu.  
...
- Je vyvoláno pole, vlastnost, událost nebo metoda, které mají návratový typ nebo typ parametru ze sestavení B.  
 ...
 Ve výchozím nastavení se používá soubor odezvy Vbc. rsp, který odkazuje na běžně používaná .NET Framework sestavení. Použijte `-noconfig` v případě, že nechcete, aby kompilátor používal Vbc. rsp.  
  
 Krátká forma `-reference` je `-r` .  
  
## <a name="example"></a>Příklad  
 Následující příkaz zkompiluje zdrojový soubor `Input.vb` a referenční sestavení z `Metad1.dll` a `Metad2.dll` do `Out.exe` .  

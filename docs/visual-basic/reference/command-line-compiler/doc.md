...
|`file`|Vyžaduje se `-doc:` , pokud se používá. Určuje výstupní soubor XML, který je vyplněn komentáři ze souborů zdrojového kódu kompilace. Pokud název souboru obsahuje mezeru, uzavřete název do uvozovek ("").|  
  
## <a name="remarks"></a>Poznámky  
 `-doc` Možnost určuje, zda kompilátor GENERUJE soubor XML obsahující dokumentační komentáře. Použijete-li `-doc:file` syntaxi, `file` parametr určuje název souboru XML. Použijete `-doc` -li `-doc+`nebo, kompilátor převezme název souboru XML ze spustitelného souboru nebo knihovny, kterou vytvořil kompilátor. Použijete `-doc-` -li nebo nezadáte `-doc` možnost, kompilátor nevytvoří soubor XML.  

...
|**Možnost**|**Chování**|
|----------------|------------------|
|`-target:exe`|Způsobí, že kompilátor vytvoří spustitelnou konzolovou aplikaci.<br /><br /> Toto je výchozí možnost, pokud není `-target` zadána žádná možnost. Spustitelný soubor se vytvoří s příponou. exe.<br /><br /> Pokud není uvedeno jinak s `-out` možností, název výstupního souboru převezme název vstupního souboru, který obsahuje `Sub Main` proceduru.<br /><br /> V souborech `Sub Main` zdrojového kódu, které jsou zkompilovány do souboru. exe, je vyžadována pouze jedna procedura. Pomocí možnosti `-main` kompilátoru určete, která třída obsahuje `Sub Main` proceduru.|

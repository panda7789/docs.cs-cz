...
Kompilátor příkazového řádku Visual Basic podporuje úplnou sadu možností, které ovládají vstupní a výstupní soubory, sestavení a možnosti ladění a preprocesoru. Každá možnost je k dispozici ve dvou vzájemně zaměnitelné formě: `-option` a `/option`. Tato dokumentace obsahuje pouze formulář `-option`.
...
|Zkompilovat všechny soubory Visual Basic v aktuálním adresáři s optimalizací na a `DEBUG` definovaným symbolem a vyprodukováním souboru Soubor2. exe|`vbc -define:DEBUG=1 -optimize -out:File2.exe *.vb`|

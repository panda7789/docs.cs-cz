...
### <a name="retrieving-values"></a>Načítání hodnot  
 K načtení hodnoty proměnné typu s povolenou hodnotou Null, měli byste nejdřív otestovat jeho <xref:System.Nullable%601.HasValue%2A> vlastnost potvrďte, že má hodnotu. Pokud se pokusíte načíst hodnotu při <xref:System.Nullable%601.HasValue%2A> je `False`, Visual Basic, vyvolá <xref:System.InvalidOperationException> výjimka. Následující příklad ukazuje doporučeným způsobem, jak načíst proměnnou `numberOfChildren` předchozí příklady.  

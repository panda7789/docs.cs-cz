...
`String` datový typ nerozšiřuje [datový typ char](../../../../visual-basic/language-reference/data-types/char-data-type.md). To znamená, že pokud chcete přiřadit literál k proměnné `Char`, je nutné buď provést zužující převod, nebo vynutit literál pro typ `Char`.
...
Jeden `Char` a pole `Char` prvků se rozšíří na `String`. `String` se ale nerozšíří na `Char()`. Chcete-li převést `String` hodnotu na pole `Char`, můžete použít metodu <xref:System.String.ToCharArray%2A> třídy <xref:System.String?displayProperty=nameWithType>.

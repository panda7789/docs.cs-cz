...
## <a name="structure-of-a-linq-query"></a>Struktura dotazu LINQ  
 ...
 Výraz dotazu začíná `From` klauzulí. Tato klauzule identifikuje zdrojová data pro dotaz a proměnné, které se používají k odkazování na jednotlivé prvky zdrojových dat. Tyto proměnné jsou pojmenované *proměnné rozsahu* nebo *proměnné iterace*. `From`Klauzule je vyžadována pro dotaz, s výjimkou `Aggregate` dotazů, kde `From` klauzule je volitelná. Po identifikaci oboru a zdroje dotazu v `From` `Aggregate` klauzulích or můžete zahrnout libovolnou kombinaci klauzulí dotazu pro upřesnění dotazu. Podrobnosti o klauzulích dotazu naleznete v tématu Visual Basic operátory dotazů LINQ dále v tomto tématu. Následující dotaz například identifikuje zdrojovou kolekci zákaznických dat jako `customers` proměnnou a proměnnou iterace s názvem `cust` .  
...
Není-li `Select` zadána klauzule, proměnné iterace pro dotaz se skládají z proměnných iterace určených `From` `Aggregate` klauzulí or.
...
Nepovinný parametr. `Distinct`Klauzule omezuje hodnoty aktuální proměnné iterace tak, aby se vyloučily duplicitní hodnoty ve výsledcích dotazu. Příklad:
...
## <a name="deferred-and-immediate-query-execution"></a>Odložené a okamžité provedení dotazu
 ...
 Použití `ToList` metod nebo `ToArray` bude také vynutit okamžité provedení. To může být užitečné, když chcete spustit dotaz okamžitě a uložit výsledky do mezipaměti. Další informace o těchto metodách naleznete v tématu [Převod datových typů](../../concepts/linq/converting-data-types.md).  
...
|[LINQ na objekty](../../concepts/linq/linq-to-objects.md)|Obsahuje obecné informace, pokyny pro programování a ukázky pro LINQ to Objects.|  
...
|[Technologie LINQ to XML](../../concepts/linq/linq-to-xml.md)|Obsahuje obecné informace, pokyny pro programování a ukázky pro LINQ to XML.|  

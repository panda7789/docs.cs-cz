...
- Z názvů proměnných. `anonProduct` anonymního typu budou mít dvě vlastnosti, `productName` a `productPrice`. Jejich datové typy budou ty z původních proměnných, `String` a `Double`, v uvedeném pořadí.

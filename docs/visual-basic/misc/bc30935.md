...
```  
 ...
 Vzhledem k tomu, že `derivedClass` dědí do parametru typu `t` @no__t-@no__t 1, budou dvě verze `doSomething` v `baseClass` přebírat stejné signatury až do `derivedClass`. V důsledku toho kompilátor nemůže určit, která verze se má přepsat.  

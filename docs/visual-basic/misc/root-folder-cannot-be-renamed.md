---
title: Kořenovou složku nelze přejmenovat.
ms.date: 07/20/2015
ms.assetid: 325eb73c-d5fe-4b65-8926-5dabff007277
ms.openlocfilehash: 19cc5e59672736f36d83e0b8a7d9040665ed1065
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 05/04/2018
ms.locfileid: "33639569"
---
# <a name="root-folder-cannot-be-renamed"></a>Kořenovou složku nelze přejmenovat.
Byl proveden pokus o přejmenování kořenový adresář. To může být způsobeno nesprávně Analyzovaná souboru cestu.  
  
## <a name="to-correct-this-error"></a>Oprava této chyby  
  
-   Zkontrolujte cestu k souboru a ujistěte se, zda je správný.  
  
## <a name="see-also"></a>Viz také  
 [My.Computer.FileSystem.RenameDirectory]xref:Microsoft.VisualBasic.MyServices.FileSystemProxy.RenameDirectory)  
 [My.Computer.FileSystem.RenameFile]xref:Microsoft.VisualBasic.MyServices.FileSystemProxy.RenameFile)  
 [Vytváření, odstraňování a přesouvání souborů a adresářů](../../visual-basic/developing-apps/programming/drives-directories-files/creating-deleting-and-moving-files-and-directories.md)  
 [Postupy: přejmenování souboru v jazyce Visual Basic](../developing-apps/programming/drives-directories-files/how-to-rename-a-file.md)  


---
title: RemoveHandler už je deklarovaný.
ms.date: 07/20/2015
f1_keywords:
- vbc31128
- bc31128
helpviewer_keywords:
- BC31128
ms.assetid: bf290050-cd9c-4ca4-a356-e6fe3bfd68ba
ms.openlocfilehash: eb61eb9ba8e76f70e0c16bf5bd27b4949237df64
ms.sourcegitcommit: f8c270376ed905f6a8896ce0fe25b4f4b38ff498
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 06/04/2020
ms.locfileid: "84395983"
---
# <a name="removehandler-is-already-declared"></a>RemoveHandler už je deklarovaný.
`RemoveHandler`V deklaraci vlastní události se objevilo víc než jedna deklarace. `RemoveHandler`Deklarace deklaruje proceduru použitou k odebrání obslužné rutiny události.  
  
 **ID chyby:** BC31128  
  
## <a name="to-correct-this-error"></a>Oprava této chyby  
  
- Odeberte redundantní `RemoveHandler` příkaz.  
  
## <a name="see-also"></a>Viz také

- [RemoveHandler – příkaz](../language-reference/statements/removehandler-statement.md)
- [Event – příkaz](../language-reference/statements/event-statement.md)

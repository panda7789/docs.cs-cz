---
title: Parametr typu '&lt;typeparametername&gt;' má stejný název jako parametr typu nadřazeného typu.
ms.date: 07/20/2015
f1_keywords:
- vbc40048
- bc40048
helpviewer_keywords:
- BC40048
ms.assetid: d5428b36-88d3-42c4-a096-cbc7bb9571f2
ms.openlocfilehash: ce09d74e818cf99d637c67bc57cb409166336255
ms.sourcegitcommit: 0888d7b24f475c346a3f444de8d83ec1ca7cd234
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 12/22/2018
ms.locfileid: "53764238"
---
# <a name="type-parameter-lttypeparameternamegt-has-the-same-name-as-a-type-parameter-of-an-enclosing-type"></a>Parametr typu '&lt;typeparametername&gt;' má stejný název jako parametr typu nadřazeného typu.
Typ parametru obecného typu je deklarována se stejným názvem jako parametr typu nadřazeného obecného typu.  
  
 V seznamu parametrů typu obecný typ každý parametr typu musí mít název liší od všechny následující názvy:  
  
-   Každý další parametr typu v seznamu parametrů stejného typu  
  
-   Každý parametr typu v seznamu parametrů typu nadřazeného obecného typu, a  
  
-   Název obecného typu, samotného.  
  
 Ve výchozím nastavení tato zpráva je upozornění. Informace o zobrazení nebo skrytí upozornění zpracování upozornění jako chyby, najdete v části [Konfigurace upozornění v jazyce Visual Basic](/visualstudio/ide/configuring-warnings-in-visual-basic).  
  
 **ID chyby:** BC40048  
  
## <a name="to-correct-this-error"></a>Oprava této chyby  
  
-   Přejmenujte parametr typu se lišit od každý název uvedeným v seznamu na tuto stránku nápovědy.  
  
## <a name="see-also"></a>Viz také  
 [Obecné typy v jazyce Visual Basic](../../visual-basic/programming-guide/language-features/data-types/generic-types.md)  
 [Seznam typů](../../visual-basic/language-reference/statements/type-list.md)

---
title: Člen atributu <membername> nemůže být cílem přiřazení, protože není deklarovaný jako Public.
ms.date: 07/20/2015
f1_keywords:
- vbc31511
- bc31511
helpviewer_keywords:
- BC31511
ms.assetid: f8c958f6-58a4-4aff-b8c3-f2e9481e8132
ms.openlocfilehash: 606dc26d4da99d7b028ea00617b41957f1af0e32
ms.sourcegitcommit: f8c270376ed905f6a8896ce0fe25b4f4b38ff498
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 06/04/2020
ms.locfileid: "84400988"
---
# <a name="attribute-member-membername-cannot-be-the-target-of-an-assignment-because-it-is-not-declared-public"></a>Člen atributu \<membername> nemůže být cílem přiřazení, protože není deklarovaný jako Public.
Byl proveden pokus o přiřazení hodnoty k soukromému členu atributu.  
  
 **ID chyby:** BC31511  
  
## <a name="to-correct-this-error"></a>Oprava této chyby  
  
1. Odeberte přiřazení.  
  
2. Pokud používáte vlastní atributy, které jste vyvinuli, změňte modifikátor přístupu člena atributu na `Public` .  
  
## <a name="see-also"></a>Viz také

- [Přehled atributů](../programming-guide/concepts/attributes/index.md)
- [Republik](../language-reference/modifiers/public.md)

---
title: Datový proud předaný TextFieldParser nelze číst.
ms.date: 07/20/2015
f1_keywords:
- vbrTextFieldParser_StreamNotReadable
ms.assetid: b6ac8043-b939-441c-822f-de6f16f45525
ms.openlocfilehash: 11964564507d2e0f2718c3c182ca57db15635a64
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 05/04/2018
ms.locfileid: "33639470"
---
# <a name="the-stream-passed-to-textfieldparser-cannot-be-read"></a>Datový proud předaný TextFieldParser nelze číst.
[Objektu TextFieldParser](../../visual-basic/language-reference/objects/textfieldparser-object.md) nelze číst datový proud, který byl předán do ní. To může být způsobeno při čtení souboru, který není do textového souboru.  
  
## <a name="to-correct-this-error"></a>Oprava této chyby  
  
-   Zkontrolujte, ujistěte se, že soubor je textový soubor.  
  
## <a name="see-also"></a>Viz také  
 [Postupy: Čtení z textových souborů ve více formátech](../../visual-basic/developing-apps/programming/drives-directories-files/how-to-read-from-text-files-with-multiple-formats.md)  
 [My.Computer.FileSystem.OpenTextFieldParser](xref:Microsoft.VisualBasic.FileIO.FileSystem.OpenTextFieldParser%2A)  
 [Analýza textových souborů pomocí objektu TextFieldParser](../../visual-basic/developing-apps/programming/drives-directories-files/parsing-text-files-with-the-textfieldparser-object.md)  
 [Objekt TextFieldParser](../../visual-basic/language-reference/objects/textfieldparser-object.md)

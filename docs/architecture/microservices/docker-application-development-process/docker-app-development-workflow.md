...
Proces vývoje pro aplikace Docker: 1 – kódování vaší aplikace, 2-zápis souboru Dockerfile/s, 3-vytváření imagí definovaných na souboru Dockerfile/s, 4 – (volitelné) psaní služeb v souboru Docker-Compose. yml, 5 spuštění kontejneru nebo Docker – sestavování, 6-testování aplikace nebo mikroslužeb, 7-nabízení do úložiště a opakování.
:::image-end:::
...
Pokud používáte sadu Visual Studio 2019, mnohé z těchto kroků jsou zpracovávány za vás, což výrazně zvyšuje vaši produktivitu. To platí hlavně v případě, že používáte Visual Studio 2019 a cílíte na aplikace s více kontejnery. Například při použití jediného kliknutí myši Visual Studio přidá `Dockerfile` `docker-compose.yml` soubor a do vašich projektů s konfigurací vaší aplikace. Při spuštění aplikace v aplikaci Visual Studio se vytvoří image Docker a aplikace s více kontejnery se spustí přímo v Docker; i když vám umožní ladit několik kontejnerů najednou. Tyto funkce zvýší vaši rychlost vývoje.
...
Kromě toho potřebujete Visual Studio 2019 verze 16,4 nebo novější s nainstalovanou úlohou **vývoje .NET Core pro různé platformy** , jak je znázorněno na obrázku 5-2.
...
**Obrázek 5-2**. Výběr úlohy **vývoje .NET Core pro různé platformy** během instalace sady Visual Studio 2019
...
Pomocí sady Visual Studio a jejích nástrojů pro Docker Tato úloha vyžaduje jenom několik kliknutí myší. Při vytváření nového projektu v aplikaci Visual Studio 2019 existuje možnost s názvem **Povolit podporu Docker**, jak je znázorněno na obrázku 5-3.
...
**Obrázek 5-3**. Povolení podpory Docker při vytváření nového projektu ASP.NET Core v aplikaci Visual Studio 2019
...
**Obrázek 5-4**. Povolení podpory Docker v existujícím projektu sady Visual Studio 2019
...
Podobným způsobem může Visual Studio také přidat `docker-compose.yml` soubor pro celé řešení s možností **Přidat > kontejner Orchestrator support...**. V kroku 4 si tuto možnost podrobněji prozkoumáme.

### <a name="using-an-existing-official-net-docker-image"></a>Použití stávající oficiální image rozhraní .NET Docker

Obvykle vytvoříte vlastní image pro kontejner nad základní imagí, kterou získáte z oficiálního úložiště, jako je registr [Docker Hub](https://hub.docker.com/) . To je přesně to, co se stane v rámci pokrývání, když povolíte podporu Docker v aplikaci Visual Studio. Vaše souboru Dockerfile použije existující `dotnet/core/aspnet` image.

Dříve jsme zjistili, které image a úložiště Docker můžete použít v závislosti na zvoleném rozhraní a operačním systému. Například pokud chcete použít ASP.NET Core (Linux nebo Windows), obrázek, který se má použít, je `mcr.microsoft.com/dotnet/core/aspnet:3.1` . Proto stačí určit, jakou základní image Docker budete pro svůj kontejner používat. Můžete to udělat přidáním `FROM mcr.microsoft.com/dotnet/core/aspnet:3.1` do svého souboru dockerfileu. To se automaticky provede v aplikaci Visual Studio, ale pokud byste chtěli verzi aktualizovat, aktualizujete tuto hodnotu.
...
V tomto případě je image založená na verzi 3,1 oficiální image ASP.NET Core Docker (více imagí pro Linux a Windows). Toto je nastavení `FROM mcr.microsoft.com/dotnet/core/aspnet:3.1` . (Další informace o této základní imagi najdete na stránce s [obrázkem Docker .NET Core](https://hub.docker.com/_/microsoft-dotnet-core/) .) V souboru Dockerfile musíte také dát Docker pokyn, aby naslouchal na portu TCP, který budete používat při běhu (v tomto případě port 80, jak je nakonfigurovaný s nastavením zpřístupnit).
...
- `mcr.microsoft.com/dotnet/core/aspnet:3.1-buster-slim` \
  Cíle: modul runtime .NET Core 3,1 – pouze v systému Linux

- `mcr.microsoft.com/dotnet/core/aspnet:3.1-nanoserver-1909` \
  Cíle: modul runtime .NET Core 3,1 – pouze v systému Windows nano Server

Pokud ale zadáte stejný název bitové kopie, dokonce i se stejnou značkou, používají se pro více imagí (jako je `aspnet` image) verze systému Linux nebo Windows, a to v závislosti na tom, jak je znázorněno v následujícím příkladu:

- `mcr.microsoft.com/dotnet/core/aspnet:3.1` \
  Vícenásobný arch: rozhraní .NET Core 3,1 runtime – pouze v systémech Linux nebo Windows nano Server v závislosti na hostitelském operačním systému Docker
...
Ale `dotnet restore` spustí se jenom v případě, že je ve složce jeden soubor projektu nebo řešení, takže je to trochu složitější a způsob, jak ho vyřešit, aniž by bylo možné získat příliš mnoho podrobností, je to:
...
2. Zahrňte `/ignoreprojectextensions:.dcproj` argument do `dotnet restore` , takže také ignoruje projekt Docker-Resolution a obnoví pouze balíčky pro řešení EShopOnContainers-ServicesAndWebApps.
...
Místo toho, aby bylo možné přímo spustit sestavení Docker ze složky projektu, můžete nejprve vytvořit nasazenou složku s požadovanými knihovnami a binárními soubory .NET spuštěním `dotnet publish` a potom použít `docker build` příkaz.
...
Příklad souboru Docker-Compose. yml definuje čtyři služby: `webmvc` službu (webová aplikace), dvě mikroslužby ( `ordering-api` a `basket-api` ) a jeden kontejner zdrojů dat, na `sqldata` základě SQL Server pro Linux spuštěnou jako kontejner. Každá služba bude nasazena jako kontejner, takže se pro každý z nich vyžaduje image Docker.

Soubor Docker-Compose. yml určuje nejen to, jaké kontejnery jsou používány, ale jak jsou konfigurovány samostatně. Například `webmvc` definice kontejneru v souboru. yml:
...
### <a name="working-with-docker-composeyml-in-visual-studio-2019"></a>Práce s Docker-Compose. yml v aplikaci Visual Studio 2019
...
V době psaní tohoto zápisu aplikace Visual Studio podporuje orchestraci **Docker Compose** a **Kubernetes/Helm** .
...
**Obrázek 5-7**. Přidání podpory Docker do sady Visual Studio 2019 tak, že kliknete pravým tlačítkem na projekt ASP.NET Core
...
**Obrázek 5-8**. Uzel stromu **Docker-psacího** uzlu přidaný do sady Visual Studio 2019 Průzkumník řešení

Pomocí příkazu můžete nasadit aplikaci s více kontejnery s jedním souborem Docker-Compose. yml `docker-compose up` . Visual Studio však přidá skupinu, takže můžete přepsat hodnoty v závislosti na prostředí (vývoj nebo produkce) a typu spuštění (vydání nebo ladění). Tato funkce bude vysvětlena v dalších částech.
...
Pokud má vaše aplikace jenom jeden kontejner, můžete ho spustit nasazením na hostitele Docker (virtuální počítač nebo fyzický server). Pokud však vaše aplikace obsahuje více služeb, můžete ji nasadit jako složenou aplikaci, a to buď pomocí jediného příkazu CLI ( `docker-compose up)` nebo pomocí sady Visual Studio, která bude tento příkaz používat v rámci pokrývání. Pojďme se podívat na různé možnosti.
...
Kontejner Docker můžete spustit pomocí `docker run` příkazu, jak je znázorněno na obrázku 5-9:

```console
docker run -t -d -p 80:5000 cesardl/netcore-webapi-microservice-docker:first
```

Výše uvedený příkaz vytvoří novou instanci kontejneru ze zadané image pokaždé, když se spustí. Tento parametr můžete použít `--name` k předání názvu kontejneru a následnému použití `docker start {name}` (nebo k použití ID kontejneru nebo automatického názvu) ke spuštění existující instance kontejneru.
...
Zobrazený hash je ID kontejneru a je mu přiřazen také náhodný čitelný název, pokud se `--name` možnost nepoužívá.
...
Spuštění aplikace s více kontejnery pomocí sady Visual Studio 2019 nemůže získat jednodušší. Stačí stisknout **kombinaci kláves CTRL + F5** ke spuštění nebo **F5** pro ladění, jako obvykle, nastavení projektu **Docker-sestavit** jako spouštěného projektu.  Visual Studio zpracovává všechna potřebná nastavení, takže můžete vytvořit zarážky jako obvykle a ladit tak, co se stane samostatnými procesy běžícími v "vzdálených serverech" s ladicím programem, který je už připojený, stejně jako takový.
...
**Obrázek 5-12**. Spouštění aplikací s více kontejnery v aplikaci Visual Studio 2019
...
### <a name="testing-and-debugging-containers-with-visual-studio-2019"></a>Testování a ladění kontejnerů pomocí sady Visual Studio 2019

Při spuštění a ladění kontejnerů pomocí sady Visual Studio 2019 můžete ladit aplikaci .NET podobným způsobem jako při spuštění bez kontejnerů.

### <a name="testing-and-debugging-without-visual-studio"></a>Testování a ladění bez sady Visual Studio

Pokud vyvíjíte pomocí přístupu Editor/CLI, kontejnery ladění jsou obtížnější a pravděpodobně budete chtít ladit vygenerováním trasování.
...
- **Steve Lasker. Vývoj .NET Docker pomocí sady Visual Studio (2017)** \

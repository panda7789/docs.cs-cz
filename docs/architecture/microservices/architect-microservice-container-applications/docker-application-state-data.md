...
Pro správu dat v aplikacích Docker se používají následující řešení:
...
- **Překryvný systém souborů**. Tato funkce Docker implementuje úlohu kopírování a zápisu, která ukládá aktualizované informace do kořenového systému souborů kontejneru. Tyto informace jsou "na začátku" původního obrázku, na kterém je kontejner založen. Pokud se kontejner odstraní ze systému, tyto změny se ztratí. Proto je možné uložit stav kontejneru v rámci svého místního úložiště. návrh systému kolem tohoto by byl v konfliktu s interním návrhem kontejneru, který ve výchozím nastavení je bezstavový.

Použití svazků Docker je teď preferovaným způsobem, jak zpracovávat místní data v Docker. Pokud potřebujete další informace o úložišti v kontejnerech, podívejte se na [ovladače úložiště Docker](https://docs.docker.com/storage/storagedriver/select-storage-driver/) a na [ovladače úložiště](https://docs.docker.com/storage/storagedriver/).
...
**tmpfs připojení** jsou v podstatě virtuální složky, které jsou živě pouze v paměti hostitele a nejsou nikdy zapsány do systému souborů. Jsou rychlé a bezpečné, ale využívají paměť a jsou určeny jenom pro dočasná, netrvalá data.

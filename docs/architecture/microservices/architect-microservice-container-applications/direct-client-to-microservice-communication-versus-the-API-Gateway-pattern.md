...
Azure API Management řeší jak vaše brána rozhraní API, tak i jejich požadavky na správu, jako je protokolování, zabezpečení, měření atd. V takovém případě, když používáte produkt, jako je Azure API Management, skutečnost, že máte jedinou bránu API, není tak riskantní, protože tyto druhy bran rozhraní API jsou "tenčí", což znamená, že neimplementujete vlastní kód C#, který by se mohl vyvíjet ke komponentě monolitické.

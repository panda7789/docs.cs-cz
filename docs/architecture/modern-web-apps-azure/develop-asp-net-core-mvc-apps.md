...
Do projektu můžete přidat `ValidateModelAttribute` jako závislost NuGet zahrnutím balíčku [Ardalis. ValidateModel](https://www.nuget.org/packages/Ardalis.ValidateModel) . Pro rozhraní API můžete použít `ApiController` atribut k vykonání tohoto chování bez nutnosti samostatného `ValidateModel` filtru.

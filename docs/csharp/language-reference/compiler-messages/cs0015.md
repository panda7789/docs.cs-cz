---
title: Chyba kompilátoru CS0015
ms.date: 07/20/2015
f1_keywords:
- CS0015
helpviewer_keywords:
- CS0015
ms.assetid: e9d3272b-2b80-435a-98ef-79bff158ae7b
ms.openlocfilehash: c5e08b57ba97df2a723ca020dd9fa5ee44b4fb27
ms.sourcegitcommit: 8b8dd14dde727026fd0b6ead1ec1df2e9d747a48
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 09/27/2019
ms.locfileid: "71332090"
---
# <a name="compiler-error-cs0015"></a><span data-ttu-id="d9e86-102">Chyba kompilátoru CS0015</span><span class="sxs-lookup"><span data-stu-id="d9e86-102">Compiler Error CS0015</span></span>

<span data-ttu-id="d9e86-103">Název typu ' type ' je příliš dlouhý.</span><span class="sxs-lookup"><span data-stu-id="d9e86-103">The name of type 'type' is too long</span></span>

 <span data-ttu-id="d9e86-104">Plně kvalifikovaný název uživatelsky definovaného typu musí mít méně než 1024 znaků, včetně teček.</span><span class="sxs-lookup"><span data-stu-id="d9e86-104">The fully qualified name of a user-defined type must have fewer than 1024 characters, including the periods.</span></span>

 <span data-ttu-id="d9e86-105">Následující ukázka generuje CS0015:</span><span class="sxs-lookup"><span data-stu-id="d9e86-105">The following sample generates CS0015:</span></span>

```csharp
// CS0015.cs

// Remove a C from one of the namespace names or the class name to resolve the error by
// reducing the number of characters in the qualified class name to fewer than 1024.

namespace CC
{
    // The following namespace name has 510 characters.
    namespace 
    {
        // The following class name has 510 characters. The qualified class name,
        // CC.CCC510chars…C.CCCC510chars…C (namespace.namespace.class), has 1024
        // characters, which causes compiler error CS0015.
        public class 
        {
            public static void Main()
            {
            }
        }
    }
}
```

...
Nelze přiřadit k proměnné, protože se jedná o typ proměnné jen pro čtení.  
 ...
 K této chybě dojde, pokud se přiřazení k proměnné vyskytne v kontextu jen pro čtení. Kontexty jen pro čtení obsahují proměnné iterace [foreach](../keywords/foreach-in.md) , [pomocí](../keywords/using-statement.md) proměnných a [pevných](../keywords/fixed-statement.md) proměnných. Chcete-li tuto chybu vyřešit, vyhněte se přiřazení k `using` proměnné příkazu `foreach` v blocích, `fixed` příkazech a příkazech.  
  
## <a name="example"></a>Příklad  
 Následující příklad generuje chybu CS1656, protože se pokusí nahradit kompletní prvky kolekce uvnitř `foreach` smyčky. Jedním ze způsobů, jak tuto chybu obejít, je `foreach` změnit smyčku na smyčku [for](../keywords/for.md) . Dalším způsobem, který zde není zobrazen, je změnit členy stávajícího elementu; To je možné u tříd, ale ne s strukturami.  

...
- Použili jste *direktivu using alias* bez úplného kvalifikovaného názvu typu? Direktiva `using` alias nepoužívá direktivy v souboru zdrojového kódu k překladu typů. `using` Následující příklad generuje CS0246, protože typ `List<int>` není plně kvalifikován. Direktiva pro `System.Collections.Generic` nebrání chybě. `using`  
    ...
    ```  
     ...
     Pokud se zobrazí tato chyba v kódu, který dříve pracoval, vyhledá se v Průzkumník řešení chybějící nebo nevyřešené odkazy. Potřebujete znovu nainstalovat balíček [NuGet](https://www.nuget.org/) ? Informace o tom, jak systém sestavení vyhledává odkazy, naleznete v tématu [řešení odkazů na soubory v týmovém sestavení](https://blogs.msdn.microsoft.com/manishagarwal/2005/09/28/resolving-file-references-in-team-build-part-2/). Pokud se zdá, že jsou všechny odkazy správné, vyhledejte v historii správy zdrojového kódu, co se změnilo v souboru. csproj nebo v místním zdrojovém souboru.  

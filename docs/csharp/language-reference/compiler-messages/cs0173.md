...
Chcete-li vyřešit CS0173, ověřte, zda existuje jeden a pouze jeden implicitní převod mezi `class1` a `class2`, bez ohledu na to, který směr je v a bez ohledu na to, ve které třídě je převod. Další informace najdete v tématech [uživatelsky definované operátory převodu](../operators/user-defined-conversion-operators.md) a [předdefinované číselné převody](../builtin-types/numeric-conversions.md).

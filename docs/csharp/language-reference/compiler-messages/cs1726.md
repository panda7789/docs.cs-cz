---
title: Chyba kompilátoru CS1726
ms.date: 07/20/2015
f1_keywords:
- CS1726
helpviewer_keywords:
- CS1726
ms.assetid: 02b71f22-72f5-42b5-bc9e-1d5dc480cce0
ms.openlocfilehash: bd79db16b754264476e179807c54aaee18e3883e
ms.sourcegitcommit: 7588136e355e10cbc2582f389c90c127363c02a5
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 03/14/2020
ms.locfileid: "79173806"
---
# <a name="compiler-error-cs1726"></a>Chyba kompilátoru CS1726
Odkaz na sestavení přítele 'reference' je neplatný. Podpisová sestavení se silným názvem musí zadat veřejný klíč ve svých deklaracích InternalsVisibleTo.  
  
 Sestavení podepsané silným názvem může udělit přístup <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute>k sestavení friend, provedené pomocí sestavení , jiným silně podepsaným sestavením.  
  
 Chcete-li vyřešit CS1726, podepište (pojmenujte) sestavení, ke kterému chcete udělit přístup přítele, nebo neudělte přístup přítele.  
  
 Další informace naleznete [v tématu Friend Assemblyies](../../../standard/assembly/friend.md).  
  
## <a name="example"></a>Příklad  
 Následující ukázka generuje CS1726.  
  
```csharp  
// Save this code as CS1726.cs  
  
// Run the following command to create CS1726.key:  
//      sn -k CS1726.key  
  
// Then compile by using the following command:
//      csc /keyfile:CS1726.key /target:library CS1726.cs  
  
using System.Runtime.CompilerServices;  
  
// The following line causes compiler error CS1726.  
[assembly: InternalsVisibleTo("UnsignedAssembly")]
  
// To get rid of the error, try the following line instead.  
//[assembly: InternalsVisibleTo("SignedAssembly, PublicKey=")]  
  
class A { }  
```  
  
## <a name="see-also"></a>Viz také

- [Postupy: Vytváření podepsaných přátelských sestavení](../../../standard/assembly/create-signed-friend.md)

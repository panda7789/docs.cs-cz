...
- Modifikátory přístupu nejsou povoleny v [lokální funkce](../../programming-guide/classes-and-structs/local-functions.md). Lokální funkce jsou vždycky privátní.
 ...
 V předchozích verzích sady Visual Studio `static` modifikátor nebylo povolené třídy, ale `static` třídy jsou povoleny, počínaje [!INCLUDE[vsprvsext](~/includes/vsprvsext-md.md)].  

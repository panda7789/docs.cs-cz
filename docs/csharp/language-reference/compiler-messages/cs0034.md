...
```
 ...
 V C# 1,1 se chyba kompilátoru mohla definovat třídu, která má implicitní uživatelsky definované převody jak na `int` a `bool`, tak pro použití bitového operátoru `AND` (`&`) pro objekty daného typu. V C# 2,0 byla tato chyba opravena tak, aby kompilátor byl kompatibilní se C# specifikací, a proto následující kód bude nyní způsobovat CS0034:

...
**Syntax visualizer** umožňuje kontrolu stromu syntaxe pro soubor kódu C# nebo Visual Basic v aktuálním aktivním okně editoru v rámci integrovaného vývojového prostředí (IDE) sady Visual Studio. Vizualizér můžete spustit kliknutím na **Zobrazit** > **ostatní** > **syntax visualizer**Windows.  Můžete také použít panel nástrojů **snadného spuštění** v pravém horním rohu. Zadejte "syntax" a příkaz pro otevření **syntax visualizer** by měl být zobrazen.
...
Vytvořte nový projekt pomocí příkazu **soubor** > **Nový projekt** . Můžete vytvořit buď projekt Visual Basic nebo C#. Když aplikace Visual Studio otevře soubor hlavního kódu pro tento projekt, Vizualizér zobrazí pro něj strom syntaxe. V této instanci sady Visual Studio můžete otevřít libovolný existující soubor C#/Visual Basic a Vizualizér zobrazí strom syntaxe tohoto souboru. Pokud máte v aplikaci Visual Studio otevřeno více souborů kódu, Vizualizér zobrazí strom syntaxe pro aktuálně aktivní soubor kódu (soubor kódu, který má fokus klávesnice).
...
Každá položka ve stromové struktuře také zobrazuje vlastní **Rozsah**. **Rozsah** je indexy (počáteční a koncové umístění) daného uzlu v textovém souboru.  V předchozím příkladu jazyka C# má vybraný token "UsingKeyword [0.. 5)" **Rozsah** , který je pět znaků široká, [0.. 5). Notace "[..)" znamená, že počáteční index je součástí rozsahu, ale koncový index není.
...
Vizualizér zobrazuje grafické znázornění podstromu root na vybrané položce. Vyzkoušejte tento postup pro uzel **MethodDeclaration** , který odpovídá `Main()` metodě v příkladu v jazyce C#. Vizualizér zobrazuje graf syntaxe, který vypadá takto:

![Zobrazení grafu syntaxe jazyka C#](media/syntax-visualizer/csharp-syntax-graph.png)

# <a name="visual-basic"></a>[Visual Basic](#tab/vb)

Zkuste stejný pro uzel **podbloku** odpovídající `Main()` metodě v předchozím příkladu Visual Basic. Vizualizér zobrazuje graf syntaxe, který vypadá takto:
...
Syntax Visualizer umožňuje základní kontrolu symbolů a sémantických informací. V `double x = 1 + 1;` příkladu jazyka C# zadejte v části Main (). Pak vyberte výraz `1 + 1` v okně Editor kódu. Vizualizér zvýrazní uzel **AddExpression** v Vizualizér. Klikněte pravým tlačítkem na tuto **AddExpression** a klikněte na **Zobrazit symbol (pokud existuje)**. Všimněte si, že většina položek nabídky má kvalifikátor "if any". Syntax Visualizer kontroluje vlastnosti uzlu, včetně vlastností, které nemusí být k dispozici pro všechny uzly.
...
Předchozí příklad lze také replikovat v Visual Basic. Zadejte `Dim x As Double = 1 + 1` soubor Visual Basic. V okně editoru `1 + 1` kódu vyberte výraz. Vizualizér zvýrazní odpovídající uzel **AddExpression** v Vizualizér. Opakujte předchozí kroky pro tento **AddExpression** a měli byste vidět identické výsledky.
...
Výše uvedené příklady Visual Basic lze snadno replikovat v jazyce C#. Zadejte `using C = System.Console;` místo `Imports C = System.Console` pro alias. Předchozí kroky v jazyce C# poskytují identické výsledky v okně Vizualizér.

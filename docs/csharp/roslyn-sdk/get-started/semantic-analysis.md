...
Zobrazí kód dokončení pro tato ukázka v [našem úložišti GitHub ukázky](https://github.com/dotnet/samples/csharp/roslyn-sdk/SemanticQuickStart).
...
<xref:Microsoft.CodeAnalysis.CSharp.CSharpCompilation.AddReferences%2A?displayProperty=nameWithType> Metoda přidá reference na kompilace. <xref:Microsoft.CodeAnalysis.MetadataReference.CreateFromFile(System.String,Microsoft.CodeAnalysis.MetadataReferenceProperties,Microsoft.CodeAnalysis.DocumentationProvider)> Metoda načte sestavení jako odkaz. 

## <a name="querying-the-semantic-model"></a>Dotazování sémantického modelu

Jakmile máte <xref:Microsoft.CodeAnalysis.Compilation> můžete požádat ho <xref:Microsoft.CodeAnalysis.SemanticModel> pro žádné <xref:Microsoft.CodeAnalysis.SyntaxTree> obsažené v tom, že <xref:Microsoft.CodeAnalysis.Compilation>. Si můžete představit sémantický model jako zdroj pro všechny informace, které by normálně získat z intellisense. A <xref:Microsoft.CodeAnalysis.SemanticModel> lze zodpovědět otázky typu "Co názvů jsou v oboru v tomto umístění?" "Co členové jsou přístupné z této metody?" "Jaké proměnné se používají v tomto bloku textu?" a "Co tento název nebo výraz k vztahuje?" Přidejte tento příkaz pro vytvoření sémantického modelu:

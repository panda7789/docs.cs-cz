...
## <a name="named-arguments"></a>Pojmenované argumenty  
 ...
 Pojmenované argumenty také zlepšují čitelnost kódu tím, že identifikují, co každý argument představuje. V níže uvedeném příkladu `sellerName` nemůže být null nebo prázdné místo. Jak `sellerName` a `productName` jsou typy řetězců namísto odeslání argumentů podle pozice, má smysl použít pojmenované argumenty k jednoznačnému odstranění těchto dvou a omezení nejasností pro kohokoli, kdo čte kód.
...
- _počínaje jazykem C# 7,2_se používají ve správné pozici. V následujícím příkladu `orderNum` je parametr ve správné pozici, ale není explicitně pojmenován.
...
- výraz formuláře `new ValType()` , kde `ValType` je hodnotový typ, jako je například [výčet](../../language-reference/builtin-types/enum.md) nebo [Struktura](../../language-reference/builtin-types/struct.md);  
...
> [!NOTE]
> Můžete také deklarovat volitelné parametry pomocí <xref:System.Runtime.InteropServices.OptionalAttribute> třídy .NET. `OptionalAttribute`Parametry nevyžadují výchozí hodnotu.  
...
## <a name="com-interfaces"></a>Rozhraní COM  
 ...
 V jazyce C# 3,0 a starších verzích je pro každý parametr požadován argument, jak je znázorněno v následujícím příkladu.  
  
 [!code-csharp[csProgGuideNamedAndOptional#3](~/samples/snippets/csharp/VS_Snippets_VBCSharp/csprogguidenamedandoptional/cs/namedandoptcom.cs#3)]  
  
 Nicméně můžete značně zjednodušit volání `AutoFormat` pomocí pojmenovaných a nepovinných argumentů, které jsou představeny v C# 4,0. Pojmenované a volitelné argumenty umožňují vynechat argument pro volitelný parametr, pokud nechcete změnit výchozí hodnotu parametru. V následujícím volání je hodnota zadána pouze pro jeden ze sedmi parametrů.  
  
 [!code-csharp[csProgGuideNamedAndOptional#13](~/samples/snippets/csharp/VS_Snippets_VBCSharp/csprogguidenamedandoptional/cs/namedandoptcom.cs#13)]  
  
 Další informace a příklady najdete v tématu [použití pojmenovaných a nepovinných argumentů v programování pro systém Office](./how-to-use-named-and-optional-arguments-in-office-programming.md) a [přístup k objektům Interop sady Office pomocí funkcí jazyka C#](../interop/how-to-access-office-onterop-objects.md).  
...
- [Jak použít pojmenované a nepovinné argumenty v programování pro sadu Office](./how-to-use-named-and-optional-arguments-in-office-programming.md)

...
V tomto příkladu `Month` je deklarován jako vlastnost, aby `set` přístupový objekt mohl zajistit, aby byla `Month` hodnota nastavena v rozmezí od 1 do 12. `Month`Vlastnost používá soukromé pole ke sledování skutečné hodnoty. Skutečné umístění dat vlastnosti se často označuje jako "záložní úložiště" vlastnosti. Vlastnosti pro použití privátních polí jako záložního úložiště je běžné. Pole je označeno jako soukromé, aby se zajistilo, že lze změnit pouze voláním vlastnosti. Další informace o omezeních veřejného a soukromého přístupu najdete v tématu [modifikátory přístupu](./access-modifiers.md).
...
`get`Přistupující objekt musí končit příkazem [return](../../language-reference/keywords/return.md) nebo [throw](../../language-reference/keywords/throw.md) a ovládací prvek nemůže přesměrovat tělo přistupujícího objektu.

Je to špatný styl programování ke změně stavu objektu pomocí `get` přístupového objektu. Například následující přistupující objekt vytváří vedlejší účinek změny stavu objektu při každém `_number` přístupu k poli.
...
Pokud v předchozím segmentu kódu nepřiřazujete hodnotu `Name` vlastnosti, vrátí hodnotu `NA` .

## <a name="the-set-accessor"></a>Přístupový objekt set

`set`Přistupující objekt se podobá metodě, jejíž návratový typ je [void](../../language-reference/builtin-types/void.md). Používá implicitní parametr `value` s názvem, jehož typ je typ vlastnosti. V následujícím příkladu `set` je přistupující objekt přidán do `Name` vlastnosti:
...
Použití implicitního názvu parametru, `value` pro deklaraci lokální proměnné v `set` přístupovém objektu, je chybné.

## <a name="remarks"></a>Poznámky

Vlastnosti mohou být označeny jako `public` , `private` , `protected` , `internal` `protected internal` nebo `private protected` . Tyto modifikátory přístupu definují, jak budou uživatelé třídy mít přístup k vlastnosti. `get` `set` Přístupové objekty a pro stejnou vlastnost mohou mít různé modifikátory přístupu. Například `get` může být `public` Povolení přístupu jen pro čtení zvenčí mimo typ a `set` může být `private` nebo `protected` . Další informace najdete v tématu [modifikátory přístupu](./access-modifiers.md).
...
- Vlastnost `Name` v odvozené třídě skrývá vlastnost `Name` v základní třídě. V takovém případě se `new` Modifikátor používá v deklaraci vlastnosti v odvozené třídě:

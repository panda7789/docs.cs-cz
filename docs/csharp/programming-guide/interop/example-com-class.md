...
- Události musí být deklarovány v rozhraní události.  
 ...
 Chcete-li vystavit vlastnosti a metody modelu COM, je nutné je deklarovat v rozhraní třídy a označit je atributem `DispId` a implementovat je ve třídě. Pořadí, ve kterém jsou členy deklarovány v rozhraní, je pořadí použité pro model COM vtable.  
  
 Chcete-li vystavit události z vaší třídy, je nutné je deklarovat v rozhraní události a označit je pomocí atributu `DispId`. Třída by neměla implementovat toto rozhraní.  

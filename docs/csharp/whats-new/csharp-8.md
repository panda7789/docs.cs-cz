...
C#8,0 se podporuje v **rozhraní .NET Core 3. x** a **.NET Standard 2,1**. Další informace najdete v tématu [ C# jazyková verze](../language-reference/configure-language-version.md).

Zbývající část tohoto článku stručně popisuje tyto funkce. Kde jsou k dispozici podrobné články, jsou uvedeny odkazy na tyto kurzy a přehledy. Tyto funkce můžete ve svém prostředí prozkoumat pomocí globálního nástroje `dotnet try`:
...
Modifikátor `readonly` lze použít pro členy struktury. Indikuje, že člen nemění stav. Je lépe podrobnější než použití modifikátoru `readonly` na deklaraci `struct`.  Vezměte v úvahu následující proměnlivou strukturu:
...
Často příkaz [`switch`](../language-reference/keywords/switch.md) vytváří hodnotu v každém z jeho `case`ch bloků. **Výrazy Switch** umožňují použít stručnější syntaxi výrazů. K dispozici je méně opakující se `case` a `break` klíčová slova a méně složených závorek.  Podívejte se například na následující výčet, který obsahuje seznam barev duha:
...
Pokud vaše aplikace definovala typ `RGBColor`, který je vytvořen z `R`, `G` a `B` komponenty, můžete převést `Rainbow` hodnotu na její hodnoty RGB pomocí následující metody, která obsahuje výraz Switch:
...
Některé typy obsahují metodu `Deconstruct`, která dekonstruuje své vlastnosti do diskrétních proměnných. Když je k dispozici metoda `Deconstruct`, můžete použít **poziční vzory** pro kontrolu vlastností objektu a použít tyto vlastnosti pro vzor.  Vezměte v úvahu následující `Point` třídu, která obsahuje metodu `Deconstruct` k vytvoření diskrétních proměnných pro `X` a `Y`:
...
Následující metoda používá **poziční vzor** k extrakci hodnot `x` a `y`. Potom používá klauzuli `when` k určení `Quadrant` bodu:
...
Nyní můžete přidat modifikátor `static` do místních funkcí, aby se zajistilo, že lokální funkce nebude zachytit (odkaz) jakékoli proměnné z ohraničujícího oboru. Tím dojde k vygenerování `CS8421`, "statická místní funkce nemůže obsahovat odkaz na \<proměnnou >." 
...
`struct` deklarovaný s modifikátorem `ref` nesmí implementovat žádná rozhraní a proto nemohou implementovat <xref:System.IDisposable>. Proto aby bylo možné vyřadit `ref struct`, musí mít přístupnou metodu `void Dispose()`. Tato funkce se vztahuje také na deklarace `readonly ref struct`.
...
Pojďme začít s pravidly pro indexy. Zvažte pole `sequence`. `0` index je stejný jako `sequence[0]`. `^0` index je stejný jako `sequence[sequence.Length]`. Všimněte si, že `sequence[^0]` vyvolá výjimku, stejně jako `sequence[sequence.Length]`. V případě libovolného čísla `n`index `^n` stejný jako `sequence.Length - n`.
...
C#8,0 zavádí `??=`operátor přiřazení s hodnotou null. Operátor `??=` lze použít k přiřazení hodnoty jeho pravého operandu k levému operandu pouze v případě, že je operand na levé straně vyhodnocen jako `null`.
...
Například s ohledem na následující definici typu Obecné `Coords<T>`:
...
Pořadí `$` a `@` tokenů v [interpolované](../language-reference/tokens/interpolated.md) doslovném řetězci může být libovolné: `$@"..."` i `@$"..."` jsou platné interpolované řetězce. V dřívějších C# verzích musí být token `$` před tokenem `@`.

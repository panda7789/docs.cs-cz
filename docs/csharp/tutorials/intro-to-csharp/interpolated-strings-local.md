...
V tomto kurzu očekává, že máte počítač, který používáte pro vývoj. Téma .NET [zahájení práce během 10 minut](https://www.microsoft.com/net/core) obsahuje pokyny pro nastavení místního vývojového prostředí v Mac, PC nebo Linux. Je rychlý přehled toho, příkazy, které použijete v [Úvod do místní kurzy](local-environment.md), s odkazy na další podrobnosti. Můžete také použít [interaktivní verze](interpolated-strings.yml) tohoto kurzu v prohlížeči.
...
Můžete pokračovat [seznamu kolekce](arrays-and-collections.md) kurz ve svém vlastním vývojovém prostředí.

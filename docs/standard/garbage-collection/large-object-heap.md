...
Uvolňování paměti .NET (GC) rozděluje objekty až do malých a velkých objektů. Když je objekt rozsáhlý, některé z jeho atributů se stanou důležitější, než když je objekt malý. Například jeho komprimací, &mdash; který znamená, je jeho zkopírování do paměti jinde na haldě &mdash; může být nákladné. Z tohoto důvodu systém uvolňování paměti umístí velké objekty na haldu velkých objektů (LOH). Tento článek pojednává o tom, co má objekt jako velký objekt, jak se shromažďují velké objekty a jaké typy dopadů na výkon mají velké objekty.

> [!IMPORTANT]
> Tento článek popisuje haldu velkých objektů v .NET Framework a .NET Core běžící pouze na systémech Windows. Nepokrývá LOH běžící na implementacích .NET na jiných platformách.
...
Obecně platí, že GC vychází z jedné z následujících tří podmínek:
...
- <xref:System.GC.Collect%2A?displayProperty=nameWithType>Metoda je volána.
  ...
  Pokud <xref:System.GC.Collect?displayProperty=nameWithType> je volána metoda bez parametrů nebo je jako argument předána jiná přetížená metoda <xref:System.GC.MaxGeneration?displayProperty=nameWithType> , je LOH shromážděn spolu se zbytkem spravované haldy.
...
## <a name="collect-performance-data-for-the-loh"></a>Shromažďování údajů o výkonu pro LOH

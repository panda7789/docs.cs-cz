...
Podobně jako třída pozitivního znaku, znak `|` lze použít k porovnávání kteréhokoli z několika jednoduchých znaků. V následujícím příkladu je použita třída pozitivního znaku a buď porovnávání se vzorem, nebo pomocí znaku `|` pro vyhledání výskytů slov "Gray" nebo "šedá" v řetězci. V tomto případě znak `|` vytvoří regulární výraz, který je podrobnější.
...
|<code>(a&#124;e)</code>|Porovnává buď se znakem "a", nebo "e".|  
|`y\b`|Porovnává s "y" na hranici slova.|  

Znak `|` lze také použít k provedení shody s více znaky nebo podvýrazy, které mohou obsahovat libovolnou kombinaci znakových literálů a prvků jazyka regulárních výrazů. (Třída znaků tuto funkci neposkytuje.) V následujícím příkladu se pomocí znaku `|` extrahuje buď číslo sociálního pojištění (rodné číslo), což je 9 číslic, ve formátu *ddd*-*DD*-*dddd*, nebo v USA (EIN) identifikační číslo (), což je 9 číslic čísla ve formátu *dd*-*ddddddd*.
...
|`\d`|Ukončí porovnávání na hranici slova.|  
...
|`\d{7}`|Pokud se shodovala hodnota `n2`, porovná sedm desítkových číslic.|  
|<code>&#124;\d{3}-\d{2}-\d{4}</code>|Pokud se neshoduje `n2`, porovná tři desítkové číslice, pomlčku, dvě desítkové číslice, další spojovník a čtyři desítkové číslice.|  
|`\b`|Odpovídá hranici slova.|  

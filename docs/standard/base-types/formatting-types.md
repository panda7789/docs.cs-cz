...
Standardní formátovací řetězce pro typy výčtu přímo ovládají řetězcovou reprezentaci hodnoty. Formátovací řetězce předané metodě `ToString` hodnoty výčtu určují, zda je hodnota zobrazena pomocí názvu řetězce (specifikátory formátu "G" a "F"), její základní celočíselné hodnoty (specifikátor formátu "D") nebo její hexadecimální hodnota (specifikátor formátu "X"). Následující příklad znázorňuje použití standardního formátovacího řetězce k formátování <xref:System.DayOfWeek> hodnoty výčtu.

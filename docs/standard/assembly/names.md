...
## <a name="name-application-components"></a>Pojmenovat součásti aplikace  
 ...
 Například pokud máte sestavení s názvem *MyAssembly. exe* , které odkazuje na sestavení s názvem *MyAssembly. dll*, vazba proběhne správně, pokud spustíte *MyAssembly. exe*. Pokud však jiná aplikace provádí *MyAssembly. exe* <xref:System.AppDomain.ExecuteAssembly%2A?displayProperty=nameWithType>pomocí metody, modul runtime určí, že je `myAssembly` již načten, `myAssembly`když aplikace *MyAssembly. exe* vyžádá vazbu. V tomto případě není *MyAssembly. dll* nikdy načtena. Protože *MyAssembly. exe* neobsahuje požadovaný typ, <xref:System.TypeLoadException> dojde k chybě.  

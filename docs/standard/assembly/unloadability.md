...
- Běžné odkazy držené mimo kolekční `AssemblyLoadContext` uložené v zásobníku zásobníku nebo v registru procesoru (místní metody, buď explicitně vytvořené kódem uživatele nebo implicitně pomocí kompilátoru JIT (just-in-time)), statické proměnné nebo silného ( Připnutí): popisovač GC a průjezdný přechod na:
...
Tento příkaz vypíše všechny objekty s názvem typu, který obsahuje `LoaderAllocator` v haldě GC. Tady je příklad:

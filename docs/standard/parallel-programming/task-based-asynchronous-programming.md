...
Můžete také použít metodu <xref:System.Threading.Tasks.TaskFactory.StartNew%2A?displayProperty=nameWithType> k vytvoření a spuštění úlohy v jedné operaci. Tuto metodu použijte, když vytváření a plánování nemusí být odděleno a požadujete další možnosti vytvoření úlohy nebo použití konkrétního plánovače, nebo pokud potřebujete předat další stav do úlohy, kterou můžete načíst prostřednictvím její <xref:System.Threading.Tasks.Task.AsyncState%2A?displayProperty=nameWithType>ové vlastnosti. , jak je znázorněno v následujícím příkladu.

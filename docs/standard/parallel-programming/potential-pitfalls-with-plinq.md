...
Pomocí metody `AsParallel` se účtují režijní náklady na rozdělení zdrojové kolekce a synchronizaci pracovních vláken. Výhody paralelismu jsou dále omezeny počtem procesorů v počítači. Neexistují žádné zrychlení, které by bylo možné vymezit spuštěním více výpočetních vláken vázaných na pouze jeden procesor. Proto musíte mít pozor, abyste paralelizovat dotaz.
...
Stejný problém se týká <xref:System.Threading.Tasks.Parallel.ForEach%2A?displayProperty=nameWithType>. Jinými slovy, `source.AsParallel().Where().ForAll(...)` by měla být důrazně upřednostňována `Parallel.ForEach(source.AsParallel().Where(), ...)`.

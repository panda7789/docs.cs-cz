...
---
# <a name="plinq-data-sample"></a><span data-ttu-id="355fb-102">Ukázková data pro PLINQ</span><span class="sxs-lookup"><span data-stu-id="355fb-102">PLINQ Data Sample</span></span>
<span data-ttu-id="355fb-103">Tato ukázka obsahuje ukázková data ve formátu .csv, společně s metodami, které transformují do kolekce v paměti zákazníky, produkty, objednávek a podrobnostmi o objednávce.</span><span class="sxs-lookup"><span data-stu-id="355fb-103">This sample contains example data in .csv format, together with methods that transform it into in-memory collections of Customers, Products, Orders, and Order Details.</span></span> <span data-ttu-id="355fb-104">Pro další pokusy pomocí jazyka PLINQ můžete vložit příklady kódů z některých dalších témat, která do kódu v tomto tématu a vyvolat z `Main` metody.</span><span class="sxs-lookup"><span data-stu-id="355fb-104">To further experiment with PLINQ, you can paste code examples from certain other topics into the code in this topic and invoke it from the `Main` method.</span></span> <span data-ttu-id="355fb-105">Tato data můžete také použít s vlastními dotazy PLINQ.</span><span class="sxs-lookup"><span data-stu-id="355fb-105">You can also use this data with your own PLINQ queries.</span></span>  
  
 <span data-ttu-id="355fb-106">Data představují podmnožinu databázi Northwind.</span><span class="sxs-lookup"><span data-stu-id="355fb-106">The data represents a subset of the Northwind database.</span></span> <span data-ttu-id="355fb-107">Záznamy o zákaznících padesát (50) jsou zahrnuty, ale ne všechna pole.</span><span class="sxs-lookup"><span data-stu-id="355fb-107">Fifty (50) customer records are included, but not all fields.</span></span> <span data-ttu-id="355fb-108">Součástí je podmnožinou řádků z objednávky a odpovídající data z Order_Detail pro každý zákazník.</span><span class="sxs-lookup"><span data-stu-id="355fb-108">A subset of the rows from the Orders and corresponding Order_Detail data for every Customer is included.</span></span> <span data-ttu-id="355fb-109">Všechny produkty jsou zahrnuty.</span><span class="sxs-lookup"><span data-stu-id="355fb-109">All Products are included.</span></span>  
  
> [!NOTE]
>  <span data-ttu-id="355fb-110">Datové sady není dostatečně velký, aby prokázat, že PLINQ je rychlejší než LINQ to Objects dotazů, které obsahují pouze základní `where` a `select` klauzule.</span><span class="sxs-lookup"><span data-stu-id="355fb-110">The data set is not large enough to demonstrate that PLINQ is faster than LINQ to Objects for queries that contain just basic `where` and `select` clauses.</span></span> <span data-ttu-id="355fb-111">Sledovat, zvyšuje rychlost pro malé datové sady takovou situaci, pomocí dotazů, které obsahují výpočetně náročné operace na každý prvek v datové sadě.</span><span class="sxs-lookup"><span data-stu-id="355fb-111">To observe speed increases for small data sets such as this, use queries that contain computationally expensive operations on every element in the data set.</span></span>  
  
### <a name="to-set-up-this-sample"></a><span data-ttu-id="355fb-112">Nastavit tuto ukázku</span><span class="sxs-lookup"><span data-stu-id="355fb-112">To set up this sample</span></span>  
  
1. <span data-ttu-id="355fb-113">Vytvoření projektu konzolové aplikace jazyka Visual Basic nebo Visual C#.</span><span class="sxs-lookup"><span data-stu-id="355fb-113">Create a Visual Basic or Visual C# console application project.</span></span>  
  
2. <span data-ttu-id="355fb-114">Nahraďte obsah Module1.vb nebo souboru Program.cs kódem, který zahrnuje následující kroky.</span><span class="sxs-lookup"><span data-stu-id="355fb-114">Replace the contents of Module1.vb or Program.cs by using the code that follows these steps.</span></span>  
  
3. <span data-ttu-id="355fb-115">Na **projektu** nabídky, klikněte na tlačítko **přidat novou položku**.</span><span class="sxs-lookup"><span data-stu-id="355fb-115">On the **Project** menu, click **Add New Item**.</span></span> <span data-ttu-id="355fb-116">Vyberte **textový soubor** a potom klikněte na tlačítko **OK**.</span><span class="sxs-lookup"><span data-stu-id="355fb-116">Select **Text File** and then click **OK**.</span></span> <span data-ttu-id="355fb-117">Zkopírujte data v tomto tématu a následně jej vložte do nového textového souboru.</span><span class="sxs-lookup"><span data-stu-id="355fb-117">Copy the data in this topic and then paste it in the new text file.</span></span> <span data-ttu-id="355fb-118">Na **souboru** nabídky, klikněte na tlačítko **Uložit**, pojmenujte soubor Plinqdata.csv a uložte ho do složky, která obsahuje soubory zdrojového kódu.</span><span class="sxs-lookup"><span data-stu-id="355fb-118">On the **File** menu, click **Save**, name the file Plinqdata.csv, and then save it in the folder that contains your source code files.</span></span>  
  
4. <span data-ttu-id="355fb-119">Stiskněte klávesu F5, chcete-li ověřit, že projekt vytvoří a spustí správně.</span><span class="sxs-lookup"><span data-stu-id="355fb-119">Press F5 to verify that the project builds and runs correctly.</span></span> <span data-ttu-id="355fb-120">V okně konzoly byste zobrazí následující výstup.</span><span class="sxs-lookup"><span data-stu-id="355fb-120">The following output should be displayed in the console window.</span></span>  
...
## <a name="data"></a><span data-ttu-id="355fb-121">Data</span><span class="sxs-lookup"><span data-stu-id="355fb-121">Data</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="355fb-122">Viz také:</span><span class="sxs-lookup"><span data-stu-id="355fb-122">See also</span></span>

- [<span data-ttu-id="355fb-123">Paralelní LINQ (PLINQ)</span><span class="sxs-lookup"><span data-stu-id="355fb-123">Parallel LINQ (PLINQ)</span></span>](../../../docs/standard/parallel-programming/parallel-linq-plinq.md)

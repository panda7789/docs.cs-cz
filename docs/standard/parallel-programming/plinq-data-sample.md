...
---
# <a name="plinq-data-sample"></a><span data-ttu-id="060a2-102">Ukázková data pro PLINQ</span><span class="sxs-lookup"><span data-stu-id="060a2-102">PLINQ Data Sample</span></span>
<span data-ttu-id="060a2-103">Tato ukázka obsahuje ukázková data ve formátu .csv spolu s metodami, které je transformují do kolekcí zákazníků, produktů, objednávek a podrobností objednávky v paměti.</span><span class="sxs-lookup"><span data-stu-id="060a2-103">This sample contains example data in .csv format, together with methods that transform it into in-memory collections of Customers, Products, Orders, and Order Details.</span></span> <span data-ttu-id="060a2-104">Chcete-li dále experimentovat s PLINQ, můžete vložit příklady kódu z některých `Main` jiných témat do kódu v tomto tématu a vyvolat jej z metody.</span><span class="sxs-lookup"><span data-stu-id="060a2-104">To further experiment with PLINQ, you can paste code examples from certain other topics into the code in this topic and invoke it from the `Main` method.</span></span> <span data-ttu-id="060a2-105">Tato data můžete také použít s vlastními dotazy PLINQ.</span><span class="sxs-lookup"><span data-stu-id="060a2-105">You can also use this data with your own PLINQ queries.</span></span>  
  
 <span data-ttu-id="060a2-106">Data představují podmnožinu databáze Northwind.</span><span class="sxs-lookup"><span data-stu-id="060a2-106">The data represents a subset of the Northwind database.</span></span> <span data-ttu-id="060a2-107">Padesát (50) záznamů zákazníků jsou zahrnuty, ale ne všechna pole.</span><span class="sxs-lookup"><span data-stu-id="060a2-107">Fifty (50) customer records are included, but not all fields.</span></span> <span data-ttu-id="060a2-108">Je zahrnuta podmnožina řádků z objednávek a odpovídajících Order_Detail dat pro každého zákazníka.</span><span class="sxs-lookup"><span data-stu-id="060a2-108">A subset of the rows from the Orders and corresponding Order_Detail data for every Customer is included.</span></span> <span data-ttu-id="060a2-109">Všechny produkty jsou zahrnuty.</span><span class="sxs-lookup"><span data-stu-id="060a2-109">All Products are included.</span></span>  
  
> [!NOTE]
> <span data-ttu-id="060a2-110">Sada dat není dostatečně velký, aby prokázal, že PLINQ je rychlejší než `where` `select` LINQ na objekty pro dotazy, které obsahují pouze základní a klauzule.</span><span class="sxs-lookup"><span data-stu-id="060a2-110">The data set is not large enough to demonstrate that PLINQ is faster than LINQ to Objects for queries that contain just basic `where` and `select` clauses.</span></span> <span data-ttu-id="060a2-111">Chcete-li sledovat zvýšení rychlosti pro malé datové sady, jako je tento, použijte dotazy, které obsahují výpočtově nákladné operace na každý prvek v sadě dat.</span><span class="sxs-lookup"><span data-stu-id="060a2-111">To observe speed increases for small data sets such as this, use queries that contain computationally expensive operations on every element in the data set.</span></span>  
  
### <a name="to-set-up-this-sample"></a><span data-ttu-id="060a2-112">Chcete-li nastavit tuto ukázku</span><span class="sxs-lookup"><span data-stu-id="060a2-112">To set up this sample</span></span>  
  
1. <span data-ttu-id="060a2-113">Vytvořte projekt aplikace konzoly Visual Basic nebo Visual C#.</span><span class="sxs-lookup"><span data-stu-id="060a2-113">Create a Visual Basic or Visual C# console application project.</span></span>  
  
2. <span data-ttu-id="060a2-114">Nahraďte obsah Module1.vb nebo Program.cs pomocí kódu, který následuje po těchto krocích.</span><span class="sxs-lookup"><span data-stu-id="060a2-114">Replace the contents of Module1.vb or Program.cs by using the code that follows these steps.</span></span>  
  
3. <span data-ttu-id="060a2-115">V nabídce **Project** klikněte na **Add New Item**.</span><span class="sxs-lookup"><span data-stu-id="060a2-115">On the **Project** menu, click **Add New Item**.</span></span> <span data-ttu-id="060a2-116">Vyberte **Textový soubor** a klepněte na **tlačítko OK**.</span><span class="sxs-lookup"><span data-stu-id="060a2-116">Select **Text File** and then click **OK**.</span></span> <span data-ttu-id="060a2-117">Zkopírujte data v tomto tématu a vložte je do nového textového souboru.</span><span class="sxs-lookup"><span data-stu-id="060a2-117">Copy the data in this topic and then paste it in the new text file.</span></span> <span data-ttu-id="060a2-118">V nabídce **Soubor** klepněte na tlačítko **Uložit**, pojmenujte soubor Plinqdata.csv a uložte jej do složky, která obsahuje soubory zdrojového kódu.</span><span class="sxs-lookup"><span data-stu-id="060a2-118">On the **File** menu, click **Save**, name the file Plinqdata.csv, and then save it in the folder that contains your source code files.</span></span>  
  
4. <span data-ttu-id="060a2-119">Stisknutím klávesy F5 ověřte, zda se projekt sestavuje a spouští správně.</span><span class="sxs-lookup"><span data-stu-id="060a2-119">Press F5 to verify that the project builds and runs correctly.</span></span> <span data-ttu-id="060a2-120">V okně konzoly by měl být zobrazen následující výstup.</span><span class="sxs-lookup"><span data-stu-id="060a2-120">The following output should be displayed in the console window.</span></span>  
...
## <a name="data"></a><span data-ttu-id="060a2-121">Data</span><span class="sxs-lookup"><span data-stu-id="060a2-121">Data</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="060a2-122">Viz také</span><span class="sxs-lookup"><span data-stu-id="060a2-122">See also</span></span>

- [<span data-ttu-id="060a2-123">Paralelní LINQ (PLINQ)</span><span class="sxs-lookup"><span data-stu-id="060a2-123">Parallel LINQ (PLINQ)</span></span>](../../../docs/standard/parallel-programming/parallel-linq-plinq.md)

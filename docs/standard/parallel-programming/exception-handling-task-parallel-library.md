...
ms.locfileid: "55255221"
---
# <a name="exception-handling-task-parallel-library"></a>Zpracování výjimek (Task Parallel Library)

Neošetřené výjimky, které jsou vyvolány uživatelským kódem spuštěným uvnitř úkolu jsou šířeny zpět do volajícího vlákna, s výjimkou zvláštních situací, které jsou popsány dále v tomto tématu. Výjimky jsou šířeny, pokud použijete jednu ze statických nebo instance <xref:System.Threading.Tasks.Task.Wait%2A?displayProperty=nameWithType> metody a zpracujete je uzavřením volání `try` / `catch` příkazu. Pokud je úloha nadřazené připojených podřízených úloh, nebo pokud čekáte na více úkolů, může být vyvoláno více výjimek.
...
Pokud nechcete k volání <xref:System.Threading.Tasks.Task.Wait%2A?displayProperty=nameWithType> metoda počkat na dokončení úkolu, můžete také načíst <xref:System.AggregateException> výjimku z úkolu <xref:System.Threading.Tasks.Task.Exception%2A> vlastnost, jak ukazuje následující příklad. Další informace najdete v tématu [pozorování výjimek pomocí vlastnosti Task.Exception](#observing-exceptions-by-using-the-taskexception-property) v tomto tématu.

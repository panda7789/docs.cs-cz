...
Používejte `try` / `catch` bloky kolem kódu, který může potenciálně generovat výjimku ***a*** váš kód může z této výjimky obnovit. V `catch` blocích vždy seřazení výjimek z největší odvozené na nejméně odvozené. Všechny výjimky jsou odvozeny z <xref:System.Exception> . Další odvozené výjimky nejsou zpracovány klauzulí catch, která předchází klauzuli catch pro základní třídu výjimky. Když se váš kód nemůže zotavit z výjimky, nezachyťte tuto výjimku. Pokud je to možné, povolte metody další v zásobníku volání pro obnovení.

Vyčistěte prostředky přidělené buď pomocí `using` příkazů, nebo `finally` bloků. Preferovat `using` příkazy k automatickému vyčištění prostředků, když jsou vyvolány výjimky. Pomocí `finally` bloků vyčistěte prostředky, které neimplementují <xref:System.IDisposable> . Kód v `finally` klauzuli je téměř vždy spouštěn i v případě, že jsou výjimky vyvolány.
...
Pokud před zavřením nezkontrolujete stav připojení, můžete výjimku zachytit `InvalidOperationException` .
...
Pro typy hodnot, zda se má použít `Nullable<T>` nebo jako výchozí používat jako indikátor chyb, je třeba zvážit konkrétní aplikaci. Pomocí `Nullable<Guid>` , `default` se `null` místo `Guid.Empty` . V některých případech se `Nullable<T>` dá přidat, takže pokud je hodnota přítomná nebo chybí, může to být jasné. Jinak přidávání `Nullable<T>` může vytvořit další případy, které kontrolují, že nepotřebujete, a sloužit jenom k vytváření potenciálních zdrojů chyb.
...
- Vyvolejte <xref:System.ArgumentException> výjimku nebo jednu z předdefinovaných tříd, které jsou odvozeny z, <xref:System.ArgumentException> Pokud jsou předány neplatné parametry.
...
Chybová zpráva, kterou uživatel vidí, je odvozena z <xref:System.Exception.Message?displayProperty=nameWithType> Vlastnosti výjimky, která byla vyvolána, a nikoli z názvu třídy Exception. Obvykle přiřadíte hodnotu k <xref:System.Exception.Message?displayProperty=nameWithType> Vlastnosti předáním řetězce zprávy `message` argumentu [konstruktoru výjimky](xref:System.Exception.%23ctor%2A).
...
Poskytněte další vlastnosti pro výjimku (kromě vlastního řetězce zprávy) pouze v případě, že existuje programový scénář, kde jsou další informace užitečné. Například <xref:System.IO.FileNotFoundException> poskytuje <xref:System.IO.FileNotFoundException.FileName> vlastnost.
...
Tento příklad znázorňuje použití `throw` pro opětovné vyvolání původní výjimky, která může usnadnit volajícím zobrazit skutečnou příčinu problému bez nutnosti prozkoumávat <xref:System.Exception.InnerException> vlastnost. Alternativou je vyvolat novou výjimku a zahrnout původní výjimku jako vnitřní výjimku:

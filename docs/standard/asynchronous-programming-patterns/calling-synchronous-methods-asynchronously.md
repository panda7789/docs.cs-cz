...
> [!NOTE]
>  Funkce technologie IntelliSense v [!INCLUDE[vsprvslong](../../../includes/vsprvslong-md.md)] zobrazí parametry `BeginInvoke` a `EndInvoke`. Pokud nepoužíváte Visual Studio nebo něco podobného, nebo pokud používáte C# s [!INCLUDE[vsprvslong](../../../includes/vsprvslong-md.md)], naleznete v tématu [asynchronního programovacího modelu (APM)](../../../docs/standard/asynchronous-programming-patterns/asynchronous-programming-model-apm.md) popis o parametrech definovaných pro tyto metody.  

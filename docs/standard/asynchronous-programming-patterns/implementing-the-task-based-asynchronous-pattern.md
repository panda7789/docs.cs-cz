...
## <a name="generating-tap-methods"></a>Generování klepněte na metody

### <a name="using-the-compilers"></a>Pomocí kompilátory
Počínaje [!INCLUDE[net_v45](../../../includes/net-v45-md.md)], libovolné metody s atributem `async` – klíčové slovo (`Async` v jazyce Visual Basic) je považován za asynchronní metodu a kompilátory jazyka C# a Visual Basic provést transformace nezbytné k implementaci Metoda asynchronně pomocí klepnutím. Asynchronní metoda by měla vrátit buď objekt <xref:System.Threading.Tasks.Task?displayProperty=nameWithType>, nebo objekt <xref:System.Threading.Tasks.Task%601?displayProperty=nameWithType>. K tomu by měla vrátit tělo funkce `TResult`, a kompilátor zajistí, že tento výsledek je k dispozici prostřednictvím výsledný objekt úlohy. Stejně tak veškeré výjimky, které nejsou zpracovány v těle metody, jsou zařazeny do výstupního úkolu, což způsobí, že výsledný úkol bude ukončen ve stavu <xref:System.Threading.Tasks.TaskStatus.Faulted?displayProperty=nameWithType>. Výjimkou je situace, kdy není zpracována výjimka typu <xref:System.OperationCanceledException> (nebo odvozeného typu). V takovém případě skončí výsledný úkol ve stavu <xref:System.Threading.Tasks.TaskStatus.Canceled?displayProperty=nameWithType>.

### <a name="generating-tap-methods-manually"></a>Klepněte na metody generování ručně
...
Výpočetní úlohy můžete vygenerovat následujícími způsoby:
...
- V [!INCLUDE[net_v45](../../../includes/net-v45-md.md)] a novější verze (včetně .NET Core a .NET Standard), použijte statickou <xref:System.Threading.Tasks.Task.Run%2A?displayProperty=nameWithType> jako zástupce metodu <xref:System.Threading.Tasks.TaskFactory.StartNew%2A?displayProperty=nameWithType>. Metodu <xref:System.Threading.Tasks.Task.Run%2A> můžete použít pro snadné spouštění výpočetního úkolu, který se zaměřuje na fond vláken. V [!INCLUDE[net_v45](../../../includes/net-v45-md.md)] a novější verze, toto je upřednostňovaný postup spouštění výpočetních úloh. Použití `StartNew` přímo jenom když chcete, jemně odstupňovanou kontrolu nad úlohu.
...
- Použití <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAll%2A?displayProperty=nameWithType> a <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAny%2A?displayProperty=nameWithType> metody. Tyto metody vytvoří nový úkol, který je naplánován na dobu, kdy skončí všechny nebo libovolná zadaná sada úkolů. Tyto metody také poskytují přetížení k ovládání plánování a provádění těchto úkolů.
...
### <a name="io-bound-tasks"></a>I/čítači úlohy
...
### <a name="mixed-compute-bound-and-io-bound-tasks"></a>Smíšená výpočetních a I/čítači úlohy
...
Tento příklad také znázorňuje, jakým způsobem lze jeden token zrušení zřetězit prostřednictvím několika asynchronních operací. Další informace najdete v části zrušení využití v [použití asynchronního vzoru založeného na úloze](../../../docs/standard/asynchronous-programming-patterns/consuming-the-task-based-asynchronous-pattern.md).
...
## <a name="see-also"></a>Viz také
 ...
 [Použití asynchronního vzoru založeného na úloze](../../../docs/standard/asynchronous-programming-patterns/consuming-the-task-based-asynchronous-pattern.md)  

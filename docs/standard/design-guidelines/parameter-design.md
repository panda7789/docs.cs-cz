 ...
 **✓ DO** ověřte argumenty předaný veřejný, chráněný nebo – explicitně implementovaná členy. Throw <xref:System.ArgumentException?displayProperty=nameWithType>, nebo jeden z jejích podtříd, pokud se ověření nezdaří.  
 ...
 **✓ CONSIDER** přidávání params – klíčové slovo do pole parametry, pokud očekáváte, koncovým uživatelům předat pole s malý počet elementů. Pokud je očekáváno, že velký počet elementů předány společné scénáře, uživatelé budou pravděpodobně není předejte tyto elementy vložené přesto a proto není nutné params – klíčové slovo.  

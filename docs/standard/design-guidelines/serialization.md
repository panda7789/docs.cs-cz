 ...
 **✓ CONSIDER** podporuje serializace Runtime v případě, že vaše typy budou použity s .NET Remoting. Například obor názvů <xref:System.AddIn?displayProperty=nameWithType> používá vzdálenou komunikaci rozhraní .NET a všechny typy vyměňované mezi `System.AddIn` doplňky musí podporovat serializaci modulu runtime.  

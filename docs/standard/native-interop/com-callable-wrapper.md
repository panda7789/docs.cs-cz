...
Hodnota **ClassInterfaceType. None** zabraňuje tomu, aby se rozhraní třídy vygenerovalo, když se metadata třídy exportují do knihovny typů. V předchozím příkladu můžou klienti modelu COM přistupovat ke `LoanApp` třídě pouze prostřednictvím `IExplicit` rozhraní.

...
Aplikace nebo knihovna může také cílit na konkrétní implementaci rozhraní .NET, aby získala přístup k rozhraním API specifickým pro implementaci. Například aplikace, která se zaměřuje na Xamarin. iOS (například), `Xamarin.iOS10` získá přístup k obálkám rozhraní API iOS v systému Xamarin pro iOS 10 nebo aplikace, která cílí na Univerzální platforma Windows (UWP), `uap10.0` má přístup k rozhraním API, která se zkompiluje pro zařízení s Windows 10.
...
| :-------------------: | :-------------------------: | :----------------------------: | :-------------------------------------: |
| .NET Standard         | 2.1                         | netstandard 2.1                 | –                                     |
...
Následující soubor projektu knihovny cílí na rozhraní API .NET Standard ( `netstandard1.4` ) a rozhraní api .NET Framework ( `net40` a `net45` ). Použijte element plural **targetframeworks** s více cílovými rozhraními. Všimněte si, jak `Condition` atributy zahrnují balíčky specifické pro implementaci při kompilaci knihovny pro dva .NET Framework TFM:

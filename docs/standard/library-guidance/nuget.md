...
| `Title`                            | `title`                    | Popisný název balíčku. Výchozí hodnota je `PackageId` .             |
...
| `PackageIcon`                   | `icon`                  | Cesta k obrázku v balíčku, který se má použít jako ikona balíčku Přečtěte si další informace o [ `icon` metadatech](/nuget/reference/nuspec#icon). |
| `PackageProjectUrl`                | `projectUrl`               | Adresa URL domovské stránky projektu nebo zdrojového úložiště.             |
| `PackageLicenseExpression`         | `license`                  | [Identifikátor SPDX](https://spdx.org/licenses/)licence projektu. Identifikátor můžou používat jenom licence OSI a FSF schválené. Ostatní licence by se měly používat `PackageLicenseFile` . Přečtěte si další informace o [ `license` metadatech](/nuget/reference/nuspec#license). |
...
> Odkaz na zdroj automaticky přidá `RepositoryUrl` a `RepositoryType` metadata do balíčku NuGet. Odkaz na zdroj také přidá informace o přesném zdrojovém kódu, ze kterého byl balíček sestaven. Například balíček vytvořený z úložiště Git bude obsahovat hodnotu hash potvrzení přidané jako metadata.

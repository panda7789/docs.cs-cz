...
XML model DOM (Document Object Model) (DOM) je plně podporující obor názvů. Jsou podporovány pouze dokumenty XML zohledňující obor názvů. Konsorcium World Wide Web (W3C) určuje, že aplikace modelu DOM, které implementují úroveň 1, můžou být nezohledňované a funkce DOM úrovně 2 se s oborem názvů nepodporují. Všechny funkce v modelu XML DOM však podporují obor názvů bez ohledu na to, zda je metoda z doporučení úrovně 1 nebo 2 DOM.  
 ...
 V prostředí, které podporuje obor názvů, volání metody DOM úrovně 2 `setAttribute("A:b", "123")` má za následek atribut s předponou `A` a místním názvem. `b` Tímto způsobem funguje Microsoft .NET Framework DOM.  
  
 Proto pro všechny metody, které přijímají parametr Name, tyto metody také převezmou předponu, která bude kvalifikována název. Parametr Name, jako je `A:b` například v metodě **SetAttributes** DOM úrovně 1, je analyzován následujícím způsobem:  

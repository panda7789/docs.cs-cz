...
Objekty jsou platné pouze v doméně aplikace, kde jsou vytvořeny. Jakýkoliv pokus o předání objektu jako parametru nebo jeho vrácení jako výsledek selže, pokud je objekt odvozen od `MarshalByRefObject` nebo je označen jako `Serializable`. Pokud je objekt označen jako `Serializable`, objekt bude automaticky serializován, přepravován z jedné aplikační domény do druhé a poté deserializovat, aby vytvořila přesnou kopii objektu v druhé aplikační doméně. Tento proces je obvykle označován jako zařazování podle hodnot.

Pokud je objekt odvozen z `MarshalByRefObject`, odkaz na objekt je předán z jedné aplikační domény do druhé, nikoli samotného objektu. Můžete také označit objekt, který je odvozen z `MarshalByRefObject` jako `Serializable`. Pokud se tento objekt používá se vzdáleným přístupem, formátovací modul zodpovědný za serializaci, která byla předem nakonfigurované s náhradním selektorm (`SurrogateSelector`), přebírá řízení procesu serializace a nahrazuje všechny objekty odvozené z `MarshalByRefObject` pomocí proxy serveru. Bez `SurrogateSelector` se architektura serializace řídí standardními pravidly serializace popsanými v [krocích procesu serializace](steps-in-the-serialization-process.md).  

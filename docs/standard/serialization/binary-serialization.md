...
Binární serializace umožňuje upravovat soukromé členy uvnitř objektu, a proto mění stav. Z tohoto důvodu jsou doporučeny jiné architektury serializace, jako je například <xref:System.Text.Json?displayProperty=fullName>, které fungují na veřejné ploše rozhraní API.
...
> | <xref:System.Data.DataTable?displayProperty=nameWithType> | Pokud nastavíte `RemotingFormat` na `SerializationFormat.Binary`, můžete ho vyměňovat jenom pomocí .NET Core 2,1 a novějších verzí. |

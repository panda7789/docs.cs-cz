...
---
# <a name="managed-threading-best-practices"></a>Osvědčené postupy spravovaného podprocesu
...
> [!NOTE]
> Počínaje rozhraním .NET Framework 4 poskytují paralelní knihovna úloh a PLINQ rozhraní API, která snižují některé složitosti a rizika programování s více vlákny. Další informace naleznete [v tématu Paralelní programování v rozhraní .NET](../../../docs/standard/parallel-programming/index.md).  
  
## <a name="deadlocks-and-race-conditions"></a>Zablokování a sporičné podmínky  
...
### <a name="deadlocks"></a>Zablokování  
 ...
 Mnoho metod tříd modelu spravovaných vláken poskytuje časové limity, které usnadňují zjišťování případů zablokování. Například následující kód se pokusí získat zámek na `lockObject`objekt s názvem . Pokud zámek není získán v 300 <xref:System.Threading.Monitor.TryEnter%2A?displayProperty=nameWithType> milisekundách, vrátí `false`.  
...
### <a name="race-conditions"></a>Podmínky závodu  
 ...
 Jednoduchým příkladem konfliktu časování je zvyšování pole. Předpokládejme, že třída má soukromé **statické** pole **(Sdílené** v jazyce Visual Basic), které se zintálí při každém vytvoření instance třídy pomocí kódu jako `objCt++;` (C#) nebo `objCt += 1` (Visual Basic). Tato operace vyžaduje načtení hodnoty z kódu `objCt` do registru, navýšení hodnoty a její uložení v kódu `objCt`.  
 ...
 Těmto konkrétním konfliktům časování se dá snadno vyhnout použitím metod třídy <xref:System.Threading.Interlocked>, jako například <xref:System.Threading.Interlocked.Increment%2A?displayProperty=nameWithType>. Další informace o dalších technikách synchronizace dat mezi více vlákny naleznete v [tématu Synchronizace dat pro vícevláknové .](../../../docs/standard/threading/synchronizing-data-for-multithreading.md)  
...
## <a name="number-of-processors"></a>Počet zpracovatelů

Zda existuje více procesorů nebo pouze jeden procesor k dispozici v systému může ovlivnit vícevláknové architektury. Další informace naleznete v [tématu Počet procesorů](https://docs.microsoft.com/previous-versions/dotnet/netframework-1.1/1c9txz50(v%3dvs.71)#number-of-processors).

Pomocí <xref:System.Environment.ProcessorCount?displayProperty=nameWithType> této vlastnosti určete počet procesorů dostupných za běhu.
  
## <a name="general-recommendations"></a>Obecná doporučení  
 Při používání více vláken zvažte následující pokyny:  
  
- Pro ukončení ostatních vláken nepoužívejte vlákno <xref:System.Threading.Thread.Abort%2A?displayProperty=nameWithType>. Volání **Abort** v jiném vlákně je podobat vyvolání výjimky v tomto vlákně, aniž by věděl, jaký bod, který podproces dosáhl v jeho zpracování.  
...
- Zajistěte, aby vlákno, které se zobrazilo v monitorovacím nástroji, jej opustilo vždy, i pokud dojde k výjimce během zobrazení vlákna v monitorovacím nástroji. Příkaz [lock](../../csharp/language-reference/keywords/lock-statement.md) jazyka C# a příkaz [SyncLock](../../visual-basic/language-reference/statements/synclock-statement.md) jazyka poskytují toto chování <xref:System.Threading.Monitor.Exit%2A?displayProperty=nameWithType> automaticky a používají blok **finally,** aby bylo zajištěno, že je volána. Pokud nemůžete zajistit, že **exit** bude volána, zvažte změnu návrhu používat **Mutex**. Objekt mutex je automaticky uvolněn, pokud se ukončí vlákno, které jej vlastní.  
...
- Pro jednoduché změny stavu je třeba zvážit použití metod třídy <xref:System.Threading.Interlocked> namísto používání příkazu `lock` (`SyncLock` v jazyce Visual Basic). Příkaz `lock` je dobrý univerzální nástroj, avšak třída <xref:System.Threading.Interlocked> poskytuje lepší výkon aktualizací, které musí být atomické. Pokud neexistují žádné kolize, provede interně jedinou předponu zámku. V revizích kódu hledejte stejný kód, jaký je uveden v následujících příkladech. V prvním příkladu je zvýšena stavová proměnná:  
    ...
    > [!NOTE]
    > V rozhraní .NET Framework 2.0 <xref:System.Threading.Interlocked.Add%2A> a novější použijte metodu pro atomické přírůstky větší než 1.  
    ...
    > [!NOTE]
    > Počínaje rozhraním .NET Framework <xref:System.Threading.Interlocked.CompareExchange%60%601%28%60%600%40%2C%60%600%2C%60%600%29> 2.0 poskytuje přetížení metody alternativu pro typy odkazů.

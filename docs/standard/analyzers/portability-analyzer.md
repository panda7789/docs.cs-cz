...
Po převedení projektu na cílení na novou platformu, jako je například .NET Core, můžete pomocí [nástroje Roslyn API Analyzer](api-analyzer.md) identifikovat rozhraní API, které vyvolává výjimky <xref:System.PlatformNotSupportedException> a další problémy s kompatibilitou.
...
V části Souhrn přenositelnosti sestavy se zobrazuje procento přenositelnosti pro každé sestavení zahrnuté v běhu. V předchozím příkladu je 71,24% .NET Framework rozhraní API používaných v aplikaci `svcutil` dostupné v rozšířeních .NET Core + Platform. Pokud spustíte nástroj Analyzátor přenositelnosti .NET pro více sestavení, musí mít každé sestavení řádek v sestavě souhrn přenositelnosti.

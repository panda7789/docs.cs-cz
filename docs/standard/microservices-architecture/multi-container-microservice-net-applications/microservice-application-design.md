...
Aplikace se skládá z více subsystémů, včetně několika úložiště uživatelského rozhraní front-endů (webová aplikace a nativní mobilní aplikace), spolu s back-end mikroslužeb a kontejnerů pro všechny požadované operace na straně serveru. 1 obrázek 8 ukazuje architekturu referenční aplikace.

![](./media/image1.png)

**Obrázek 8-1**. Aplikaci eShopOnContainers odkazovat na aplikaci zobrazující přímá komunikace klienta mikroslužeb a událostí Service bus

**Hostitelské prostředí**. Obrázek 8-1, se zobrazí několik kontejnerů, které jsou nasazené v rámci jednoho hostitele Dockeru. Který by tomu bylo při nasazování pomocí dockeru na jednoho hostitele Docker-compose up příkazu. Ale pokud jste pomocí orchestrátoru nebo clusteru služby container, každý kontejner může být spuštěn v jiného hostitele (uzel) a libovolný uzel může používat libovolný počet kontejnerů, jak jsme je vysvětleno výše v části architektura.
...
-   Přímá komunikace klienta mikroslužeb. Používá se pro dotazy a při přijetí aktualizací nebo transakční příkazy z klientských aplikací.
...
Aplikace je nasazená jako sada mikroslužeb v podobě kontejnerů. Klientské aplikace můžou komunikovat s těchto kontejnerů také komunikace mezi mikroslužbami. Jak už bylo zmíněno, tato počáteční architektura používá architekturu přímá komunikace klienta mikroslužeb, která znamená, že klientská aplikace může provádět požadavky jednotlivých mikroslužeb přímo. Každá mikroslužba je veřejný koncový bod jako https://servicename.applicationname.companyname. V případě potřeby jednotlivých mikroslužeb můžete použít jiný port TCP. V produkčním prostředí, která umožní mapování adresy URL pro nástroj pro vyrovnávání zatížení mikroslužeb, která distribuuje požadavky mezi instance k dispozici mikroslužeb.

**Důležitá poznámka na bránu rozhraní API vs. V aplikaci eShopOnContainers přímá komunikace.** Jak je popsáno v části architektura tohoto průvodce, architektura přímá komunikace klienta mikroslužeb může mít nevýhody, pokud vytváříte velkých a složitých aplikací založených na mikroslužbách. Ale může být dostatečné pro malou aplikaci, jako jsou v aplikaci eShopOnContainers spuštění aplikace, kde je cílem soustředit na získávání jednodušší aplikací založených na kontejnerech Dockeru a nechceme k vytvoření jedné monolitické rozhraní API brány, který může mít vliv mikroslužby vývojové autonomie.

Ale pokud chcete navrhnout rozsáhlé aplikace založené na mikroslužbách s desítky mikroslužeb, důrazně doporučujeme vezměte v úvahu vzor brány rozhraní API, jak jsme je popsáno v části architektura.
Toto rozhodnutí o architektuře nemohly být refaktorovány po uvažujete o aplikace připravené pro produkční prostředí a speciálně provedené fasády pro vzdálené klienty. S více bran vlastní rozhraní API v závislosti na provedení klientské aplikace může poskytnout výhody ve vztahu agregace různých dat na klientské aplikace navíc můžete skrýt interní mikroslužby nebo rozhraní API do klientských aplikací a autorizaci v dané jedné úrovni. 

Nicméně a jak bylo zmíněno mějte na paměti, proti velké a monolitické brány rozhraní API, která může ukončit mikroslužeb vývojové autonomie.

### <a name="data-sovereignty-per-microservice"></a>Svrchovanost dat v jednotlivých mikroslužbách

V ukázkové aplikaci jednotlivých mikroslužeb vlastní svou vlastní databázi nebo zdroj dat a každá databáze, nebo zdroj dat je nasazená jako jiný kontejner. Toto rozhodnutí o návrhu byla vytvořena pouze k tomu, aby vývojáři získat kód z Githubu, naklonovat ho a otevřete ho v sadě Visual Studio nebo Visual Studio Code. Nebo alternativně to usnadňuje kompilovat vlastní Image Dockeru pomocí rozhraní příkazového řádku .NET Core a rozhraní příkazového řádku Dockeru a pak nasaďte a spusťte je v vývojové prostředí pro Docker. V obou případech použití kontejnerů pro data zdrojů umožňuje vývojářům vytvářet a nasazovat během několika minut bez nutnosti zřizovat externí databáze nebo kterýkoli jiný zdroj dat se pevný závislostmi na infrastrukturu (cloudu nebo místně).

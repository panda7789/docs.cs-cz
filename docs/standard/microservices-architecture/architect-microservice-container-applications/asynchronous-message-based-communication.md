...
Existují dva druhy asynchronní zasílání zpráv komunikace: komunikace založená na zprávách jednoho příjemce a komunikace založená na zprávách více příjemců. V následujících částech zajišťuje informace o nich.
...
Pokud systém využívá konečné konzistence řízené událostmi, integrace, se doporučuje, že tento přístup proveden úplně jasné koncového uživatele. Systém by neměly používat přístup, který napodobuje události integrace, jako je SignalR nebo dotazování systémy od klienta. Koncový uživatel a majitel firmy musí explicitně zapojte konečnou konzistenci v systému a Uvědomte si, že v mnoha případech firmu nemá jakýkoli problém s tímto přístupem tak dlouho, dokud je explicitní.

Jak je uvedeno výše v [výzvy a řešení pro správu dat distribuovaných](#challenges-and-solutions-for-distributed-data-management) části Integrace událostí můžete použít k implementaci obchodní úlohy, které jsou rozmístěny v několika mikroslužeb. Proto bude mít konečnou konzistenci mezi těmito službami. Konečnou konzistenci transakcí se skládá z kolekce distribuovaných akcí. Na každou akci související mikroslužeb aktualizuje entita domény a publikuje jiné integrace událostí, která vyvolá další akce v rámci stejné úlohy obchodní začátku do konce.

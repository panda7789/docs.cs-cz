...
Tento článek popisuje některé hlavní problémy, které byste měli zvážit a doporučené postupy, které můžete použít při zpracování řetězců, datum a časové hodnoty a číselných hodnot v globalizovaných aplikacích.
...
Ve výchozím nastavení používá .NET řetězců v kódu Unicode. Řetězec s kódováním Unicode se skládá z nula, jeden nebo více <xref:System.Char> objektů, z nichž každý představuje jednotku kódu UTF-16. Existuje reprezentace Unicode pro téměř všechny znaky v každé znakové sady používané po celém světě.
...
### <a name="test-strings-for-equality"></a>Testování rovnosti řetězců

Pokud chcete otestovat dva řetězce pro rovnosti spíše než určit, jak je jejich srovnání v pořadí řazení, použijte <xref:System.String.Equals%2A?displayProperty=nameWithType> metoda namísto metody porovnání řetězců, jako <xref:System.String.Compare%2A?displayProperty=nameWithType> nebo <xref:System.Globalization.CompareInfo.Compare%2A?displayProperty=nameWithType>.
...
Obvykle řazené řetězce, které se mají zobrazit v uživatelském rozhraní mají být řazeny podle jazykové verze. Ve většině případů porovnávání těchto řetězců zpracováváno implicitně pomocí .NET při volání metody, která řadí řetězce, jako například <xref:System.Array.Sort%2A?displayProperty=nameWithType> nebo <xref:System.Collections.Generic.List%601.Sort%2A?displayProperty=nameWithType>. Ve výchozím nastavení jsou řetězce seřazeny pomocí konvence řazení pro aktuální jazykovou verzi. Následující příklad ukazuje rozdíl, když je pole řetězců seřazeno pomocí úmluv jazykové verze Angličtina (Spojené státy) a švédština (Švédsko) jazykovou verzi.
...
.NET používá k provedení řazení zohledňující jazykovou verzi na řetězec data tabulky. Obsah těchto tabulek, které obsahují údaje o řazení váhy a řetězci normalizace, závisí na verzi standard Unicode implementované konkrétní verzi rozhraní .NET. V následující tabulce jsou uvedeny verze Unicode implementované zadanou verzí rozhraní .NET Framework a .NET Core. Všimněte si, že tento seznam podporovaných verzí sady Unicode se vztahuje na znak porovnání a řazení. To se nevztahují na klasifikaci znaků Unicode podle kategorie. Další informace najdete v tématu v části "Řetězců a ve standardu Unicode" <xref:System.String> článku.
...
|Rozhraní .NET framework 4.5 a později na Windows 7|Standard Unicode 5.0|
|Rozhraní .NET framework 4.5 a novější na systému Windows 8 a novějších operačních systémech|Unicode 6.3.0|
|.NET core (všechny verze)|Závisí na verzi Unicode Standard podporuje základní operační systém.|

Spouští se s rozhraním .NET Framework 4.5 a ve všech verzích .NET Core, porovnání a řazení řetězců závisí na operačním systému. Rozhraní .NET Framework 4.5 a novější a systémem Windows 7 načte data ze svých tabulek, které implementují Standard Unicode 5.0. Rozhraní .NET Framework 4.5 a novější spuštěnou ve Windows 8 a novějších verzích načítá data z tabulek operačního systému, které implementují 6.3 kódování Unicode. V rozhraní .NET Core podporované verze sady Unicode závisí na příslušný operační systém. Pokud serializujete seřazená data zohledňující jazykovou verzi, můžete použít <xref:System.Globalization.SortVersion> třídu k určení, kdy je potřeba serializovaná data seřadit tak, aby byla konzistentní s využitím .NET a pořadí řazení operačního systému. Příklad najdete v tématu <xref:System.Globalization.SortVersion> třídě.
...
### <a name="serialization-and-time-zone-awareness"></a>Serializace a časové pásmo povědomí
...
### <a name="perform-date-and-time-arithmetic"></a>Provedení aritmetika data a času
...
### <a name="use-culture-sensitive-names-for-date-elements"></a>Použití názvů zohledňujících jazykovou verzi pro elementy kalendářního data
...
> [!NOTE]
> V operacích analýzy a formátování, .NET rozpozná pouze základní znaky latinky 0 až 9 (U + 0030 až U + 0039) jako číselné znaky.
...
### <a name="persist-numeric-values"></a>Zachovat číselných hodnot
...
## <a name="work-with-culture-specific-settings"></a>Práce s nastavením specifické pro jazykovou verzi

V rozhraní .NET <xref:System.Globalization.CultureInfo> třída představuje konkrétní jazykovou verzi nebo oblast. Některé vlastnosti vrátí objekty, které obsahují podrobné informace o určitém aspektu kultury:
...
- Hodnoty jednotlivých vlastností se mohou lišit mezi verzemi verze rozhraní .NET nebo operačního systému.

- .NET podporuje náhradní jazykové verze. Díky tomu je možné definovat nové vlastní jazykové verze, které doplňují stávající standardní jazykové verze nebo zcela nahrazují stávající standardní jazykovou verzi.

- V systémech Windows, může uživatel přizpůsobit nastavení specifické pro jazykovou verzi pomocí **oblast a jazyk** aplikace v Ovládacích panelech. Při vytváření instance <xref:System.Globalization.CultureInfo> objektu, můžete určit, zda odráží tyto úpravy uživatele voláním <xref:System.Globalization.CultureInfo.%23ctor%28System.String%2CSystem.Boolean%29?displayProperty=nameWithType> konstruktoru. Obvykle pro aplikace koncových uživatelů měli respektovat uživatelské předvolby tak, aby uživatel dostal data ve formátu, který očekává.

...
Další informace najdete v článku [požadavky pro .NET Core v systému Windows](../windows-prerequisites.md) .
...
1. Spusťte Visual Studio. Z řádku nabídek vyberte **soubor** > **Nový** **projekt**  > . V dialogovém okně **Nový projekt** vyberte uzel  **C# vizuálu** následovaný uzlem **.NET Core** . Pak vyberte šablonu projektu **aplikace konzoly (.NET Core)** . Do textového pole **název** zadejte "HelloWorld". Klikněte na tlačítko **OK** .

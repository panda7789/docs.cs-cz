...
* .NET Core lze také hostovat s `ICLRRuntimeHost4` rozhraním v [knihovně Mscoree. h](https://github.com/dotnet/coreclr/blob/master/src/pal/prebuilt/inc/mscoree.h). Toto rozhraní API bylo asi delší než CoreClrHost. h, takže jste se mohli setkat s jeho staršími hostiteli. Pořád funguje a umožňuje nad hostitelským procesem větší kontrolu než CoreClrHost. U většiny scénářů se ale CoreClrHost. h upřednostňuje teď z důvodu jednodušších rozhraní API.
...
Následující kroky podrobně popisují, jak pomocí `nethost` knihoven a `hostfxr` spustit modul runtime .NET Core v nativní aplikaci a volat do spravované statické metody. [Ukázka](https://github.com/dotnet/samples/tree/master/core/hosting/HostWithHostFxr) `nethost` používá hlavičku a knihovnu nainstalovanou se sadou .NET SDK [`coreclr_delegates.h`](https://github.com/dotnet/core-setup/blob/master/src/corehost/cli/coreclr_delegates.h) a kopie souborů a [`hostfxr.h`](https://github.com/dotnet/core-setup/blob/master/src/corehost/cli/hostfxr.h) v úložišti [dotnet/Core-Setup](https://github.com/dotnet/core-setup) .
...
Knihovna se našla pomocí `get_hostfxr_path`. `hostfxr` Pak se načte a jeho exporty se načtou.
...
Po nalezení se knihovna načte pomocí `LoadLibraryEx` (ve Windows) nebo `dlopen` (na Linux/Mac).
...
* `coreclr_shutdown_2`: Například `coreclr_shutdown`, ale také načte ukončovací kód spravovaného kódu.

Po načtení knihovny CoreCLR je dalším krokem získat odkazy na tyto funkce pomocí `GetProcAddress` (ve Windows) nebo `dlsym` (na Linux/Mac).
...
* `PLATFORM_RESOURCE_ROOTS`Tento seznam obsahuje cesty ke sondě pro satelitní sestavení prostředků (v podadresářích specifických pro jazykovou verzi).
...
Vzhledem k tomu, že je ukázka jednoduchá, potřebuje `TRUSTED_PLATFORM_ASSEMBLIES` pouze vlastnost:
...
Na rozdíl od rozhraní API pro hostování aplikace Mscoree. h (popsané níže), rozhraní API CoreCLRHost. h spustí modul runtime a vytvoří výchozí doménu AppDomain s jediným voláním. Funkce přebírá základní cestu, název a vlastnosti popsané dříve a vrací zpětný popisovač do hostitele `hostHandle` přes parametr. `coreclr_initialize`
...
V této ukázce může hostitel nyní zavolat `managedDelegate` ke `ManagedWorker.DoWork` spuštění metody.

Alternativně lze `coreclr_execute_assembly` funkci použít ke spuštění spravovaného spustitelného souboru. Toto rozhraní API přebírá cestu sestavení a pole argumentů jako vstupní parametry. Načte sestavení na této cestě a vyvolá jeho metodu Main.
...
Rozhraní API modulu runtime .NET Core jsou v *CoreCLR. dll* (ve Windows). Chcete-li získat naše hostující`ICLRRuntimeHost4`rozhraní (), je nutné najít a načíst *CoreCLR. dll*. Pro definování konvence, jak bude vyhledána *Knihovna CoreCLR. dll*, je k dis na hostitele. Někteří hostitelé očekávají, že se soubor nachází v dobře známém umístění v rámci počítače (například *%ProgramFiles%\dotnet\shared\Microsoft.NETCore.App\2.1.6*). Ostatní očekávají, že *CoreCLR. dll* se načte z umístění vedle samotného hostitele nebo aplikace, která se má hostovat. I když ostatní můžou najít knihovnu v proměnné prostředí.

V systému Linux nebo Mac je základní běhová knihovna *libcoreclr.so* nebo *libcoreclr. DYLIB*, v uvedeném pořadí.

Náš Ukázkový hostitel sonduje několik běžných umístění pro *CoreCLR. dll*. Po nalezení se musí načíst přes `LoadLibrary` (nebo `dlopen` na Linux/Mac).
...
Hostující rozhraní je načteno voláním `GetProcAddress` (nebo `dlsym` na Linux/Mac) na `GetCLRRuntimeHost`a následně vyvoláním této funkce. `ICLRRuntimeHost4`
...
* `TRUSTED_PLATFORM_ASSEMBLIES`Toto je seznam cest sestavení (oddělený `;` systémem Windows a `:` na platformě Linux/Mac), ve kterých by měla doména AppDomain upřednostňovat načítání a poskytovat úplný vztah důvěryhodnosti (i v částečně důvěryhodných doménách). Tento seznam má obsahovat sestavení architektury a jiné důvěryhodné moduly, podobně jako globální mezipaměť sestavení (GAC) v .NET Frameworkch scénářích. Někteří hostitelé vloží do tohoto seznamu všechny knihovny v *CoreCLR. dll* , ostatní mají pevně kódované manifesty, které uvádějí důvěryhodná sestavení pro jejich účely.
* `APP_PATHS`Toto je seznam cest k testům pro sestavení, pokud jej nelze najít v seznamu důvěryhodných platforem sestavení (TPA). Vzhledem k tomu, že hostitel má větší kontrolu nad tím, která sestavení jsou načtena pomocí seznamu TPA, je osvědčeným postupem pro hostitele k určení, která sestavení chtějí načíst a jejich seznam explicitně. Pokud je tato vlastnost potřebná ke zjišťování za běhu, může tento scénář povolit.
* `APP_NI_PATHS`Tento seznam se velmi podobá APP_PATHS s tím rozdílem, že se jedná o cesty, které budou zjišťovány pro nativní bitové kopie.
...
Pokud co nejdříve nefunguje, poklikejte na to, že *CoreCLR. dll* je k dispozici v umístění očekávaném hostitelem, že všechny nezbytné knihovny rozhraní jsou v seznamu TPA a že CoreCLR bitová verze (32-nebo 64) odpovídá sestavení hostitele.

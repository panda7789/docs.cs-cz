...
\*Při použití sady SDK verze 3,0 je spustitelný soubor závislý na architektuře výchozím režimem publikování při spuštění příkazu `dotnet publish` Basic. To platí pouze v případě, že projekt cílí na **rozhraní .NET core 2,1** nebo **.NET Core 3,0**.
...
Není-li jinak nastaveno, je [`dotnet publish`](../tools/dotnet-publish.md) `./bin/<BUILD-CONFIGURATION>/<TFM>/publish/`výstupní adresář příkazu. Výchozí režim **konfigurace sestavení** je **ladění** , pokud se `-c` nezmění s parametrem. Například `dotnet publish -c Release -f netcoreapp2.1` publikování na `myfolder/bin/Release/netcoreapp2.1/publish/`.
...
Soubor `Program.cs` nebo`Program.vb` , který je generován šablonou konzoly, je třeba změnit na následující:
...
Při publikování aplikace jako FDD `<PROJECT-NAME>.dll` se `./bin/<BUILD-CONFIGURATION>/<TFM>/publish/` ve složce vytvoří soubor. Chcete-li spustit aplikaci, přejděte do výstupní složky a použijte `dotnet <PROJECT-NAME>.dll` příkaz.
...
Pro .NET Core SDK 3. x CLI, spustitelný soubor závislý na rozhraní (FDE) je výchozím režimem pro základní `dotnet publish` příkaz. Není nutné zadávat žádné další parametry, pokud chcete cílit na aktuální operační systém.
...
Je nutné (kromě .NET Core 3. x při cílení na aktuální platformu) použít k publikování FDE s `dotnet publish` příkazem následující přepínače:
...
K publikování SCD je nutné použít následující přepínače `dotnet publish` s příkazem:

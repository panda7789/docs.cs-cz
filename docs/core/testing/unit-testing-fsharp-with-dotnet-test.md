...
Nastavte *MathService* na aktuální adresář a spusťte `dotnet new classlib -lang "F#"` pro vytvoření zdrojového projektu. Vytvoříte nefunkční implementaci služby Math Service:
...
Testovací projekt vyžaduje pro vytvoření a spuštění testů jednotek další balíčky. `dotnet new` v předchozím kroku jsme přidali xUnit a xUnit Runner. Nyní přidejte knihovnu tříd `MathService` jako jinou závislost do projektu. Použijte příkaz `dotnet add reference`:
...
Test se nezdařil. Ještě jste nevytvořili implementaci. Proveďte tento test průchodu tak, že napíšete nejjednodušší kód ve třídě `MathService`, která funguje:
...
Spuštění `dotnet test` spustí testy a ukáže vám, že nový test se nezdařil. Nyní aktualizujte metodu `squaresOfOdds` pro zpracování tohoto nového testu. Vyfiltrujete všechna sudá čísla mimo sekvenci k provedení tohoto testu. Můžete to udělat tak, že napíšete malou funkci filtru a použijete `Seq.filter`:

...
> [!IMPORTANT]
> Je nutné na Přejít **IP adresu kontejneru** (nikoli `http://localhost`) ve svém prohlížeči přímo při používání kontejnerů Windows. Můžete získat IP adresu vašeho kontejneru pomocí následujících kroků:

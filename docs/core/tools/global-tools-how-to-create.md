...
1. Nahraďte `Main` metodu následujícím kódem pro zpracování argumentů příkazového řádku pro aplikaci.
   ...
   Pokud nejsou předány žádné argumenty, zobrazí se krátká zpráva help. V opačném případě jsou všechny argumenty zřetězeny do jednoho řetězce a vytištěny voláním `ShowBot` metody, kterou vytvoříte v dalším kroku.
...
1. Otevřete soubor *Microsoft. botsay. csproj* a přidejte na konec uzlu tři nové uzly XML `<PropertyGroup>` :
...
1. Vytvořte balíček NuGet spuštěním příkazu [dotnet Pack](dotnet-pack.md) :
   ...
   Vytvoří se soubor *Microsoft. botsay. 1.0.0. nupkg* ve složce určené `<PackageOutputPath>` hodnotou ze souboru *Microsoft. botsay. csproj* , který je v tomto příkladu složkou *./nupkg* .
  
   Pokud chcete veřejně vydávat nástroj, můžete ho nahrát do `https://www.nuget.org` . Jakmile je nástroj k dispozici v NuGet, mohou vývojáři nainstalovat nástroj pomocí příkazu pro [instalaci nástroje dotnet](dotnet-tool-install.md) . V tomto kurzu nainstalujete balíček přímo z místní složky *nupkg* , takže není potřeba balíček nahrát do NuGet.

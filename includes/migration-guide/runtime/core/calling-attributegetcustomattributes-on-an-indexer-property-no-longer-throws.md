...
---
### <a name="calling-attributegetcustomattributes-on-an-indexer-property-no-longer-throws-ambiguousmatchexception-if-the-ambiguity-can-be-resolved-by-indexs-type"></a>Volání Attribute.GetCustomAttributes na vlastnost indexeru již nevyvolá AmbiguousMatchException, pokud lze nejednoznačnost vyřešit typem indexu

|   |   |
|---|---|
|Podrobnosti|Před rozhraním .NET Framework 4.6 by volání <code>GetCustomAttribute(s)</code> na vlastnost indexeru, která se lišila <xref:System.Reflection.AmbiguousMatchException?displayProperty=name>od jiné vlastnosti pouze typem indexu, mělo za následek . Počínaje rozhraním .NET Framework 4.6 budou atributy vlastnosti vráceny správně.|
|Návrh|Uvědomte si, že GetCustomAttribute(s) bude fungovat častěji nyní. Pokud aplikace byla dříve spoléhat na <xref:System.Reflection.AmbiguousMatchException?displayProperty=name>, reflexe by nyní použít k explicitně vyhledat více indexerů, místo toho.|
|Rozsah|Edge|
|Version|4.6|
|Typ|Modul runtime|

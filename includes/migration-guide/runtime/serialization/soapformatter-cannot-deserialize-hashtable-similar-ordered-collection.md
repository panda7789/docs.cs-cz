...
|   |   |
|---|---|
|Podrobnosti|<xref:System.Runtime.Serialization.Formatters.Soap.SoapFormatter?displayProperty=name> Nemá není záruku, že objekty serializované do jedné verze rozhraní .NET Framework bude úspěšně deserializovat pod jinou verzi. Konkrétně některé seřazené kolekce (jako je <xref:System.Collections.Hashtable?displayProperty=name>) přidat členy mezi 4.0 a 4.5 tak, aby objekty z těchto typů nelze deserializovat pomocí rozhraní .NET Framework 4.0, pokud byly serializovat s příznakem rozhraní .NET Framework 4.5. Všimněte si, že pokud serializovaná data jak serializovat a deserializovat se stejnou verzí rozhraní .NET Framework, žádné problému.|

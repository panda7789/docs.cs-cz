...
`SignInAsync` akceptuje všechny objekty zabezpečení a identity, včetně identit, ve kterých `IsAuthenticated` `false`.
...
Původní chování bylo problematické, protože ve výchozím nastavení se tyto objekty zabezpečení zamítly `[Authorize]` @ no__t-1 @ no__t-2.

#### <a name="recommended-action"></a>Doporučená akce

V ASP.NET Core 3,0 Preview 6 existuje příznak `RequireAuthenticatedSignIn` v `AuthenticationOptions`, který je ve výchozím nastavení `true`. Nastavením tohoto příznaku `false` obnovte původní chování.

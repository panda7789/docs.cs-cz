...
---
### <a name="parameter-names-changed-in-reference-assemblies"></a>Názvy parametrů se změnily v referenčních sestaveních.
...
V předchozích verzích rozhraní .NET se některé názvy parametrů [sestavení odkazů](../../../../docs/standard/assembly/reference-assemblies.md) liší podle jejich odpovídajících parametrů v sestavení implementace. To může způsobit problémy při použití pojmenovaných argumentů a reflexe.

V rozhraní .NET 5,0 byly tyto neshodné názvy parametrů aktualizovány v referenčních sestaveních tak, aby přesně odpovídaly názvům odpovídajících parametrů v sestaveních implementace.

Následující tabulka uvádí názvy rozhraní API a parametrů, které se změnily.
...
Názvy parametrů se změnily na konzistenci a aby se předešlo chybám při použití pojmenovaných argumentů a reflexe.
...
Pokud dojde k chybě kompilátoru z důvodu změny názvu parametru, aktualizujte odpovídající název parametru.
